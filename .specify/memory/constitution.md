...
| `Elsa.Workflows.ExecutionEvidence` *(Draft #1133 amendment; reviewed for implementation readiness on 2026-08-05; remains unratified)* | Captures, stores, and exposes committed workflow-runtime facts for deterministic verification without teaching Runtime evidence vocabulary. | `Elsa.Workflows.ExecutionEvidence.Core` (contracts only), `Elsa.Workflows.ExecutionEvidence` (provider-neutral session/capture/Runtime-adapter base), `Elsa.Workflows.ExecutionEvidence.InMemory` (explicit process-local provider leaf), and `Elsa.Workflows.ExecutionEvidence.Api` (transport). The API feature may legally inherit the base feature registration and must call `base.ConfigureServices`; it depends on Core/base, never the InMemory leaf. |
