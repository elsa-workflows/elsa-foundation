...
git merge-base --is-ancestor "$source_commit" "$capture_runner_commit" || {
...
git -C "$repo_root" archive "$capture_runner_commit" tools/compatibility/capture-workflows-design-before.sh | tar -x -C "$worktree_dir"
for dependency in \
