...
public sealed class QueueDrainAdapterTests
{
    [Fact]
    public async Task Runs_frozen_correctness_and_prepares_operations_over_real_sqlite_runtime_stores()
    {
        var root = Path.Combine(Path.GetTempPath(), $"groundwork-queue-adapter-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            await using var adapter = new QueueDrainAdapter(request, connectionString, root);
            ...
            Assert.Equal(RuntimeQueueDrainWorkload.ExpectedResultDigest, correctness.ObservedResultDigestSha256);
            ...
            Assert.Equal(
                ReproducibleWorkloadScenarioCatalog.Get(RuntimeQueueDrainWorkload.WorkloadId).OperationSequence,
    ...
    private static RunRequest Request() => new(
        ...
        WorkloadId: RuntimeQueueDrainWorkload.WorkloadId,
        ...
        PhysicalForm: QueueDrainAdapter.PhysicalForm,
        Scale: "small",
        ...
        Seed: RuntimeQueueDrainWorkload.Seed,
        InputFingerprintSha256: RuntimeQueueDrainWorkload.ExpectedInputFingerprint,
        NativePlanIdentity: "queue-drain-test-provenance",
        NativePlanEvidenceReference: "queue-drain.sqlite.set.native-plan.json",
