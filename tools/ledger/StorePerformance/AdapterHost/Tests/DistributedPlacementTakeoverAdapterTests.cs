...
public sealed class DistributedPlacementTakeoverAdapterTests
{
    [Fact]
    public async Task Runs_frozen_correctness_and_prepares_operations_over_real_sqlite_runtime_stores()
    {
        var root = Path.Combine(Path.GetTempPath(), $"groundwork-placement-adapter-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        var database = Path.Combine(root, "placement.db");
        ...
        try
        {
            ...
            await using var adapter = new DistributedPlacementTakeoverAdapter(request, connectionString, root);
            ...
            Assert.Equal(DistributedPlacementTakeoverWorkload.ExpectedResultDigest, correctness.ObservedResultDigestSha256);
    ...
    private static RunRequest Request() => new(
        ...
        WorkloadId: DistributedPlacementTakeoverWorkload.WorkloadId,
        ...
        PhysicalForm: DistributedPlacementTakeoverAdapter.PhysicalForm,
        Scale: "small",
        ...
        Seed: DistributedPlacementTakeoverWorkload.Seed,
        InputFingerprintSha256: DistributedPlacementTakeoverWorkload.ExpectedInputFingerprint,
        NativePlanIdentity: "placement-test-provenance",
        NativePlanEvidenceReference: "placement-takeover.sqlite.set.native-plan.json",
