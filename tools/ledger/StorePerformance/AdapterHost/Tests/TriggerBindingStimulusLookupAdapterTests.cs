...
public sealed class TriggerBindingStimulusLookupAdapterTests
{
    [Fact]
    public async Task Runs_frozen_correctness_and_prepares_operations_over_real_sqlite_runtime_stores()
    {
        var root = Path.Combine(Path.GetTempPath(), $"groundwork-trigger-adapter-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        var database = Path.Combine(root, "trigger.db");
        ...
        try
        {
            ...
            await using var adapter = new TriggerBindingStimulusLookupAdapter(request, connectionString, root);
            ...
            Assert.Equal(RuntimeTriggerBindingStimulusLookupWorkload.ExpectedResultDigest, correctness.ObservedResultDigestSha256);
    ...
    private static RunRequest Request() => new(
        ...
        WorkloadId: RuntimeTriggerBindingStimulusLookupWorkload.WorkloadId,
        ...
        PhysicalForm: TriggerBindingStimulusLookupAdapter.PhysicalForm,
        Scale: "small",
        ...
        Seed: RuntimeTriggerBindingStimulusLookupWorkload.Seed,
        InputFingerprintSha256: RuntimeTriggerBindingStimulusLookupWorkload.ExpectedInputFingerprint,
        NativePlanIdentity: "trigger-binding-test-provenance",
        NativePlanEvidenceReference: "trigger-binding.sqlite.set.native-plan.json",
