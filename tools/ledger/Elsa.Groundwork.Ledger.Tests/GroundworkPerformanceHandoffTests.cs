...
""";
    ...
    [Fact]
    public void Operator_runner_delegates_evidence_admission_to_the_harness()
    {
        // Issue #1593: the admission rules have exactly one implementation, NativePlanEvidenceAdmission,
        // reached through the AdapterHost admit-evidence command. The runner may orchestrate, not judge.
        var runner = File.ReadAllText(Path.Combine(RepoRoot, "tools", "groundwork", "run-e3-medium-baseline.py"));

        Assert.Contains("\"admit-evidence\"", runner, StringComparison.Ordinal);
        Assert.Contains("--require-complete", runner, StringComparison.Ordinal);
        foreach (var rule in new[] { "RawPlanReference", "TraceDetailConstituents", "RouteContract\"] ==", "HasStorageScopePredicate", "SAFE_RAW_PLAN_REFERENCE" })
            Assert.DoesNotContain(rule, runner, StringComparison.Ordinal);
    }
