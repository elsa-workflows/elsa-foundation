...
default_feature_keys() {
  local file="$repo_root/src/Server/appsettings.json"
  [ -f "$file" ] || return 0
  awk '
...

{
  ...
  echo
  echo 'Records CShells feature identity, public feature properties, and dependency evidence from direct project/package references. It does not generate appsettings JSON and does not treat `src/Server` composition choices as canonical.'
  echo
  echo "## Summary"
  echo
  ...
  echo '- IConfiguration feature-registration shape observed from: `src/Server/appsettings.json`'
  echo
  echo "## IConfiguration Shape Evidence"
  echo
  echo 'The test shell is not a composition source of truth. The only evidence taken from `src/Server/appsettings.json` is the JSON shape: `CShells:Shells:{shellName}:Features:{featureId}` with optional nested feature settings such as `Options`, plus shell-level `Configuration`.'
  echo
