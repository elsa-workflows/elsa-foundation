...
function Write-TestMap {
    ...
    foreach ($test in $testProjects) {
        ...
        $sourceFiles = if ($files.Count -eq 0) { "-" } else { $files -join "<br>" }
        $lines += "| $($test.Name) | $(Escape-Cell $sourceFiles) |"
    }
