...
`elsa-foundation` is the .NET 10 workflow engine "Elsa 4": a set of libraries under `src/essentials` that a host
...
- **Design** (`src/essentials/Workflows/Design`, `src/essentials/Activities/Design`): authoring. Definitions, drafts and
  immutable versions, plus the activity catalog and validations. Nothing here runs a workflow.
- **Publishing** (`src/essentials/Workflows/Publishing`): compiles a definition version into a content-addressed
...
SQLite through `ConnectionStrings:Elsa`; each module can override its provider and connection.
