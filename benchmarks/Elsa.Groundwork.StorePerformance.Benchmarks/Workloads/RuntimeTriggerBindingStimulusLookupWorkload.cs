...
public sealed class RuntimeTriggerBindingStimulusLookupWorkload
{
    ...
    private static SeededScope CreateExpectedScope(string logicalScope)
    {
        ...
        var bindingsByPublication = Enumerable.Range(0, PublicationCount)
            .ToDictionary(
                publication => PublicationId(logicalScope, publication),
                publication => (IReadOnlyList<WorkflowTriggerBinding>)CreateBindings(logicalScope, publication, PublicationId(logicalScope, publication))
                    .Select(binding => binding with { IsActive = publication != PublicationCount - 1 })
                    .OrderBy(binding => binding.TriggerBindingId, StringComparer.Ordinal)
                    .ToArray(),
                StringComparer.Ordinal);
