...
/// </summary>
public sealed class ListConstructableActivitiesRequestHandler(IQueries<ActivityDefinitionVersion> versions)
    : IRequestHandler<ListConstructableActivities, IEnumerable<ConstructableActivityView>>
{
    ...
    {
        var rows = await versions.List(cancellationToken);
