...
The Groundwork composition (`Elsa.Workflows.Publishing.Persistence.Groundwork`) replaces the two publishing-owned
authority stores (`IPublicationRecordStore`, `IPublicationPolicyStore`) and `IActivityPublicationReceiptStore`.
It must preserve one immutable request fingerprint per tenant-owned idempotency key; receipt lookup must use the
request authorization context's tenant scope and must not depend on the continued existence of the source
draft. An Applied receipt is written by
...
- **`IPublicationProjectionIntentStore` no longer exists either** (spec 151, T122). It was the delivery-intent
  ledger of the projection reconciler ADR 0043 designed; T121 deleted that reconciler and
  `IWorkflowActivationCoordinator`, which replaced it, carries no such ledger — its prepare/activate/compensate
  sequence is in-process and its recovery is the next activation request. Publishing therefore keeps no record
  of projection delivery, and the `publishingProjectionIntent` storage unit is gone with it.

The supplied Groundwork provider implements these two contracts. Compose
...
**Publishing owns no projection preparer.** `IPublicationProjectionPreparer` and its
`PublicationProjectionReconciler` default are deleted (spec 151, T121): preparing, activating, removing and
restoring the serving projections belong to `IWorkflowActivationCoordinator`, in **both** lifecycle directions —
publishing's unpublish handler calls its `DeactivateAsync` rather than driving a retraction of its own. Two
components that had to know the same ordering invariant independently is exactly how they drifted apart once, so
there is now one. Derived projection notifications still occur only after the durable serving set reaches its
final state; Runtime HTTP consumes the neutral `IWorkflowTriggerIndexObserver` seam and performs a full refresh
when authority changes. See the [Runtime catalog](../Runtime/EXTENSION_POINTS.md) for the full sequence.
...
1. Implement the two publishing-owned authority stores (`IPublicationRecordStore`, `IPublicationPolicyStore`)
   plus `IActivityPublicationReceiptStore` and register them as one composition unit. **The activation slot is
   not among them**: it is `IWorkflowActivationAuthority`, backed by the *runtime* Groundwork store family, and
   a publishing persistence package must neither implement nor register it. **Nor is a projection-delivery
   ledger** — there is none to back (see Authority stores above).
2. Enforce unique slot identity and compare-and-swap revisions in storage, not only in process memory.
3. Index publication records by slot.
4. Version wire documents and provide upcasters/fixtures for every prior version.
5. Prove restart behavior, stale-revision rejection, and compensation with provider tests.
