...
- **Usage:** after the trigger indexer rewrites an artifact's bindings, this observer rebuilds the whole route table from `IHttpEndpointRoutesResolver` **when the publish could change the HTTP route set** (full re-projection, not an incremental diff — republish is delete-and-resave, and the durable index is the source of truth). Runs in a fresh DI scope (the indexer is a shell singleton; the resolver and `IRouteTable` are scoped, and the route table's state lives in the shared memory cache). An exception fails the publish, per the seam's failure policy. **HTTP-affecting gate (#592 item 8):** the observer skips the refresh for a publish whose snapshot `Bindings` contain no HTTP-endpoint binding *and* whose artifact it has not previously seen contribute one — a non-HTTP workflow can't change the route set, so it shouldn't pay for the (full-kind-scan) re-projection. It tracks per-artifact HTTP contribution in a process-local set that only widens under a refresh, so the removal case (an artifact dropping its last HTTP route) still refreshes to reconcile the vanished route out; the startup task rebuilds the whole table on shell start regardless. This is the sole consumer of `WorkflowTriggerIndexSnapshot.Bindings`.
