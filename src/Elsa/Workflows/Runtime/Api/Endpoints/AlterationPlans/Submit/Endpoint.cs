...
public sealed class Endpoint(IWorkflowAlterationPlanApiService alterations) : ApiEndpointWithResult<SubmitWorkflowAlterationPlan, WorkflowAlterationPlanSubmissionView>
{
    ...
    public override async Task<EndpointResult<WorkflowAlterationPlanSubmissionView>> HandleAsync(SubmitWorkflowAlterationPlan request, CancellationToken cancellationToken)
    {
        ...
        if (request.IdempotencyKey.Length > WorkflowAlterationPlanApiService.MaximumIdempotencyKeyLength)
        ...
        var result = await alterations.SubmitAsync(request, cancellationToken);
