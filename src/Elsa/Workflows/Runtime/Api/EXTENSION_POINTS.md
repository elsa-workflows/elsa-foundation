...
| `IActivityExecutionInspectionAuthorizationContext` / `IActivityExecutionInspectionAuthorizationContextAsync` | Separately authorizes structural inspection, redacted value evidence, and raw captured-value payload resolution. The async seam delegates to Foundation Identity, and its effective authorization profile participates in cursor binding. |
