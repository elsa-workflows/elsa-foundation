...
| `WorkflowExecutableInspector` / `IWorkflowExecutableInspector` | Assembles stable executable and provenance views from Runtime-owned stores. The endpoint classes dispatch through the interface seam; both registrations use `TryAddScoped`, so a host may replace the projection strategy. |
...
| `IActivityExecutionValuePayloadReader` | Owns the non-disclosing raw-payload resolution result contract. The feature registers only this contract; hosts may replace the implementation without coupling the endpoints to the default reader. Resolution fails closed unless the authorization context supplies a stable attributable audit subject. |
...
| Runtime diagnostics settings store and `IRuntimeDiagnosticsSettingsService` | Back the canonical diagnostics settings resource; durable or centrally managed hosts may replace the underlying settings persistence. |
