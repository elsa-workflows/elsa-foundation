...
public static class WorkflowsRuntimeApi
{
    ...
    public static void MapWorkflowsRuntimeApi(IEndpointRouteBuilder endpoints)
    {
        ...
        // T117: activation-slot reads are runtime-owned. Publishing keeps slot lifecycle mutations only, so that
        // the publishing API is not made responsible for knowing runtime-owned data.
        Map(endpoints.MapGet(RouteConstants.ActivationSlots, (RequestDelegate)ListActivationSlotsAsync), "ListActivationSlots", PermissionNames.WorkflowRuntimeRead, typeof(WorkflowActivationSlotListView), description);
        Map(endpoints.MapGet(RouteConstants.ActivationSlot, (RequestDelegate)GetActivationSlotAsync), "GetActivationSlot", PermissionNames.WorkflowRuntimeRead, typeof(WorkflowActivationSlotView), description);
    ...
    private static async Task ListActivationSlotsAsync(HttpContext context) =>
        await ExecuteRequestAsync(context, new ListWorkflowActivationSlots(Route(context, "definitionId") ?? string.Empty), WorkflowsRuntimeJsonContext.Default.WorkflowActivationSlotListView);

    private static async Task GetActivationSlotAsync(HttpContext context) =>
        await ExecuteRequestAsync(context, new GetWorkflowActivationSlot(Route(context, "definitionId") ?? string.Empty, Route(context, "slotName") ?? string.Empty), WorkflowsRuntimeJsonContext.Default.WorkflowActivationSlotView);
