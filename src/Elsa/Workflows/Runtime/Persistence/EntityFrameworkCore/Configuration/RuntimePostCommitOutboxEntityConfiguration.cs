...
public sealed class RuntimePostCommitOutboxEntityConfiguration : IEntityTypeConfiguration<RuntimePostCommitOutboxEntity>
{
    public void Configure(EntityTypeBuilder<RuntimePostCommitOutboxEntity> b)
    {
        ...
        b.Property(x => x.OutboxItemIdOrderKey).HasMaxLength(RuntimePostCommitOutboxEfModule.PhysicalIdentityOrderKeyMaximumLength).IsRequired();
        ...
        b.HasIndex(x => new { x.ScopeKeyHash, x.DeliverableAtUtcTicks, x.RecordedAtUtcTicks, x.OutboxItemIdOrderKey });
        b.HasIndex(x => new { x.ScopeKeyHash, x.WorkflowExecutionIdHash, x.DeliverableAtUtcTicks, x.RecordedAtUtcTicks, x.OutboxItemIdOrderKey });
        b.HasIndex(x => new { x.ScopeKeyHash, x.IntentKindHash, x.DeliverableAtUtcTicks, x.RecordedAtUtcTicks, x.OutboxItemIdOrderKey });
        b.HasIndex(x => new { x.ScopeKeyHash, x.ClaimableIsEligible, x.ClaimableAtUtcTicks, x.RecordedAtUtcTicks, x.OutboxItemIdOrderKey });
    }
}
