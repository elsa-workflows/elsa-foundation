    ...
    IRuntimeRecoveryContinuationCodec continuationCodec) : IWorkflowExecutionStateStore
{
    ...
    private HistoryCursor? DecodeHistoryCursor(string? token, WorkflowExecutionStatePageQuery query, string scope)
    {
        ...
        try { var cursor = JsonSerializer.Deserialize<HistoryCursor>(Encoding.UTF8.GetString(continuationCodec.Decode(HistoryCursorPurpose, token))) ?? throw new FormatException(); if (cursor.Version != 1 || cursor.ScopeHash != Hash(scope) || cursor.FilterScope != WorkflowExecutionStateHistory.Scope(query) || cursor.OrderKey != OrderKey(cursor.ExecutionId)) throw new FormatException(); return cursor; }
        catch (Exception ex) when (ex is ArgumentException or FormatException or JsonException or InvalidDataException) { throw new ArgumentException("The workflow execution history cursor is invalid or does not belong to this query.", nameof(token), ex); }
    }
    private string EncodeHistoryCursor(WorkflowExecutionStateEntity row, WorkflowExecutionStatePageQuery query, string scope) => continuationCodec.Encode(HistoryCursorPurpose, Encoding.UTF8.GetBytes(JsonSerializer.Serialize(new HistoryCursor(1, Hash(scope), WorkflowExecutionStateHistory.Scope(query), Decode(row.WorkflowExecutionId), row.SortTimestampUtcTicks, row.WorkflowExecutionIdOrderKey))));
    private CaptureCursor? DecodeCaptureCursor(string? token, WorkflowExecutionAlterationCaptureQuery query, string scope)
    {
        if (token is null) return null;
        try { var cursor = JsonSerializer.Deserialize<CaptureCursor>(Encoding.UTF8.GetString(continuationCodec.Decode(CaptureCursorPurpose, token))) ?? throw new FormatException(); if (cursor.Version != 1 || cursor.ScopeHash != Hash(scope) || cursor.FilterScope != CaptureScope(query) || cursor.OrderKey != OrderKey(cursor.ExecutionId)) throw new FormatException(); return cursor; }
        catch (Exception ex) when (ex is ArgumentException or FormatException or JsonException or InvalidDataException) { throw new ArgumentException("The alteration capture cursor is invalid or does not belong to this query.", nameof(token), ex); }
    }
    private string EncodeCaptureCursor(WorkflowExecutionStateEntity row, WorkflowExecutionAlterationCaptureQuery query, string scope) => continuationCodec.Encode(CaptureCursorPurpose, Encoding.UTF8.GetBytes(JsonSerializer.Serialize(new CaptureCursor(1, Hash(scope), CaptureScope(query), Decode(row.WorkflowExecutionId), row.WorkflowExecutionIdOrderKey))));
    private static string CaptureScope(WorkflowExecutionAlterationCaptureQuery query) => Convert.ToHexString(SHA256.HashData(JsonSerializer.SerializeToUtf8Bytes(new { query.TenantPartition, query.SystemIdentity, query.RootInitiator, query.AuthorityPartitionKey, Metadata = query.AuthorityMetadata.OrderBy(x => x.Key, StringComparer.Ordinal), query.Selector.DefinitionId, query.Selector.Status, query.Selector.RunKind, From = query.Selector.From?.UtcTicks, To = query.Selector.To?.UtcTicks, query.Selector.CorrelationId, query.Selector.WorkflowExecutionId, query.Selector.ArtifactId, query.Selector.MatchAllAuthorized })));
