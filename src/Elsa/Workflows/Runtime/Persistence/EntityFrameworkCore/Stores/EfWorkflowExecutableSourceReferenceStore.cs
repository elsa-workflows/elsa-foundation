    ...
    IPersistenceAccessContextAccessor access) : IWorkflowExecutableSourceReferenceStore
{
    public async ValueTask SaveAsync(WorkflowExecutableSourceReference reference, CancellationToken cancellationToken = default)
    {
        ...
        var existing = await context.WorkflowExecutableSourceReferences.AsNoTracking().SingleOrDefaultAsync(x => x.Id == reference.SourceReferenceId, cancellationToken);
        ...
        context.WorkflowExecutableSourceReferences.Add(ToEntity(reference, scope));
    ...
    public async ValueTask<WorkflowExecutableSourceReference?> FindAsync(string sourceReferenceId, CancellationToken cancellationToken = default)
    {
        ...
        var scope = ScopeForRead();
        var row = await context.WorkflowExecutableSourceReferences.AsNoTracking().SingleOrDefaultAsync(x => x.Id == sourceReferenceId, cancellationToken);
    ...
    private async ValueTask<RuntimeStorePage<WorkflowExecutableSourceReference>> Page(RuntimeStorePageRequest request, Route route, string? artifact, string? definition, WorkflowExecutableSourceReferencePageQuery? all, CancellationToken cancellationToken)
    {
        ...
        var scope = ScopeForRead(); var binding = artifact ?? definition ?? (all?.Scope?.ToString() ?? "all");
        ...
        if (scope is not null) query = query.Where(x => x.ScopeKeyHash == EfRelationalIdentity.Hash(scope) && x.ScopeKey == EfRelationalIdentity.Encode(scope));
        if (artifact is not null) query = query.Where(x => x.ArtifactIdHash == EfRelationalIdentity.Hash(artifact) && x.ArtifactId == artifact);
        if (definition is not null) query = query.Where(x => x.DefinitionVersionIdHash == EfRelationalIdentity.Hash(definition) && x.DefinitionVersionId == definition);
        if (all?.Scope is { } sourceScope) query = query.Where(x => x.Scope == sourceScope.ToString());
        if (all?.LiveOnly == true) query = query.Where(x => !x.IsRetired && x.ExpiresAtUtcTicks > all.Now!.Value.UtcTicks);
        if (cursor is not null) query = query.Where(x => string.CompareOrdinal(x.SourceReferenceIdOrderKey, cursor.Key) > 0);
        var rows = await query.OrderBy(x => x.SourceReferenceIdOrderKey).Take(request.Limit + 1).ToArrayAsync(cancellationToken);
        var hasMore = rows.Length > request.Limit; if (hasMore) rows = rows[..request.Limit];
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(sourceReferenceId); var scope = ScopeForRead();
        var row = await context.WorkflowExecutableSourceReferences.SingleOrDefaultAsync(x => x.Id == sourceReferenceId, cancellationToken);
        if (row is null) return false; var current = Read(row, scope, sourceReferenceId); if (current.DeletedAt is not null) return true;
        Copy(row, current.Retire(deletedAt, reason), scope ?? EfRelationalIdentity.Decode(row.ScopeKey));
        try { await context.SaveChangesAsync(cancellationToken); return true; } catch (DbUpdateConcurrencyException) { context.ChangeTracker.Clear(); return false; }
    }
    ...
    private async ValueTask<bool> TryReplace(WorkflowExecutableSourceReference expected, WorkflowExecutableSourceReference replacement, bool restore, CancellationToken cancellationToken)
    {
        ...
        var scope = ScopeForRead(); var row = await context.WorkflowExecutableSourceReferences.SingleOrDefaultAsync(x => x.Id == expected.SourceReferenceId, cancellationToken);
        if (row is null) return false; var current = Read(row, scope, expected.SourceReferenceId); if (!WorkflowExecutableSourceReferenceComparer.SameSnapshot(current, expected)) return false;
        Copy(row, replacement, scope ?? EfRelationalIdentity.Decode(row.ScopeKey));
        try { await context.SaveChangesAsync(cancellationToken); return true; } catch (DbUpdateConcurrencyException) { context.ChangeTracker.Clear(); return false; }
    }
    ...
    public async ValueTask<bool> DeleteAsync(string sourceReferenceId, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(sourceReferenceId); var scope = ScopeForRead();
        var row = await context.WorkflowExecutableSourceReferences.SingleOrDefaultAsync(x => x.Id == sourceReferenceId, cancellationToken);
        if (row is null) return false; _ = Read(row, scope, sourceReferenceId); context.Remove(row);
        try { await context.SaveChangesAsync(cancellationToken); return true; } catch (DbUpdateConcurrencyException) { context.ChangeTracker.Clear(); return false; }
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(batch); var scope = ScopeForRead();
        ...
        foreach (var row in rows) { var current = Read(row, scope, row.SourceReferenceId); if (await DeleteAsync(current.SourceReferenceId, cancellationToken)) deleted.Add(current.SourceReferenceId); }
        return deleted;
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(candidates); var scope = ScopeForRead();
        var query = context.WorkflowExecutableSourceReferences.Where(x => !x.IsRetired && x.ExpiresAtUtcTicks > now.UtcTicks);
        if (scope is not null) query = query.Where(x => x.ScopeKeyHash == EfRelationalIdentity.Hash(scope));
    ...
    private string? ScopeForRead() => access.Current.Scope is null || access.Current.AcrossScopes ? null : access.Current.Scope.Value;
    private string ScopeForWrite(WorkflowExecutableSourceReference reference)
    {
        var scope = ScopeForRead(); if (scope is null && reference.TenantId is null) throw new InvalidOperationException("A source reference write requires an explicit persistence scope.");
        if (reference.TenantId is not null) access.Current.EnsureTenantScope(reference.TenantId); return reference.TenantId ?? scope!;
    }
    private static WorkflowExecutableSourceReferenceEntity ToEntity(WorkflowExecutableSourceReference value, string scope) { var row = new WorkflowExecutableSourceReferenceEntity(); Copy(row, value, scope); return row; }
    ...
    {
        row.Id=value.SourceReferenceId; row.SourceReferenceId=value.SourceReferenceId; row.SourceReferenceIdOrderKey=OrderKey(value.SourceReferenceId); row.ArtifactId=value.ArtifactId; row.ArtifactIdHash=EfRelationalIdentity.Hash(value.ArtifactId); row.DefinitionVersionId=value.DefinitionVersionId; row.DefinitionVersionIdHash=EfRelationalIdentity.Hash(value.DefinitionVersionId); row.ScopeKey=EfRelationalIdentity.Encode(scope); row.ScopeKeyHash=EfRelationalIdentity.Hash(scope); row.Scope=value.Scope.ToString(); row.IsRetired=value.DeletedAt is not null; row.ExpiresAtUtcTicks=(value.ExpiresAt??DateTimeOffset.MaxValue).UtcTicks; row.ContentJson=RuntimeArtifactJson.Serialize(value); row.SchemaVersion=RuntimeArtifactEfModule.SchemaVersion; if(row.Revision<=0)row.Revision=1;else row.Revision++;
    }
    private static WorkflowExecutableSourceReference Read(WorkflowExecutableSourceReferenceEntity row,string? expectedScope,string expectedId)
    {
        try
        {
            if(row.Id!=expectedId||row.SourceReferenceId!=expectedId||row.SchemaVersion!=RuntimeArtifactEfModule.SchemaVersion||row.SourceReferenceIdOrderKey!=OrderKey(expectedId)||row.ScopeKeyHash!=EfRelationalIdentity.Hash(EfRelationalIdentity.Decode(row.ScopeKey))||row.ArtifactIdHash!=EfRelationalIdentity.Hash(row.ArtifactId)||row.DefinitionVersionIdHash!=EfRelationalIdentity.Hash(row.DefinitionVersionId)||row.Revision<=0)throw new InvalidDataException("The persisted workflow executable source reference row is corrupt.");
            var value=RuntimeArtifactJson.Deserialize<WorkflowExecutableSourceReference>(row.ContentJson);Validate(value);
            if((expectedScope is not null&&EfRelationalIdentity.Decode(row.ScopeKey)!=expectedScope)||value.SourceReferenceId!=expectedId||value.ArtifactId!=row.ArtifactId||value.DefinitionVersionId!=row.DefinitionVersionId||value.Scope.ToString()!=row.Scope||row.ExpiresAtUtcTicks!=(value.ExpiresAt??DateTimeOffset.MaxValue).UtcTicks||row.IsRetired!=(value.DeletedAt is not null))throw new InvalidDataException("The persisted workflow executable source reference projection is corrupt.");
            return value;
        }
    ...
    private enum Route{All,Artifact,DefinitionVersion} private sealed record Cursor(int Version,Route Route,string?ScopeHash,string Binding,string Key);
}
