...
public interface IWorkflowAlterationStore
{
    ...
    /// <summary>Lists a bounded page of non-terminal plans in stable plan-ID order for restart-safe orchestration.</summary>
    ...
    /// <summary>Terminalizes an unsealed capture and discards its provisional target jobs.</summary>
    ...
    /// <summary>Terminalizes an unsealed capture failure and discards its provisional target jobs.</summary>
    ...
    /// <summary>Marks not-yet-claimed jobs cancelled using one caller-derived safe skipped outcome per envelope.</summary>
    ValueTask CancelPendingJobsAsync(string planId, IReadOnlyCollection<WorkflowAlterationOutcome> skippedOutcomes, DateTimeOffset completedAt, CancellationToken cancellationToken = default);
    ...
    ValueTask<WorkflowAlterationJobState?> ClaimNextAsync(string ownerId, DateTimeOffset now, TimeSpan leaseDuration, CancellationToken cancellationToken = default);
