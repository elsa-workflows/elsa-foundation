...
- **Usage:** post-commit notification so a projection derived from waiting bookmarks (e.g. the per-shell HTTP route table for mid-flow endpoints) refreshes as bookmarks come and go, without the runtime depending on any consumer. `OnBookmarkCreatedAsync` fires AFTER the bookmark-created checkpoint commits (`WorkflowCreateBookmarkSchedulerWorkHandler`); `OnBookmarkConsumedAsync` AFTER the bookmark-consumed checkpoint commits (`BookmarkConsumptionCheckpointService`). **Failure policy (opposite of `IWorkflowTriggerIndexObserver`):** this fires on the RUN path — an observer exception is caught and logged by `BookmarkLifecycleNotifier` and NEVER faults the run (a stale route simply 404s until the next refresh). Keep observer work idempotent and cheap; a throw is swallowed.
