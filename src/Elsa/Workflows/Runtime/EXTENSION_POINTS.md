...
- **Signature:** explicit `PrepareAsync(...)` and `CommitPreparedAsync(...)`, bounded stable `PagePreparedAsync(RuntimeCheckpointPreparedQuery, ...)`, and provider-atomic `CommitPreparedFoldAsync(RuntimeCheckpointPreparedFoldRequest, ...)`.
...
- **Usage:** retains canonical, digest-verifiable `Prepared` reservations before enrichment and exposes workflow-bound pages capped at 250. The terminal-fold member is contract scaffolding only: the in-memory provider, Groundwork provider, and coalescing decorator fail closed with `NotSupportedException` until the reviewed adoption/fold work unit enables its semantics. Merely registering the capability does not authorize terminal folding, abandoned-preparation recovery, source dispatch, adoption, or inferred terminal outcomes.
- **Provider expectations:** paging must be stable and cursor-bound; preparation identity, source authority, canonical bytes/digest, provenance/order, and original fence/revisions remain immutable; mutable authority binding is separate. No current provider may mutate state, context, outbox, marker, receipt, compaction, or high-watermark through `CommitPreparedFoldAsync`.
...
- **Usage:** `WorkflowDrainOrchestrator` opens a scope around a coalesced drain. An empty active session may reconcile consumed queue work, but any boundary or quiescence path containing buffered Prepared state fails closed without durable mutation or queue advancement until the reviewed adoption/fold work unit is enabled. The factory does not recover abandoned durable `Prepared` reservations. Per execution, the orchestrator first consults `IRuntimeCheckpointCadenceResolver` (per-run stamp > authored-on-executable cadence > host default) and skips the scope entirely for an authored-Immediate run. Only registered by the opt-in extension.
