...
- **Usage:** the default scheduler command router gates externally arriving live work through a router-internal deny-list predicate (change which kinds are gated by replacing the scoped router via `IWorkflowExecutionCommandExecutor`, not through a seam of its own). Recovery and control-plane commands (`RunSchedulerWork`, `Cancel`, `PauseWorkflowExecution`, `UnpauseWorkflowExecution`) bypass admission: the sweep carries its own bound (`RuntimeResumptionOptions.MaxExecutionsPerSweep`) and refusing an operator's cancel at capacity would remove the tool for ending the overload. `AlterWorkflow` also never reaches this gate — it is routed to the alteration executor above it and currently carries no load bound of its own (#1320 tracks the admission composition gaps). The default signal meters **dispatches, not commands** — an External-leaf run pays ~56 dispatches where a fusable run pays ~5, so a command-count limit would under-report exactly the shape production traffic has.
