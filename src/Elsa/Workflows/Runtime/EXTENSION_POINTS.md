...
- **Child-fault absorption (spec 115):** a child-fault callback may stage `RequestChildFaultAbsorption(incidentId, reason)` naming the evaluation's incident. The incident resolves (`IncidentStatus.Resolved`, `IncidentResolutionAction.Continue`) and the faulted child's leftover subtree is reclaimed via the same planner (`Cancelled`/`FaultAbsorbed` descendants, cleanup capture, other incidents suppressed) in the continuation's commit. This is the only runtime path that resolves an incident; absorption only prevents `BlockingIncidentWorkflowFaultObserver` from faulting the run when the incident and the absorbing evaluation land in the same drain.
