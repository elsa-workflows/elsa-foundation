...
### `IRuntimeRequirementChecker` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (§2.6.2 — exactly one checker is meaningful per engine; `TryAddScoped` in `AddWorkflowRuntime()` is the conflict prevention, first-wins per ADR 0033, so a second registration cannot silently win).
- **Signature:** `Check(RuntimeRequirementCheckSubject subject)` → `RuntimeRequirementCheckResult`. The subject is neutral over executables and reusable-activity templates (`FromExecutable` / `FromTemplate`) — both declare the same requirement sets and both carry nodes, so a replacement must answer for **both**; a checker that handles only executables silently under-gates activity-template publication.
- **Usage:** one call, one verdict, two independent axes — (a) declared consumer capabilities and durable-value storage drivers, matched by exact ordinal set membership against `IRuntimeActivityConsumerCapability` advertisements and unversioned driver keys; (b) per-node CLR activity-type presence in `IWellKnownTypeRegistry`. **The axes never intersect** (capabilities are per activation *mechanism* and say nothing about type availability), so either alone under-gates. Extracted from publishing's deployment preflight (spec 151, FR-B-005/005a) and now *shared*, not duplicated: the preflight wraps it and keeps its own scoping/views, the artifact importer calls it per artifact. Evaluation semantics were relocated verbatim — a replacement that redefines them changes what both callers accept.
- **Default implementation:** `RuntimeRequirementChecker`.

### `IWorkflowExecutableHasher` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (§2.6.2 — the hash **is** artifact identity under [ADR 0038](../../../../docs/adr/0038-artifact-hash-is-purely-behavioral-and-executables-are-content-addressed.md), so two implementations would mean two definitions of identity; `TryAddScoped`, first-wins).
- **Signature:** `ComputeHash(ExecutableNode)`, the full behavioural overload (node tree + input contract + dependencies + checkpoint cadence + workflow variables + incident strategy), and `CreateArtifactId(artifactIdPrefix, artifactHash)`.
- **Usage:** **the canonical payload shape is wire-significant** — any change here moves every artifact hash and id in existence, which is a migration, not a tweak. The payload is behavioural-only, so equal hash ⇔ equal behaviour in both directions. That invariant is what makes the importer's recompute meaningful: the executable store is create-only and dedups by id, so persisting an unverified payload under a claimed id would let a corrupted file *become* that id's content on a fresh engine. Two consumers: the publishing compiler derives, the artifact importer recomputes before persistence.
- **Default implementation:** `WorkflowExecutableHasher`. **Composition note:** `WorkflowsPublishingFeature` also `TryAdd`s this contract, so a publishing-only host (no `AddWorkflowRuntime()`) still resolves it; whichever composes first wins, and both register the same type.

### `IWorkflowArtifactClosureSerializer` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (one wire codec per engine; `TryAddSingleton`). Singleton to match `IPayloadSerializer`'s lifetime and keep System.Text.Json's per-options metadata cache alive.
- **Signature:** `Serialize(WorkflowArtifactClosure)` and `Deserialize(string)`.
- **Usage:** the single place the portable envelope's bytes are decided, shared by the exporting engine and the importing one (FR-B-001/010). A closure carries `WorkflowExecutable`s, whose constructor recomputes `Nodes`/`NodesById` from the root; encoded naively those projections ride along, duplicating the whole activity graph in the file **and** making an exported artifact's bytes differ from the same artifact's bytes in the durable store. One codec on both sides is what makes export/import round-trip byte-consistency true rather than aspirational. Encoding rides `IPayloadSerializer`'s options, so the envelope inherits the engine's determinism guarantees instead of asserting its own.
- **Exception discipline:** this is a codec, not a feature boundary — a malformed document surfaces as `JsonException`. Callers that own an identifier worth naming (the importer owns a file path, the export endpoint a version id) wrap it per §2.23.5. Wrapping here would only strip the identifier the codec does not have.
- **Default implementation:** `WorkflowArtifactClosureSerializer`.

### `IWorkflowActivationAuthority` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (§2.6.2 — a second implementation would mean two answers to "what is live"; `TryAddSingleton`, first-wins).
- **Signature:** `FindAsync(definitionId, slotName, ...)`, `ListByDefinitionAsync(definitionId, ...)`, `TryActivateAsync(WorkflowActivationSlotRequest, ...)`, `TryDeactivateAsync(definitionId, slotName, source, expectedRevision, updatedAt, ...)`.
- **Usage:** the definition-keyed ledger of which activation is live per `(DefinitionId, SlotName)`, with CAS-guarded transitions and **explicit ownership**. **One ledger per engine, physically as well as contractually**: the publish pipeline and the artifact importer share it, which is what makes a dual-reconciliation overlap structurally detectable instead of a silent double activation. A revision mismatch yields `WorkflowActivationConflict.RevisionMismatch` and must leave the slot unchanged; a request from a non-owning source yields `ForeignSource` with a diagnostic naming the owner. Ownership is read from the slot's `WorkflowActivationSource` field **only** — never inferred from id prefixes. This contract owns the CAS transition and the ownership rule and nothing else; same-artifact idempotency belongs to the coordinator.
- **Supersedes:** publishing's `IPublicationSlotStore`, **deleted rather than relocated** (spec 151, FR-B-006) so the runtime does not inherit a concept named "Publication". A publishing persistence package must neither implement nor register this contract.
- **Default implementation:** `InMemoryWorkflowActivationAuthority` *(process-local; the Groundwork runtime store family replaces it with `GroundworkWorkflowActivationAuthority` over the `workflowActivationSlot` document kind)*. `WorkflowsPublishingFeature` `TryAdd`s the same in-memory default so a publishing-only host still composes.

### `IWorkflowActivationCoordinator` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (§2.6.2 — one activation lifecycle per engine; `TryAddScoped`, first-wins). Scoped because it composes the scoped trigger indexer, lease manager and observers.
- **Signature:** `ActivateAsync(WorkflowActivationCommand command, ...)` and `DeactivateAsync(WorkflowDeactivationCommand command, ...)`, both → `WorkflowActivationResult`.
- **Usage:** the single entry point for making an executable live, owning the **complete** sequence for every calling path: take the root-write lease (so reference GC cannot race the activation) → mint and save the live source reference → prepare **both** serving projections (trigger bindings *and* recurring schedules — a binding-only activation imports timer/cron workflows that never fire) → CAS the slot on `IWorkflowActivationAuthority` → activate both projections → notify every `IWorkflowTriggerIndexObserver` → retire the predecessor's reference with reason `"activation-replaced"`. On mid-sequence failure it compensates — restore the replaced activation, unconditionally re-activate the predecessor's projections, remove the candidate's, retire the candidate's reference with reason `"activation-failed"` — best-effort, never masking the original failure, with every non-converging step appended to the result diagnostic.
- **Deactivation is the same owner's other direction (`DeactivateAsync`, spec 151 T121).** CAS the slot empty on `IWorkflowActivationAuthority` → remove the retracted activation's serving projections → notify every `IWorkflowTriggerIndexObserver`. A slot that already serves nothing is `AlreadyInactive` and writes nothing, so a repeated retraction converges. On a post-flip failure it compensates by re-activating the slot and **re-preparing and re-activating** the projections from the artifact — the identical preparation, in the identical order, that `ActivateAsync` runs. That shared preparation is the point: the retraction path used to be a second component (`IPublicationProjectionPreparer` / `PublicationProjectionReconciler`, both now deleted) that had to know the same ordering invariant independently, and the two drifted apart once already.
- **`DeactivateAsync` is an in-process contract method, not a transport surface.** Its only production caller is publishing's `UnpublishPublicationSlotRequestHandler`; `Elsa.Workflows.Runtime.Api` references neither this contract nor `IWorkflowActivationAuthority` and deliberately exposes **no** deactivation endpoint. A runtime-only engine composes no publishing and therefore offers no way to retract an activation from outside — it deactivates only internally, on supersession and on compensation, which is the immutability that makes a hardened runtime worth having.
- **Callers request activation; they never implement it.** Publishing keeps compilation, publication policy and its `IPublicationRecordStore` attempt journal; the artifact reconciler keeps closure validation and import logic. A parallel copy of this sequence in either is the duplicated authority the contract exists to remove. Publishing still applies its own four-condition same-artifact guard *before* calling in (it includes trigger-retention and tenant conditions the runtime cannot see); the coordinator's own no-op is a safety net for the importer and for idempotent re-requests, not the decider for publishing.
- **Failure surface:** refusals and compensated failures come back as a result value — the importer's batch isolation and publishing's per-attempt journal both need them as data. Only conditions that prevent the sequence from being attempted at all (an uncomposed trigger spine, an unavailable retention lease, an infrastructure fault escaping the lease manager) throw, wrapped in `WorkflowActivationException` per §2.23.5.
- **Default implementation:** `WorkflowActivationCoordinator`. The trigger serving spine stays optional (`WorkflowsRuntimeTriggers` registers it), so the coordinator resolves from a bare `AddWorkflowRuntime()` and refuses loudly if asked to activate without it.
...
- **Kind:** Replacement (one indexer writes the activation-scoped trigger projection).
- **Signature:** `PrepareActivationAsync(WorkflowExecutable executable, string activationId, string slotId, CancellationToken cancellationToken = default)` — the only member; there is no default implementation, so every implementer must provide it.
- **Usage:** invoked by `IWorkflowActivationCoordinator` between storing the executable and the activation slot CAS. It completes preflight for the whole executable, runs every `IWorkflowTriggerIndexValidator` over the completed binding set, then persists that set as the candidate activation's **prepared** (non-serving) projection via `IWorkflowTriggerBindingStore.PrepareActivationAsync`. All semantic failures occur before mutation. It writes **only** the candidate activation's rows — it must not delete by artifact, because a second slot or the predecessor activation may legitimately share the artifact. Making the projection serve, superseding the predecessor, notifying `IWorkflowTriggerIndexObserver`, and compensating a failed attempt all belong to the coordinator; prepared rows do not serve, so an indexer that notified observers would project routes that are not live.
- **Removed (spec 151, FR-B-006 writer census):** the artifact-scoped `IndexAsync(executable, ct)` member and its default-interface fallback `PrepareActivationAsync => IndexAsync`. That path deleted every binding of the artifact and wrote rows born active, bypassing prepare/activate entirely; the fallback meant an implementation providing only the documented `IndexAsync` signature was silently routed into that artifact-wide wipe. An implementation of the legacy shape alone no longer satisfies this contract.
- **Default implementation:** `WorkflowTriggerIndexer`. It owns the trigger-binding projection and **only** that one: the recurring-schedule projection is prepared beside it by `IRecurringTriggerScheduleProjectionPreparer`, which the coordinator calls in its own right. Replacing this contract therefore cannot disarm recurring activation. Until spec 151's T044b it could: a `RecurringTriggerScheduleIndexer` decorator (now deleted) prepared the recurring projection inside this call, so a host substituting its own indexer silently lost it and found out only after the slot CAS.
...
- **Usage:** PRE-write validation (the pre-write counterpart of `IWorkflowTriggerIndexObserver`, issue #592 item 2) so a stimulus family can enforce activation-time constraints over the index without the indexer knowing any stimulus type. Called after extraction, before the prepared projection is written. **Failure policy:** a throw fails the activation with the durable index untouched — no rollback needed, and a bad publish can never poison the store for later publishes or startup. Keep family-specific constraints in the owning module: do NOT enforce cross-definition stimulus uniqueness generically — for most stimulus types (e.g. two definitions on one Timer cron) shared identity is legitimate fan-out.
...
- **Kind:** Contribution (fan-in; enumerable). Register with `services.TryAddEnumerable(ServiceDescriptor.Scoped<IWorkflowTriggerIndexObserver, MyObserver>())` (or Singleton); `IWorkflowActivationCoordinator` resolves `IEnumerable<IWorkflowTriggerIndexObserver>`.
- **Signature:** `OnTriggersIndexedAsync(WorkflowTriggerIndexSnapshot snapshot, CancellationToken ct = default)` — the snapshot carries `ArtifactId` + the newly serving `IReadOnlyCollection<WorkflowTriggerBinding>` of the activation.
- **Usage:** post-activation notification so a projection derived from the trigger index (e.g. the per-shell HTTP route table) refreshes as an atomic part of making an artifact live, without the coordinator depending on any consumer. Called by the coordinator **after** the projections are activated (never by the indexer, whose rows are still prepared and non-serving), and again during compensation once both sides have reached their final serving state. `RequiresProjectionRefresh` is set on the snapshot because authority can move while the extracted bindings stay identical (or empty), so consumers must not apply a repeated-indexing skip to it. **Failure policy:** exceptions are NOT swallowed — an observer that throws fails the activation (same rule as an unindexed trigger). Keep observer work idempotent so a retried activation converges.
...
- **Usage:** the default scheduler command router gates externally arriving live work through a router-internal deny-list predicate (change which kinds are gated by replacing the scoped router via `IWorkflowExecutionCommandExecutor`, not through a seam of its own). Recovery and control-plane commands (`RunSchedulerWork`, `Cancel`, `PauseWorkflowExecution`, `UnpauseWorkflowExecution`) bypass admission: the sweep carries its own bound (`RuntimeResumptionOptions.MaxExecutionsPerSweep`) and refusing an operator's cancel at capacity would remove the tool for ending the overload. `AlterWorkflow` **is** gated (#1325): the gate sits above the alteration hand-off, so an alteration is now a **sheddable, counted-as-one** command and a dispatch nested inside it takes the ambient-charge path. It is counted as one and no more — an alteration holds the seed unit every admitted command pays for as long as its actor call runs, but performs no scheduler dispatches of its own (the product's only `RecordDispatch()` call site is in `WorkflowSchedulerDrainer`, which the alteration branch never reaches), so it accrues none of the per-dispatch units a `Start` does, flat regardless of plan size, and the scheduler work it commits is charged to whoever drains it next, which is never the alteration: the drain loop is per-execution and runs until the queue is dry, so under the exempt `RunSchedulerWork` that work stays uncounted, while under a subsequently admitted live command (a `ResumeBookmark`, `Checkpoint`, `CompleteActivity`) its dispatches are charged to *that* command. Because it dispatches nothing, its completion is also excluded from the adaptive sample (`ChargedUnits <= 1`), so its sub-millisecond exits cannot collapse the latency baseline. It is the one gated kind besides `Start` whose refusal writes nothing: parking an `AlterWorkflow` work item would hand it to `NoopWorkflowSchedulerWorkHandler`, which matches every kind except `InvokeActivity`/`GeneratedEvent`/`ResumeBookmark` and silently drops it (`ContinueVolatileWait`, `DeliverSignal`, and — in the core composition, whose handler comes from `ActivitiesRuntimeFeature` — `NotifyParentActivity` share that handler-less property and have no faulting fallback either; latent as router envelopes only because nothing constructs them as such). The outright refusal is safe because the alteration pump (`WorkflowAlterationOrchestrationPumpTask`, registered by `AddWorkflowRuntime` itself rather than by any opt-in feature, though as an `IRecurringTask` it runs only where the Tasks domain schedules it — and it is also the only producer of `AlterWorkflow` envelopes, so a host that does not run it cannot reach the refusal) re-claims the job once its lease lapses: a full `WorkflowAlterationOrchestrationOptions.JobLeaseDuration`, **1 minute by default**, against a `RetryAfter` in seconds. Parking on shed is conditional on a re-driver being composed (**#1320**): the router reads the injected `IEnumerable<IWorkflowDispatchDurabilityEvidence>` once when it is constructed — once per dispatched command, since it is scoped — and parks only when some evidence names `WorkflowDispatchDurabilityComponents.Resumption`, which in the product only `WorkflowsRuntimeResumptionFeature` contributes and `AddWorkflowRuntime` does not. Without it every gated kind takes the start-shaped refusal — shed, nothing written, and (because the agent consumes the idempotency key off `ShedWorkQueued` rather than off the kind) the key left intact so the caller's retry is honest — instead of promising a delivery no sweep would perform. The check is on the component *name*, not on its durability level, so a process-local re-driver would count too; an absent enumerable reads as "no re-driver", which is the conservative direction for a router composed by hand. What the evidence establishes is that the sweep is **registered**, not that it runs — the same caveat the alteration pump carries above, since `RuntimeResumptionPumpTask` is likewise an `IRecurringTask` and runs only where the Tasks domain schedules it: a host calling `WorkflowsRuntimeResumptionFeature.ConfigureServices` directly with no Tasks host contributes the evidence, parks, and never sweeps, which is #1320 again in a composition this rule reads as safe (a shell-composed host does not, that feature declaring `DependsOn = { "Tasks" }`). The refusal itself is reached by hand-composed and in-memory hosts — those composing `AddWorkflowRuntime` / `WorkflowsRuntimeApi` and no resumption feature — and not by a shell composing Groundwork runtime-store or unified persistence, since all nine of those features declare `DependsOn = { "WorkflowsRuntimeResumption" }` (the distributed-placement Groundwork feature does not, but it leaves the scheduler queue in-memory, so such a shell does reach the refusal). On a host composing no alteration executor at all, an `AlterWorkflow` arriving at capacity is refused before the router's "alterations are not composed" throw; that masks a composition error behind a retryable refusal only in principle, since one registration composes both that executor and the pump that is the only producer of `AlterWorkflow` envelopes. The default signal meters **dispatches, not commands** — an External-leaf run pays ~56 dispatches where a fusable run pays ~5, so a command-count limit would under-report exactly the shape production traffic has.
- **Safety:** a replacement signal must keep compare-and-reserve atomic; a separate read followed by a reserve lets N simultaneous callers all observe the same below-limit load and all pass, which is the burst the limiter exists to stop. `InFlightDispatches` must also remain a cheap, always-safe read: the controller reads it on the exempt paths (admission disabled, ambient charge), so the property may not throw or block. A replacement charge must also **seed at one unit** on admission and add one per `RecordDispatch()`: the controller reads `ChargedUnits <= 1` as "this command dispatched nothing" and excludes that completion from the adaptive sample (#1325), so a charge seeding at zero makes every single-dispatch command report one and silently drops those completions from the sample, biasing it toward heavier commands. Adaptation keeps running on everything that dispatched twice or more, which is why the miscalibration surfaces as drift rather than as an error. The adaptive controller and the `StaticLimit` operator override share one code path (the override clamps the adaptive range to a single value), so a replacement must not fork them.
...
### `IRecurringTriggerScheduleProjectionPreparer` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (one preparer writes the activation-scoped recurring-schedule projection; `TryAddScoped` by `WorkflowsRuntimeRecurringTriggers`, first-wins).
- **Signature:** `PrepareActivationAsync(WorkflowExecutable executable, string activationId, string slotId, CancellationToken cancellationToken = default)` — the only member.
- **Usage:** invoked by `IWorkflowActivationCoordinator` **before** `IWorkflowTriggerIndexer`, so every recurrence is materialized and validated before the first trigger binding is written and an invalid or exhausted expression fails the activation with neither projection mutated. It walks the pinned executable's compiler-marked start-trigger nodes, asks every `IRecurringTriggerScheduleProvider` to describe them, seeds each `NextOccurrence` through `IRecurringScheduleCalculator`, and persists the whole set as the candidate activation's **prepared** (non-serving) projection via `IRecurringTriggerScheduleStore.PrepareActivationAsync`. Preparation is **unconditional**: with no provider composed it still writes an explicit empty projection, so a later activate or compensate has a projection to move rather than silently nothing. Making the projection serve and compensating a failed attempt belong to the coordinator, which calls this same preparation on both lifecycle directions.
- **Separated from the indexer (spec 151, T044b).** This was a decorator over `IWorkflowTriggerIndexer` until T044b, which made that **replacement** contract silently own a second projection: a host substituting its own indexer lost recurring preparation, and the loss surfaced only after the slot CAS, in compensation. As two contracts, replacing either can no longer disarm the other — and the coordinator refuses the half-composition outright, throwing `WorkflowActivationException` when an `IRecurringTriggerScheduleStore` is composed with no preparer.
- **Default implementation:** `RecurringTriggerScheduleProjectionPreparer` *(registered by the `WorkflowsRuntimeRecurringTriggers` feature alongside the store, the calculator and the pump)*.
...
- **Usage:** the recurring-schedule sibling of `IActivityTriggerStimulusProvider`. At **activation time** — publishing and artifact reconciliation alike — `IRecurringTriggerScheduleProjectionPreparer` asks every provider to describe a node; a provider returns the node's recurrence descriptors (interval / cron expression → next occurrence) when it recognizes the activity type, or an **empty collection** ("no contribution": not-my-type or recognized-with-no-schedules). A single-schedule trigger (`Timer`/`Cron`) returns one descriptor; a composite start-trigger activity may **fan out** one descriptor per authored recurring start element (spec 117 — a BPMN process with several timer start events). Multiple *providers* claiming one node fail with contextual provider ids rather than selecting by registration order. The preparer materializes the complete schedule candidate set before anything is written, keying single-descriptor nodes by `RecurringTriggerSchedule.BuildId(activationId, artifactId, nodeId)` and fan-out nodes by `BuildFanOutId(activationId, artifactId, nodeId, stimulusHash)` so their rows do not collapse. Invalid expressions, calculator failures, and exhausted Cron schedules are wrapped in contextual `WorkflowTriggerPreflightException` failures before either projection is mutated. A recurring-trigger activity contributes both seams: a trigger binding and a materialized schedule per element, sharing the same `(StimulusType, StimulusHash)`. Providers read only the pinned published `ExecutableNode`.
...
- **Kind:** Contributor (one contribution per durability boundary a host composes).
- **Signature:** `string Component { get; }` and `WorkflowDispatchDurabilityLevel Level { get; }`. Component names are the constants on `WorkflowDispatchDurabilityComponents`; `Required` lists the five a durable host must cover.
- **Register:** `services.TryAddEnumerable(ServiceDescriptor.Singleton|Scoped<IWorkflowDispatchDurabilityEvidence, TEvidence>())`. `TryAddEnumerable` de-duplicates by **implementation type**, so each boundary needs its own distinct type rather than one type registered repeatedly. Either lifetime is fine — but a *consumer* must be scoped, since some contributions are (see below).
- **Usage:** two consumers, and they are not the same kind of consumer. `WorkflowDispatchReadinessAssessor` aggregates the whole set into a `WorkflowDispatchReadinessReport` — pure reporting. `WorkflowSchedulerCommandRouter` reads it as a **write decision** (#1320): it parks a shed command's work item only when some contribution names `WorkflowDispatchDurabilityComponents.Resumption`, and otherwise refuses outright, writing nothing. The match is on the component *name* and not on `Level`, so a process-local re-driver counts. Two consequences for implementers: a leaf that supplies its own re-driver without contributing that component silently turns parking off for every gated kind on that host; and the contribution proves the re-driver is **registered**, not that it runs, since `RuntimeResumptionPumpTask` is an `IRecurringTask`. The refusal rule itself is documented in the `IRuntimeAdmissionLoadSignal` / `IRuntimeAdmissionController` entry above.
- **Known implementations (shipped):** four process-local defaults registered unconditionally by `AddWorkflowRuntime` — `ProcessLocalCheckpointEvidence`, `ProcessLocalDispatchStoreEvidence`, `ProcessLocalOutboxEvidence`, `ProcessLocalSchedulerEvidence` *(intra-domain — default)*; `DurableResumptionEvidence` from `WorkflowsRuntimeResumptionFeature`, the only contributor of the `resumption` component anywhere in the product *(intra-domain)*; `ProcessLocalDistributionEvidence` from `WorkflowsRuntimeDistributedFeature` *(intra-domain)*; the four **scoped** `Groundwork*DurabilityEvidence` types from `Elsa.Persistence.Groundwork` *(cross-domain — domain `Elsa.Persistence`)*; and the scoped `GroundworkDistributionDurabilityEvidence` from `Elsa.Workflows.Runtime.Distributed.Persistence.Groundwork` *(intra-domain — that project is domain `Elsa.Workflows`, same as this contract)*. `WorkflowDispatchDurabilityEvidence` (Core, `Models/WorkflowDispatchReadiness.cs`) is a shipped public record implementing the interface; nothing registers it, and it exists so a caller or test can express a contribution without declaring a type.
...
### `IRuntimeActivityConsumerCapability` *(Core — `Elsa.Activities.Runtime.Core`)* — the engine's own contribution
- **Kind:** Contribution (fan-in; enumerable). Registered `services.TryAddEnumerable(ServiceDescriptor.Singleton<IRuntimeActivityConsumerCapability, WorkflowIntrinsicActivityConsumerCapability>())` in `AddWorkflowRuntime()`. **Never a replacement** — it stands beside the CLR and graph advertisements, and registering a capability with `Replace(...)` would silently un-advertise every other consumer this engine can execute.
- **Contributed here:** `WorkflowIntrinsicActivityConsumerCapability` advertises `WellKnownRuntimeActivityConsumers.Intrinsic` at exactly `RuntimeActivityDescriptor.InitialSchemaVersion` — the descriptor key `ExecutableNodeCompiler` stamps onto every intrinsic node (`Set`, `Merge`, `Return`, `Control`, `SetCorrelationId`, …).
- **Why an advertisement with no matching activation strategy:** intrinsics are executed by `WorkflowIntrinsicExecutor` — the engine spine itself — not by an `IActivityActivationStrategy`, so nothing in the strategy registry speaks for them. Capability *advertisement* is a distinct concept from strategy registration: it answers "can this runtime execute material declaring this consumer?", and for intrinsics the answer is yes with the runtime as provider. A capability without a strategy is correct here, not an inconsistency.
- **Why it is required:** `WorkflowExecutable`'s constructor derives `RuntimeRequirements` from every node's consumer key, so any compiled workflow containing an intrinsic declares `RuntimeRequirement("intrinsic", "1")`. Without the advertisement that requirement reads `Missing` to `IRuntimeRequirementChecker`, and **every intrinsic-bearing artifact is rejected at the import gate and reported unready by publishing's deployment preflight**. Advertising is preferred over excluding intrinsics from the derivation, because the requirement set is meant to be a *complete* statement of what a portable artifact needs — so a future trimmed runtime that dropped an intrinsic is caught at import rather than faulting on first execution.
- **Advertised by others:** `GraphActivitiesRuntimeFeature` (`elsa.graph-activity`) — see the [Activities Graph Runtime catalog](../../Activities/Graph/Runtime/EXTENSION_POINTS.md).
...
- Portable-artifact import (the source contract, the reconciler, the feature-inheritance point) — the second caller of the activation coordinator, the requirements checker, the hasher and the closure codec above: [`Elsa.Workflows.Runtime.Reconciliation/EXTENSION_POINTS.md`](Reconciliation/EXTENSION_POINTS.md).
