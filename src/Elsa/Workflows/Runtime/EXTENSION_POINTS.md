...
- **Known implementations (shipped):** `Sequence`, `Flowchart`, `Parallel`, `If`, `Switch`, `For`, `ForEach`, `While`, `Do`, and `BpmnProcess` (also a `[TriggerActivity]`, spec 117, an `IRuntimeLiveChildActivityConsumer`, spec 119, and an `IRuntimeScopedVariableReader`, spec 123).
