...
- **Signature:** `ActivateAsync(WorkflowActivationCommand command, ...)` → `WorkflowActivationResult`.
...
- **Default implementation:** `WorkflowTriggerIndexer` *(decorated by `RecurringTriggerScheduleIndexer` when `WorkflowsRuntimeRecurringTriggers` is composed, which prepares the recurring-schedule projection in the same call — including an explicit empty projection when no recurring providers are registered, so the recurring projection has exactly one owned write)*.
