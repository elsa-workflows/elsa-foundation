...
- **Kind:** Replacement (§2.6.2 — exactly one checker is meaningful per engine; `TryAddScoped` in `AddWorkflowRuntime()` is the conflict prevention, first-wins, so a second registration cannot silently win).
