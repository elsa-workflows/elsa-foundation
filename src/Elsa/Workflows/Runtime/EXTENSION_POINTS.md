...
`Elsa.Workflows.Runtime.Core.Models`; no contract in this layer depends on EF Core or another
storage provider.
...
- **Safety:** the guarded provider claim rejects conflicting durable providers in either composition order. Provider transactions serialize scope close against detached child admission and commit a Started child's cancellation marker with its deterministic cancel outbox responsibility. A cleanup that finds that cancel item already recorded converges when the item carries the same responsibility (`WorkflowDispatchLifecycle.EnsureTestScopeCancellationResponsibility`), whatever its delivery progress, and keeps the item unchanged; it must not re-save it as pending (spec 102 FR-014). A different responsibility under the same identity is a conflict.
- **Defaults:** `InMemoryWorkflowTestScopeStore`; the EF Core module replaces the complete family.
...
- **Usage and dependents:** successful DispatchWorkflow terminal enrichment uses the exact committed item to preserve replay identity after uncertain acknowledgement. The in-memory checkpoint store, coalescing overlay, and EF outbox provider implement the capability alongside their write store.
...
- **Alternative implementation:** `ActivitySourceWorkflowEngineTracer` *(opt-in — composed by the `WorkflowsRuntimeTracing` shell feature in `Elsa.Workflows.Runtime`, which `services.Replace(...)`s the no-op; still costs nothing until an `ActivityListener`/OpenTelemetry `AddSource` attaches)*.
...
- **Default implementation:** `InMemoryBookmarkStateStore` / `EfBookmarkStateStore` *(each also implements this interface)*. The opt-in `WorkflowsRuntimeBookmarksEntityFrameworkCorePersistence` feature selects `EfBookmarkStateStore`, which implements the same bounded index over the Runtime-owned EF model.
...
- **Default implementation:** `InMemoryWorkflowTriggerBindingStore` *(single-node in-memory default; `EfWorkflowTriggerBindingStore` replaces it for durable storage over the `workflowTriggerBinding` document kind)*.
...
- ### `IExecutionPlacementStore` *(leaf)*
  ...
  - **Default implementation:** `InMemoryExecutionPlacementStore` *(single-process/two-node-harness default; a scoped adapter over shared state keyed by partition + execution ID)*. `EfExecutionPlacementStore` *(durable `IDocumentStore`-backed bridge, W27 — opt-in leaf `Elsa.Workflows.Runtime.Distributed.Persistence.EntityFrameworkCore`, swapped in by the `WorkflowsRuntimeDistributedEntityFrameworkCorePersistence` feature; exact cross-node CAS via the provider's ExpectedVersion contract, including create-only first claims)*. `EfExecutionPlacementStore` *(opt-in leaf `Elsa.Workflows.Runtime.Distributed.Persistence.EntityFrameworkCore`; provider-neutral EF Core model for SQLite, SQL Server, PostgreSQL, and MySQL; replaces placement only and leaves command transport ownership unchanged)*.
...
- **Default implementation:** `InMemoryWorkflowSchedulerWorkQueue` *(single-node in-memory default for the current runtime slice)*. `EfSchedulerWorkQueueStore` *(durable `IDocumentStore`-backed bridge; swapped in by `AddRuntimeEntityFrameworkCore` so scheduler work survives a process crash — see [docs/runtime-durable-resumption.md](../../../../docs/runtime-durable-resumption.md))*.
...
- **Default implementation:** `InMemoryDurableTimerStore` *(single-node in-memory default; Delay works but is **not** restart-durable without a durable store)*. `EfDurableTimerStore` *(durable `IDocumentStore`-backed bridge; swapped in by `AddRuntimeEntityFrameworkCore`)*.
- **Follow-ups (W8):**
  - **Native due-time range index.** `MaxTimersPerTick` bounds the *dispatch* burst, not the *load*; a native range index on `DueTime` is the scale follow-up.
...
- **EF Core implementation:** `EfActivityExecutionStateStore` *(opt-in `WorkflowsRuntimeActivityExecutionEntityFrameworkCorePersistence`;)*.
...
- **Known provider implementations:** `Elsa.Persistence.EntityFramework` — `EfActivityExecutionInspectionStore` *(cross-domain persistence provider replacement)*.
- **EF Core implementation:** `EfActivityExecutionInspectionStore` *(opt-in `WorkflowsRuntimeActivityExecutionEntityFrameworkCorePersistence`;)*.
...
- **Default implementation:** `InMemoryBookmarkStateStore` *(single-node in-memory default for the current runtime slice)*. The opt-in `WorkflowsRuntimeBookmarksEntityFrameworkCorePersistence` feature selects `EfBookmarkStateStore`;.
...
- **Default implementation:** `InMemoryWorkflowExecutableStore` *(single-process default)*. The EF Core module supplies `EfWorkflowExecutableStore` as a keyed backend; SQLite reference features select a bounded `CachingWorkflowExecutableStore`, while PostgreSQL/distributed and legacy direct registrations preserve direct reads by default. The cache coalesces same-ID misses, retains only positive provider results, invalidates on save/unconditional delete and successful guarded delete, and never populates from `ListAsync`. Root-write lease and deletion-guard transitions pass through to the provider unchanged; custom providers can replace the keyed backend without a concrete dependency.
...
- **Default implementation:** `InMemoryExecutableActivityTemplateStore`; the EF Core module replaces it for durable hosts.
...
- **Default implementation:** `RuntimeInMemoryActivityExecutionHierarchyStore`; the EF Core module replaces it for durable hosts. Opaque page cursors are encoded by `IActivityExecutionHierarchyCursorCodec`.
- **EF Core implementation:** `EfActivityExecutionHierarchyStore` *(opt-in `WorkflowsRuntimeActivityExecutionEntityFrameworkCorePersistence`;)*.
