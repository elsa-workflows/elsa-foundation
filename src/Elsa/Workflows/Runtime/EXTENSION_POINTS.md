...
### `IRuntimeCheckpointCommitStore` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (one provider owns generic checkpoint preparation, replay, and final atomic commit for a runtime composition).
- **Signature:** `PrepareAsync(RuntimeCheckpointPrepareRequest request, ...)`, `CommitPreparedAsync(RuntimeCheckpointPreparationToken token, RuntimeCheckpointCommit commit, RuntimeCheckpointPersistenceDecision decision, ...)`, plus the migration-only `CommitAsync(...)` compatibility surface.
- **Preparation obligation:** compare the complete canonical request identity before allocating anything. `RuntimeCheckpointPreparationPayloadCodec` is the provider-neutral authority for that identity and for recovery: its canonical payload and SHA-256 are the values persisted and compared. Only the typed top-level `ExpectedFence` and `Checkpoint.Provenance` members are omitted from the canonical raw request; identically named properties inside business JSON are preserved. The ledger's typed `Raw*` members are compatibility projections decoded from that payload; they are not an independent recovery or identity source. A new logical commit reserves one positive workflow-local order and a bounded immutable context snapshot in a durable `Prepared` ledger entry before any enricher runs. A matching marker or ledger entry returns the stored token/provenance/receipt without allocating another order; changed source, operation, requested context, or raw commit material returns `Conflict`. Any exceptional preparation failure, including one after order reservation, rolls back its order, high-watermark, and ledger residue before surfacing the original exception.
- **Atomic finalization obligation:** validate the complete token, current order/context revisions, and ownership fence in the same provider transaction that writes state, outbox, the create-only commit marker, terminal ledger status/receipt, committed high-watermark, and any context mutation. `Conflict` and `OwnershipLost` expose none of those writes. A matching retry returns `Replay`; changed enriched material returns `Conflict` rather than a provider-specific exception.
- **Failure and recovery:** a `Prepared` entry is recovery input, not a visible committed checkpoint. Providers must retain the codec envelope so recovery can verify the canonical input and deterministically rerun enrichment; an invalid version, hash, JSON shape, or non-canonical payload fails closed. Because reservation deliberately precedes enrichment, an enricher may introduce a state-change category whose provider is absent. Finalization then fails composition without exposing state, outbox, or a commit marker, while the original `Prepared` reservation remains recoverable; re-preparing the same raw request reuses its workflow-local order and context snapshot while issuing the current ledger token/revision. A standalone `Skipped` finalization advances no committed high-watermark; an atomic terminal fold advances its committed watermark through every explicit `Committed`, `Skipped`, or trusted `Failed` disposition without exposing state/outbox effects for the latter two. Serialized tokens and provenance are compared semantically, including canonical context entries, rather than by collection identity.
- **Compatibility and migration:** the default interface methods are a source-compatible adapter only; they do not certify durable preparation. Existing null-provenance checkpoint JSON and fingerprints remain stable. Providers migrating an existing legacy marker must reconcile it as replay without rewriting the marker or inventing a conflicting final fingerprint. Durable replacements must implement both preparation methods before they claim this atomicity contract; the InMemory implementation is the process-local reference.
- **Default implementation:** `InMemoryRuntimeCheckpointCommitStore` *(single-process provider-owned ledger, revisions, high-watermarks, and atomic checkpoint/outbox state)*. `InMemoryCheckpointTransactionCoordinator` provides strict two-phase locking over the exact affected built-in participants: participant gates cover public reads and writes, including workflow-test-scope operations and dispatch redrive, so observers see the all-old state until commit and the all-new state after release. Preparation preflights the complete affected participant set before reserving an order or ledger entry; missing, custom non-participant, mismatched, or null nested participants are rejected. A composition that replaces one constituent checkpoint store must therefore replace the checkpoint commit provider with the same transaction owner or supply a compatible participant. The coordinator captures only identities named by the mutation plan, so rollback preserves unrelated concurrent state. Restore runs in reverse order and is best effort; if any restore fails, every enlisted participant is faulted and rejects later access rather than exposing a possibly partial provider state.

### `IRuntimeSchedulerWorkItemResolver` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Provider capability bound to the undecorated durable `IWorkflowSchedulerWorkQueue` implementation.
- **Signature:** `ResolveAsync(RuntimeCheckpointRecoveryAuthority authority, ...)` returns exactly one of `Exact`, `Missing`, `FingerprintMismatch`, `UnsupportedVersionOrKind`, or `Ambiguous`; only the Runtime recovery router can produce `Absent` for an originally null authority.
- **Provider obligation:** inspect claimed-but-still-durable work, use exact lookup or workflow-bound stable pages capped at 250, and never consult a coalescing overlay that can hide or synthesize work. The in-memory and Groundwork scheduler-work providers implement this capability directly.
- **Registration and replacement:** `AddWorkflowRuntime` registers one scoped resolver against the undecorated in-memory queue; Groundwork replaces it with a scoped binding to `GroundworkWorkflowSchedulerWorkQueue`. A durable queue replacement must explicitly register the matching resolver capability with the same persistence scope. Provider failures surface as `RuntimeSchedulerWorkRecoveryResolutionException`; cancellation and the six protocol outcomes retain their ordinary meaning.

### `IRuntimeCheckpointRecoveryAuthorityAccessor` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (one ambient accessor carries the immutable scheduler-work recovery authority for the active dispatch).
- **Signature:** nullable `RuntimeCheckpointRecoveryAuthority Current { get; }` and `IDisposable Push(RuntimeCheckpointRecoveryAuthority authority)`.
- **Default and registration:** `AmbientCheckpointRecoveryAuthorityAccessor`, registered as a singleton by `AddWorkflowRuntime` so its `AsyncLocal` stack flows through nested asynchronous handler calls while remaining isolated by execution context.
- **Usage:** `WorkflowSchedulerDrainer` alone derives and pushes authority from the actually acquired durable scheduler item; `RuntimeCheckpointCommitter` alone reads and copies it into `RuntimeCheckpointPrepareRequest`. Checkpoint callers cannot supply an override and persistence providers must not infer or interpret it. Scopes must restore the previous value on success, exception, and cancellation.
- **Replacement expectations:** replacements must preserve nullable-outside-dispatch behavior, stack restoration, nested asynchronous flow, and exact object identity. They must not introduce service location, provider-specific branches, or a process-global mutable value.

### `IRuntimeCheckpointPreparationReplayer` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement (one deterministic service reconstructs a verified prepared logical checkpoint from provider-owned canonical input).
- **Signature:** `RehydrateAsync(RuntimeCheckpointPreparedReservation reservation, CancellationToken cancellationToken = default)` returns `RuntimeCheckpointPreparedCommit`.
- **Default and registration:** `RuntimeCheckpointPreparationReplayer`, registered as scoped by `AddWorkflowRuntime`; `RuntimeCheckpointCommitter` receives the same contract rather than maintaining a second enrichment path.
- **Usage:** verifies and decodes the canonical preparation envelope, restores the provider-assigned provenance, runs ordered `IRuntimeCheckpointCommitEnricher` contributions, folds deterministic post-commit intents, applies the existing after-enrichment persistence override, and returns the verified commit fingerprint. It performs no provider write, scheduler dispatch, authority adoption, or terminal fold.
- **Replacement expectations:** a replacement must remain deterministic and provider-neutral, preserve canonical-input and provenance validation, use the configured enrichment/contribution ordering, propagate cancellation, and fail closed on changed identity or malformed input.

### `IRuntimeCheckpointPreparedLedgerStore` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Opt-in provider capability over `IRuntimeCheckpointCommitStore`; it has no default interface, sequential, or compatibility-token implementation.
- **Signature:** explicit `PrepareAsync(...)` and `CommitPreparedAsync(...)`, bounded stable `PagePreparedAsync(RuntimeCheckpointPreparedQuery, ...)`, provider-atomic exact-set `AdoptPreparedAsync(RuntimeCheckpointPreparedAdoptionRequest, ...)`, and `CommitPreparedFoldAsync(RuntimeCheckpointPreparedFoldRequest, ...)`.
- **Registration:** not selected by the default Immediate composition. `AddCoalescingRuntimeCheckpointPersistence` requires the chosen checkpoint provider to implement the capability and then registers the capability against the decorated checkpoint-store chain. The in-memory and Groundwork checkpoint providers are shipped implementations.
- **Usage:** retains canonical, digest-verifiable `Prepared` reservations before enrichment and exposes workflow-bound pages capped at 250. Adoption atomically re-reads and compares the complete ordered scope through its inclusive maximum; it changes only current authority fence and CAS revision, and exact replay at the already-current fence is idempotent. Terminal folding requires an explicit complete ordered prepared membership with explicit dispositions; it writes state/context/outbox once, terminalizes exact memberships, and compacts canonical input only in that successful transaction. Recovery invokes that fold only for an originally source-free contiguous prefix; live coalescing may fold the exact source-bound membership owned by its active durable dispatch. Merely registering the capability does not authorize source dispatch, synthetic recovery work, or inferred terminal outcomes.
- **Provider expectations:** paging must be stable and cursor-bound; preparation identity, source authority, canonical bytes/digest, provenance/order, and original fence/revisions remain immutable; mutable authority binding is separate. Every new adoption must atomically prove its strictly higher global fencing token is the durable active ownership lease; exact replay does not require that old lease to remain active. Adoption must reject missing, extra, partial, mixed, stale, downgrade, or forged-target requests atomically. A terminal fold must canonical-compare the caller payload with the shared Runtime Core committed-only aggregate, preserve exact terminal receipts and consumed orders, retain markers only for `Committed`, and leave no effects for explicit `Skipped`/`Failed` members. Live delivery may subtract only explicitly listed member-derived delivered outbox IDs.

### `IRuntimeCheckpointPreparedRecoveryCoordinator` *(Core — `Elsa.Workflows.Runtime.Core`)*
- **Kind:** Replacement recovery gate used by the Runtime drain orchestrator when coalescing is selected.
- **Signature:** `RecoverPreparedAsync(string workflowExecutionId, ...)` returns dispatch-ready or retry-required; unsafe, incomplete, ambiguous, stale, or provider-rejected recovery throws `RuntimeCheckpointPreparedRecoveryException`.
- **Usage:** runs before cadence resolution, a coalescing scope, or scheduler/source dispatch, including for an authored-Immediate execution under a coalescing composition. Recovery requires the ambient active ownership lease to match the requested workflow execution; the lease's exact `ToFence()` value is the only adoption/fold target, and absent or mismatched ownership fails before paging or mutation. It then pages and resolves the complete durable `Prepared` frontier. An exact source-bound set is adopted as one exact CAS while preserving each member's distinct original source authority and remains `Prepared` for normal source redelivery. Only an exact contiguous originally source-free prefix is deterministically rehydrated read-only and then authority-adopted plus terminally folded in one provider transaction. If that fold exposes a source-bound suffix, the coordinator returns retry-required and the orchestrator dispatches nothing; a later pass adopts the suffix from a fresh durable snapshot. No route creates synthetic scheduler work or infers a terminal disposition.
...
- **Kind:** Replacement (one factory opens the per-drain coalescing scope and enforces the quiescence stage gate).
...
- **Usage:** `WorkflowDrainOrchestrator` first runs `IRuntimeCheckpointPreparedRecoveryCoordinator` for every execution in a coalescing composition, including authored-Immediate executions. Only after recovery reports dispatch-ready does it consult `IRuntimeCheckpointCadenceResolver` (per-run stamp > authored-on-executable cadence > host default); an authored-Immediate run then skips the coalescing scope. Other dispatch-ready executions open a scope around the coalesced drain. Quiescence and eligible live boundaries terminally fold the session's exact buffered `Prepared` membership through the durable provider; any rejected fold preserves the session and fails closed before queue advancement. An empty active session may reconcile consumed queue work. A new `Committed`, empty/unmutated scheduler-only boundary may keep the session live: exact Pending rows are imported and acknowledgement is delayed. Overlay-only work is materialized durably before source-bound preparation, forces the Immediate handoff, and retires after a successful commit. `Replay`, nonqualifying, and failed boundaries deactivate into ordinary processing. Only registered by the opt-in extension.
...
- **Usage:** an `AsyncLocal` push/pop stack, mirroring `IRuntimeCoalescingSessionAccessor`. `WorkflowDrainOrchestrator.DrainCoreAsync` pushes a scope on the Immediate (no coalescing scope factory) branch, bounded by the RT-2 single-writer lease. While the scope is ambient, `RuntimePostCommitOutboxProcessor` delivers `EnqueueSchedulerWork` intents for the owning execution in-memory — idempotent `EnqueueAsync` plus a direct `Pending -> Delivered` mark via the existing non-claim recording contract — skipping the durable claim round-trip. The checkpoint committer may also restore a raw in-process scheduler-work conduit to this scope, but only after a new prepared finalization has `Committed` exactly one matching durable `EnqueueSchedulerWork` intent: its identity/association and both the raw candidate intent payload and independently serialized candidate work-item payload must structurally equal that authoritative durable payload. The carrier is never restored for replay, skip, failure, exception, mismatch, or active coalescing ownership; the durable payload remains truth. It never engages while a coalescing session is active (the overlay is authoritative) and leaves other intent kinds, other executions, and unmarked callers (recovery sweep, v1 stores) on the durable claim path. Registered by default because Immediate is the default cost model.
