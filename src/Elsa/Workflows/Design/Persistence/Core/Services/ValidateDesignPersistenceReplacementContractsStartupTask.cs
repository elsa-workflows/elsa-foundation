...
/// <summary>Fails startup when a replacement contract has more than one registered implementation.</summary>
public sealed class ValidateDesignPersistenceReplacementContractsStartupTask(IServiceCollection services) : IStartupTask
{
    public Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        var duplicates = DesignPersistenceBackend.ReplacementContractTypes
        ...
        if (duplicates.Length > 0)
            throw new InvalidOperationException($"Workflow-design replacement contracts must have exactly one implementation; duplicate registrations: {string.Join(", ", duplicates)}.");
