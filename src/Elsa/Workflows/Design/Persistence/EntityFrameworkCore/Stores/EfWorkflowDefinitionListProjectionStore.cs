...
public sealed class EfWorkflowDefinitionListProjectionStore(WorkflowsDesignDbContext db, IPersistenceAccessContextAccessor access) : IWorkflowDefinitionListProjectionStore
{
    public async Task<IReadOnlyList<WorkflowDefinitionListProjection>> ListByDefinitionIdsAsync(IReadOnlyCollection<string> ids, CancellationToken cancellationToken = default)
    {
        ...
        var drafts = await EfDesignSupport.InScope(db.Drafts.AsNoTracking(), access, x => x.TenantId).Where(x => distinct.Contains(x.WorkflowDefinitionId)).OrderByDescending(x => x.LastModifiedAt).ThenByDescending(x => x.Id).ToListAsync(cancellationToken);
        var versions = await EfDesignSupport.InScope(db.Versions.AsNoTracking(), access, x => x.TenantId).Where(x => distinct.Contains(x.DefinitionId)).OrderByDescending(x => x.SemVerSortKey).ThenByDescending(x => x.Id).ToListAsync(cancellationToken);
