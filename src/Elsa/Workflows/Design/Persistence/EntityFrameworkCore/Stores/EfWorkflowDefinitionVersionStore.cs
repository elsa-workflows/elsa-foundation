...
public sealed class EfWorkflowDefinitionVersionStore(WorkflowsDesignDbContext db, IPayloadSerializer serializer, IWorkflowDefinitionStore definitions, IPersistenceAccessContextAccessor access) : IWorkflowDefinitionVersionStore
{
    ...
    public async Task<WorkflowDefinitionVersion?> FindLatestVersionAsync(string definitionId, CancellationToken cancellationToken = default) { var row = await EfDesignSupport.ReadAsync("reading latest workflow definition version", () => Query().Where(x => x.DefinitionId == definitionId).OrderByDescending(x => x.SemVerSortKey).ThenByDescending(x => x.Id).FirstOrDefaultAsync(cancellationToken)); return row is null ? null : EfDesignSupport.MapVersion(serializer, row); }
    public async Task<IReadOnlyList<WorkflowDefinitionVersion>> ListByDefinitionAsync(string definitionId, CancellationToken cancellationToken = default) => (await EfDesignSupport.ReadAsync("listing workflow definition versions", () => Query().Where(x => x.DefinitionId == definitionId).OrderBy(x => x.SemVerSortKey).ThenBy(x => x.Id).ToListAsync(cancellationToken))).Select(x => EfDesignSupport.MapVersion(serializer, x)).ToArray();
    public Task<bool> ExistsAsync(string definitionId, string semVerSortKey, CancellationToken cancellationToken = default) => EfDesignSupport.ReadAsync("checking workflow definition version", () => Query().AnyAsync(x => x.DefinitionId == definitionId && x.SemVerSortKey == semVerSortKey, cancellationToken));
}
