...
public sealed class EfWorkflowDefinitionDraftStore(WorkflowsDesignDbContext db, IPayloadSerializer serializer, IPersistenceAccessContextAccessor access) : IWorkflowDefinitionDraftStore
{
    ...
    public async Task<WorkflowDefinitionDraft?> FindByWorkflowDefinitionIdAsync(string workflowDefinitionId, CancellationToken cancellationToken = default) { var key = EfDesignSupport.LookupHash(EfDesignSupport.SearchKey(workflowDefinitionId)); var row = await EfDesignSupport.ReadAsync("reading workflow definition draft", () => Query().Where(x => EF.Property<string>(x, "WorkflowDefinitionIdLookupHash") == key).OrderByDescending(x => x.LastModifiedAt).ThenByDescending(x => x.Id).FirstOrDefaultAsync(cancellationToken)); return row is null ? null : EfDesignSupport.MapDraft(serializer, row); }
    public async Task<IReadOnlyList<WorkflowDefinitionDraft>> ListByWorkflowDefinitionIdAsync(string workflowDefinitionId, CancellationToken cancellationToken = default) { var key = EfDesignSupport.LookupHash(EfDesignSupport.SearchKey(workflowDefinitionId)); return (await EfDesignSupport.ReadAsync("listing workflow drafts", () => Query().Where(x => EF.Property<string>(x, "WorkflowDefinitionIdLookupHash") == key).OrderByDescending(x => x.LastModifiedAt).ThenByDescending(x => x.Id).ToListAsync(cancellationToken))).Select(x => EfDesignSupport.MapDraft(serializer, x)).ToArray(); }
