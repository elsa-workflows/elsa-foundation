...
public sealed class EfCreateDraftCommand(WorkflowsDesignDbContext db, IPersistenceAccessContextAccessor access, IDesignAtomicWriter atomic, IIdentityGenerator identities, IPayloadSerializer serializer, IDistributedLockProvider? lockProvider = null, IInlineEventPublisher? inlineEvents = null, IDeferredEventPublisher? deferredEvents = null) : EfDesignCommand(db, access, atomic), ICreateDraftCommand
{
    ...
    private async Task<string> ExecuteCore(DesignOperationKey key, string definitionId, string? sourceVersionId, WorkflowDefinitionDraft draft, IReadOnlyCollection<DesignMetadataRecord> layout, CancellationToken ct)
    {
        ...
        var outcome = await Atomic.ExecuteAsync(key, "workflow.draft.create.v1", new CreateDraftRequestMaterial(definitionId, draft.StateSource!, EfDesignSupport.LayoutMaterial(layout), sourceVersionId), [DesignPersistenceUnitNames.Drafts, DesignPersistenceUnitNames.DraftLayouts], async (_, token) =>
        {
            ...
            var definition = await Scoped(Db.Definitions.AsNoTracking(), x => x.TenantId).SingleOrDefaultAsync(x => x.IdLookupHash == definitionKey, token) ?? throw EntityNotFoundException.ForEntity(typeof(WorkflowDefinition), definitionId);
    ...
    private async Task<WorkflowDefinitionVersionAdded> ExecuteCore(DesignOperationKey key, string definitionId, WorkflowDefinitionState state, CancellationToken ct, Action<string>? onVersionAllocated = null)
    {
        ...
        }, beforeAttempt: async token =>
        {
            ...
            var candidates = await Scoped(Db.Versions.AsNoTracking(), x => x.TenantId)
                .Where(x => x.DefinitionIdLookupHash == actualDefinitionKey)
            ...
            var result = await Atomic.ExecuteAsync(
                ...
                async (_, token) =>
                {
                    ...
                    var layout = new WorkflowDefinitionVersionLayout
                    {
                        ...
                        CreatedAt = Now,
                        LastModifiedAt = Now,
                        Records = draftLayout is null ? [] : EfDesignSupport.ReadLayout(draftLayout.RecordsJson),
                        ActivityPresentation = draftLayout is null ? [] : EfDesignSupport.ReadPresentation(draftLayout.ActivityPresentationJson)
                    };
                    Db.Versions.Add(row);
