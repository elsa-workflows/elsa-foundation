...
public abstract class EfDesignCommand(WorkflowsDesignDbContext db, IPersistenceAccessContextAccessor access, IDesignAtomicWriter atomic)
{
    ...
    protected static T RequireOutcome<T>(DesignAtomicWriteResult<T> outcome, string operationKind, DesignOperationKey key) => outcome.Status switch
    {
        DesignAtomicWriteStatus.Committed or DesignAtomicWriteStatus.Reconciled or DesignAtomicWriteStatus.Replayed => outcome.Value!,
        DesignAtomicWriteStatus.Conflict => throw new InvalidOperationException($"Design operation '{operationKind}/{key.Value}' conflicts with an earlier request."),
...
{
    public async Task Execute(DesignOperationKey key, WorkflowDefinition definition, CancellationToken ct = default) { ArgumentNullException.ThrowIfNull(definition); WorkflowDefinitionLimits.Validate(definition); Tenant(definition.TenantId); await Atomic.ExecuteAsync(key, "workflow.definition.save.v1", new SaveWorkflowDefinitionRequestMaterial(definition.Id, definition.Name, definition.Description, definition.DeletedAt, definition.DeletedReason, definition.IsSourceOwned), [DesignPersistenceUnitNames.Definitions], async token => { var lookupHash = EfDesignSupport.LookupHash(EfDesignSupport.SearchKey(definition.Id)); var row = await Scoped(Db.Definitions, x => x.TenantId).SingleOrDefaultAsync(x => EF.Property<string>(x, "IdLookupHash") == lookupHash, token) ?? throw EntityNotFoundException.ForEntity(typeof(WorkflowDefinition), definition.Id); EfDesignSupport.EnsureDefinitionIdentity(definition.Id, row.Id, "workflow definition save lookup"); row.Name = definition.Name; row.Description = definition.Description; row.DeletedAt = definition.DeletedAt; row.DeletedReason = definition.DeletedReason; row.IsSourceOwned = definition.IsSourceOwned; row.LastModifiedAt = Now; EfDesignSupport.SetDefinitionSearchKeys(Db, row); return definition.Id; }, ct); }
}
...
public sealed class EfCreateDraftCommand(WorkflowsDesignDbContext db, IPersistenceAccessContextAccessor access, IDesignAtomicWriter atomic, IIdentityGenerator identities, IPayloadSerializer serializer, IDistributedLockProvider? lockProvider = null, IInlineEventPublisher? inlineEvents = null, IDeferredEventPublisher? deferredEvents = null) : EfDesignCommand(db, access, atomic), ICreateDraftCommand
{
    ...
    private async Task<string> ExecuteCore(DesignOperationKey key, string definitionId, string? sourceVersionId, WorkflowDefinitionDraft draft, IReadOnlyCollection<DesignMetadataRecord> layout, CancellationToken ct)
    {
        ...
        var outcome = await Atomic.ExecuteAsync(key, "workflow.draft.create.v1", new CreateDraftRequestMaterial(definitionId, draft.StateSource!, EfDesignSupport.LayoutMaterial(layout), sourceVersionId), [DesignPersistenceUnitNames.Drafts, DesignPersistenceUnitNames.DraftLayouts], async (_, token) =>
        {
            ...
            EfDesignSupport.EnsureDefinitionIdentity(definitionId, definition.Id, "workflow definition draft creation lookup");
            draft.WorkflowDefinitionId = definition.Id;
            draft.TenantId = WriteTenant(definition.TenantId);
            if (inlineEvents is not null)
                errors = (await inlineEvents.DeriveValidationErrorsAsync(draft, token)).ToArray();
            Db.Drafts.Add(draft);
            var sibling = WorkflowDefinitionDraftLayout.CreateFor(identities, draft.Id, layout);
            sibling.TenantId = draft.TenantId;
            EfDesignSupport.Stamp(sibling, Now);
            EfDesignSupport.SetLayout(Db, sibling, layout);
...
public sealed class EfCloneDraftFromVersionCommand(WorkflowsDesignDbContext db, IPersistenceAccessContextAccessor access, IDesignAtomicWriter atomic, IIdentityGenerator identities, IPayloadSerializer serializer, IDistributedLockProvider? lockProvider = null, IInlineEventPublisher? inlineEvents = null, IDeferredEventPublisher? deferredEvents = null) : EfDesignCommand(db, access, atomic), ICloneDraftFromVersionCommand
{
    public async Task<string> Execute(DesignOperationKey key, string sourceVersionId, CancellationToken ct = default)
    {
        ...
        try
        {
            var outcome = await Atomic.ExecuteAsync(key, "workflow.draft.clone-from-version.v1", new CloneDraftRequestMaterial(sourceVersionId), [DesignPersistenceUnitNames.Drafts, DesignPersistenceUnitNames.DraftLayouts], async (_, token) =>
            {
                ...
                Db.Drafts.Add(draft); var sibling = WorkflowDefinitionDraftLayout.CreateFor(identities, id, records, presentation); sibling.TenantId = source.TenantId; EfDesignSupport.Stamp(sibling, Now); EfDesignSupport.SetLayout(Db, sibling, records, presentation); Db.DraftLayouts.Add(sibling); return DesignAtomicWriteStage<string>.Accepted(id);
            }, cancellationToken: ct);
...
public sealed class EfAddWorkflowDefinitionVersionCommand(WorkflowsDesignDbContext db, IPersistenceAccessContextAccessor access, IDesignAtomicWriter atomic, IPayloadSerializer serializer, IIdentityGenerator identities, IDistributedLockProvider? lockProvider = null) : EfDesignCommand(db, access, atomic), IAddWorkflowDefinitionVersionCommand
{
    ...
    private async Task<WorkflowDefinitionVersionAdded> ExecuteCore(DesignOperationKey key, string definitionId, WorkflowDefinitionState state, CancellationToken ct, Action<string>? onVersionAllocated = null)
    {
        ...
        var outcome = await Atomic.ExecuteAsync(key, "workflow.version.add.v1", new AddWorkflowDefinitionVersionRequestMaterial(definitionId, stateJson), [DesignPersistenceUnitNames.Versions], async (_, token) =>
        {
            var definitionKey = EfDesignSupport.LookupHash(EfDesignSupport.SearchKey(definitionId));
            var definition = await Scoped(Db.Definitions.AsNoTracking(), x => x.TenantId).SingleOrDefaultAsync(x => EF.Property<string>(x, "IdLookupHash") == definitionKey, token) ?? throw EntityNotFoundException.ForEntity(typeof(WorkflowDefinition), definitionId);
            EfDesignSupport.EnsureDefinitionIdentity(definitionId, definition.Id, "workflow definition version allocation lookup");
        ...
        }, beforeAttempt: async token =>
        {
            ...
            var candidates = await Scoped(Db.Versions.AsNoTracking(), x => x.TenantId)
                .Where(x => EF.Property<string>(x, "DefinitionIdLookupHash") == actualDefinitionKey)
                .OrderByDescending(x => x.SemVerSortKey)
                .ToListAsync(token);
            foreach (var candidate in candidates)
                EfDesignSupport.EnsureDefinitionIdentity(definition.Id, candidate.DefinitionId, "workflow definition version allocation lookup");
