    ...
    public async Task<string> Execute(DesignOperationKey key, string sourceVersionId, CancellationToken ct = default)
    {
        ...
        try
        {
            result = await Atomic.ExecuteAsync(key, "workflow.draft.clone-from-version.v1", new { sourceVersionId }, [DesignPersistenceUnitNames.Drafts, DesignPersistenceUnitNames.DraftLayouts], async token =>
            {
                ...
                staged = true; var id = identities.Generate(); var draft = Draft(id, source.DefinitionId, source.TenantId, state, sourceVersionId); SaveDraftState(draft, state);
