...
**Known implementations (shipped):** every aggregating handler in the repo — `ExecuteValidations`, `RegisterJsonConverters`, `PreProcessScript`, `PostProcessScript`, `BuildDeclarationsDocument`, `RegisterActivityImplementationResolvers`, `RegisterImplementationDescriptors`, `ApplyEntitySavingHandlers`, `ApplyEntityLoadingHandlers`, `ActivityVersionsReconcilingHandler`, `WorkflowVersionsReconcilingHandler`, and others. See each domain's feature catalog for detail.
