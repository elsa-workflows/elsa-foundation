...
/// activities-design and workflows-publishing — with a single Groundwork SQLite document store. Composing this one feature is all a host
...
[ShellFeature(
    ...
    Description = "Backs the workflow runtime, workflows-design, activities-design and workflows-publishing persistence seams with a single Groundwork SQLite document store. Durable storage keeps runtime checkpoints, publication authority, post-commit outbox items and queued scheduler work across a crash; compose alongside Workflows Runtime Resumption so a background pump re-drives that work after a restart.",
