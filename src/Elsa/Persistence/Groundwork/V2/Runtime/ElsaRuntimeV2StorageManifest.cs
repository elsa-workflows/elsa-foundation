...
public static class ElsaRuntimeV2StorageManifest
{
    ...
    [
        Unit(BookmarkStateDocumentKind, "runtime_bookmark_state", [
            String(WorkflowExecutionIdField, 128), String(StimulusHashField), String(StimulusTypeField), String(StimulusLookupKeyField), String(StimulusTypeLookupKeyField), String("bookmarkId", 128)], [
            ...
            IncludedIndex("by-stimulus-and-type-and-bookmark-identity", StimulusLookupKeyField, WorkflowExecutionIdField, "bookmarkId"),
            IncludedIndex("by-stimulus-type-and-bookmark-identity", StimulusTypeLookupKeyField, WorkflowExecutionIdField, "bookmarkId"),
            IncludedIndex("by-workflow-execution-and-bookmark-id", WorkflowExecutionIdField, "bookmarkId")]),
        ...
        Unit(DurableTimerDocumentKind, "runtime_durable_timer", [String(CollectionField, 128), String(WorkflowExecutionIdField, 128), String(DurableTimerIdField, 128), DateTime(DurableTimerDueTimeField), String(DurableTimerClaimOrderKeyField, 256)], [Index(ByCollectionIndex, CollectionField), Index(ByWorkflowExecutionIndex, WorkflowExecutionIdField), Index("by-due-time", DurableTimerDueTimeField), IncludedIndex("by-due-time-and-timer-id", DurableTimerDueTimeField, DurableTimerIdField), Index(ByTimerIdIndex, DurableTimerIdField), IncludedIndex("by-workflow-execution-and-timer-id", WorkflowExecutionIdField, DurableTimerIdField), IncludedIndex("by-claim-order", DurableTimerClaimOrderKeyField)]),
        Unit(WorkflowTriggerBindingDocumentKind, "runtime_workflow_trigger_binding", [String(StimulusHashField), String(StimulusTypeField, 240), String(StimulusLookupKeyField), String(StimulusTypeLookupKeyField), Boolean(WorkflowTriggerBindingIsActiveField), String(TriggerBindingIdField, 128), String(ArtifactIdField, 128), String(PublicationIdField, 128)], [Index(WorkflowTriggerBindingByActive, WorkflowTriggerBindingIsActiveField), Index(ByArtifactIndex, ArtifactIdField), IncludedIndex("by-artifact-and-trigger-binding-id", ArtifactIdField, TriggerBindingIdField), Index(ByPublicationIndex, PublicationIdField), IncludedIndex("by-publication-and-trigger-binding-id", PublicationIdField, TriggerBindingIdField), Index(ByStimulusIndex, StimulusHashField), IncludedIndex("by-stimulus-and-type", StimulusLookupKeyField, WorkflowTriggerBindingIsActiveField, TriggerBindingIdField), Index(ByStimulusTypeIndex, StimulusTypeField), IncludedIndex("by-stimulus-type-and-active", StimulusTypeLookupKeyField, WorkflowTriggerBindingIsActiveField, TriggerBindingIdField), UniqueIndex("by-trigger-binding-id", TriggerBindingIdField)]),
        ...
        IReadOnlyList<IndexSpec> indexes)
    {
        ...
        var uniqueColumns = indexes
            .Where(index => index.Unique)
            .SelectMany(index => index.Columns)
            .ToHashSet(StringComparer.Ordinal);
        foreach (var column in columns)
            column.AddTo(declaration, uniqueColumns.Contains(column.Name));
        foreach (var index in indexes)
            index.AddTo(declaration);

        var built = declaration.Build();
        var missingValues = indexes.ToDictionary(index => index.Name, index => index.MissingValues, StringComparer.Ordinal);
        return built with
        {
            SchemaVersion = StorageSchemaVersion,
            Indexes = built.Indexes
                .Select(index => missingValues.TryGetValue(index.Name, out var missing) ? index with { MissingValues = missing } : index)
                .ToArray()
        };
    }
    ...
    {
        public void AddTo(StorageDeclarationBuilder declaration, bool required)
        {
            Action<ColumnBuilder>? configure = required ? column => column.Required() : null;
            switch (Type)
            {
                case PortableType.String:
                    declaration.String(Name, MaxLength ?? IdMaximumLength, configure);
                    break;
                case PortableType.Int64:
                    declaration.Int64(Name, configure);
                    break;
                case PortableType.DateTimeOffset:
                    declaration.Timestamp(Name, configure);
                    break;
                case PortableType.Boolean:
                    declaration.Boolean(Name, configure);
                    break;
                default:
        ...
        MissingValueBehavior MissingValues = MissingValueBehavior.Excluded)
    {
        ...
        {
            if (Unique)
                declaration.UniqueIndex(Name, Columns.ToArray());
            else
