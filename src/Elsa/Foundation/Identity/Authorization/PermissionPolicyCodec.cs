...
public sealed class PermissionPolicyCodec : IPermissionPolicyCodec
{
    ...
    public PermissionPolicyParseResult Parse(string policyName)
    {
        ...
        var descriptor = mode.Value switch
        {
            PermissionRequirementMode.Single => PermissionPolicyDescriptor.Single(permissions[0]),
            PermissionRequirementMode.Any => PermissionPolicyDescriptor.Any([.. permissions]),
            PermissionRequirementMode.All => PermissionPolicyDescriptor.All([.. permissions]),
            _ => throw new ArgumentOutOfRangeException(nameof(policyName))
        };
        return new(PermissionPolicyParseStatus.Valid, descriptor);
    }
