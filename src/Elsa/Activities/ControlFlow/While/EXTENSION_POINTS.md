# Elsa.Activities.While Extension Points

This module does not expose replaceable service contracts in v1. Its activity-owned contracts are:

- `While.Body` child slot (at most one body activity)
- A `Done` composite outcome emitted once the condition no longer holds
...
- `WhileStructureHandler` implements `IActivityStructureHandler` (`Elsa.Workflows.Design.Core`). It
  ...
  re-reading the design document. `While` is a condition-only loop with no per-iteration item or index and
