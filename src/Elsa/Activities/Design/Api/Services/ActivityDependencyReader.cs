...
public sealed class HmacActivityDependencyCursorCodec(IOptions<ActivityTokenSigningOptions> options) : IActivityDependencyCursorCodec
{
    private readonly HmacTokenCodec<ActivityDependencyCursorState> _codec = new(options.Value.SigningKey, "Activity dependency cursor");

    public string Encode(ActivityDependencyCursorState state) => _codec.Encode(state);

    public ActivityDependencyCursorState Decode(string cursor) =>
        _codec.Decode(cursor) is { Position: >= 0 } state && !string.IsNullOrWhiteSpace(state.Watermark)
            ? state
