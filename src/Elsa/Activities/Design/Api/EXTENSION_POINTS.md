...
| `IActivityAuthoringContextAsync` / `IActivityDependencyAuthorizationContextAsync` | `HttpContextActivityDesignAuthorizationContext` | Async request authorization seams for authoring, provider payload, management, and tenant-scoped dependency decisions. They delegate permission evaluation to Foundation Identity; provider-specific rules receive a stable resource object. |
