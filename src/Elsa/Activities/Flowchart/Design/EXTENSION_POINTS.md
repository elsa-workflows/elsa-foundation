# Extension points — Flowchart composite (Design)

T128 split `Elsa.Activities.Flowchart` into a `.Design` and a `.Runtime` package so a runtime-only engine can compose
...
|---|---|---|
| Authored Flowchart structure | `ActivitiesFlowchartDesignFeature` registers `FlowchartStructureHandler : IActivityStructureHandler` | Implemented |
| Container-scoped variables | `FlowchartStructureHandler.ProjectScopedVariables` (ADR 0027) | Implemented |
