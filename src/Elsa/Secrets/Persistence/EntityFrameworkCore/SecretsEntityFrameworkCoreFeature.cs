...
public class SecretsEntityFrameworkCoreFeature : IShellFeature
{
    ...
    [ManifestSetting(
        ...
        Description = "AutoMigrate runs Database.MigrateAsync (EF 9 lock) on feature enable and CShells reload. Validate fails when migrations are pending or legacy projection rows still require the out-of-process apply/reindex path.",
        Category = "Persistence")]
