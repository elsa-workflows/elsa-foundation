...
/// <summary>
/// A stable identity reused for every attempt to commit one drain batch. <see cref="IssuedAt"/> is
/// captured with the identity so adapters which require clock-bound operation ids can reuse the exact
/// same operation across acknowledgement-loss retries.
/// </summary>
public readonly record struct DiagnosticsDrainBatchId(Guid Value, DateTimeOffset IssuedAt)
{
    public DiagnosticsDrainBatchId(Guid value) : this(value, DateTimeOffset.UtcNow) { }

    public static DiagnosticsDrainBatchId New() => new(Guid.NewGuid(), DateTimeOffset.UtcNow);
