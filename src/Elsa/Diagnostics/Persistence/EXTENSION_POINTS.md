...
### `IDiagnosticsPersistenceDrain` *(Feature contract — `Elsa.Diagnostics.Persistence`)*

- **Kind:** additive host-lifecycle contribution; one contribution per selected singleton store.
- **Role:** lets a concrete diagnostics adapter expose explicit start and bounded stop without leaking
  provider types into host composition.
- **Registration:** the concrete store must already have exactly one singleton registration, then
  `AddDiagnosticsPersistenceLifecycle<TDrain>` aliases that same instance into the shared lifecycle.
...
Stores that own a capture drain are an explicit singleton exception: the lifecycle registration
rejects missing, duplicate, or non-singleton concrete registrations. The live-feed observer,
counters, and bridge are also explicitly singleton because their totals span subscriptions and they
hold neither payloads nor scoped dependencies.
