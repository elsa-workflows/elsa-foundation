...
public sealed class CatalogDocumentSerializer
{
        ...
        long? expectedRevision)
    {
        try
        {
            return new(
                documentKind,
                id,
                CatalogDocuments.SchemaVersion,
        ...
        where TDocument : class
    {
        try
        {
            ...
            if (!StringComparer.Ordinal.Equals(envelope.SchemaVersion, CatalogDocuments.SchemaVersion))
                throw new CatalogPayloadException(
                    $"OpenTelemetry catalog schema '{envelope.SchemaVersion}' is not supported.");
            if (envelope.Version < 0)
                throw new CatalogPayloadException("The OpenTelemetry catalog revision cannot be negative.");

            var document = JsonSerializer.Deserialize<TDocument>(envelope.ContentJson, SerializerOptions)
                           ?? throw new CatalogPayloadException("The OpenTelemetry catalog payload is empty.");
            return map(document);
        }
        catch (CatalogPayloadException)
        {
    ...
    {
        if (!StringComparer.Ordinal.Equals(envelope.Id, id))
            throw new CatalogPayloadException("The OpenTelemetry catalog payload identity does not match its envelope.");
        if (!StringComparer.Ordinal.Equals(envelope.Id, retentionTieBreaker))
