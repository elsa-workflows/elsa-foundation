...
Captures host log events (`Microsoft.Extensions.Logging`) into an in-memory store and exposes them to Elsa Studio over HTTP (recent history, known sources) and Server-Sent Events (live tail). It is a **server** shell feature; persistence is deferred behind the store contract so a future slice can swap in durable storage without touching capture, transport, or the UI.

Feature name (manifest / appsettings key): **`DiagnosticsStructuredLogs`**.

## What this feature provides

- **`InMemoryStructuredLogStore`** — the default implementation of `IStructuredLogStore`, `IStructuredLogLiveFeed`, and `IStructuredLogSink` (registered as one shared singleton). A bounded ring buffer for recent history plus an independent bounded channel per live subscriber.
...
All four store/source contracts are overridable — see [`EXTENSION_POINTS.md`](EXTENSION_POINTS.md). The common extension is replacing `IStructuredLogStore` (and usually `IStructuredLogLiveFeed`) with a persistent implementation while leaving capture and transport unchanged.
