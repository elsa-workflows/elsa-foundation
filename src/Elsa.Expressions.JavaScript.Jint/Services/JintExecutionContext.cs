...
namespace Elsa.Expressions.JavaScript.Jint.Services
{
    public sealed class JintExecutionContext(IPreparedScriptFactory preparedScriptFactory, Engine engine) : IJavaScriptExecutionContext
    {
        ...
        {
            engine.SetValue<Func<object[], object?>>(
                function.Name,
                (parameters) => function.Execute(this, parameters)
            );
        }

        public void RegisterObject(IJavaScriptObject @object)  => engine.SetValue(@object.Name, @object.Value);
