<#
.SYNOPSIS
    SetVariable intrinsic: assign a workflow-scoped variable, then read it back.
.DESCRIPTION
    ...
    visible declaring scope. The simplest end-to-end-working shape is a WORKFLOW-SCOPE variable (declared in
    `state.Variables` via `Submit-Workflow -Variables`) referenced with no `declaringScopeId`, as below.
    Container scope also works when the declaration is on the container and every reference supplies that
    container's node id as `declaringScopeId`; only a declaration/reference mismatch fails (see ../README.md).
