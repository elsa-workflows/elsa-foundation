...
- **FR-004**: Each contributing feature assembly MUST embed its fragment as an assembly resource at build time, byte-identical to the fragment merged into the repository artifact.
...
- **FR-013**: The emitter MUST run as a build-integrated step whose diagnostics surface as canonical build warnings/errors (visible to IDEs, CI, and coding agents), MUST be process-isolated from the build it observes, and MUST also be runnable standalone against an arbitrary set of feature assemblies (the same tool consumers run against their own activity packages).
