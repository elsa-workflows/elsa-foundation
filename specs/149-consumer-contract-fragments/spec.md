...
3. **Given** a consumer's own feature selection (their shell composition), **When** they intersect the merged fragments with their enabled feature ids **and with the set of fragments their host actually ships**, **Then** they obtain exactly their composition's static catalog. *(Revised after consumer validation of `207326e0c`: the two-way intersection the RFC states is incomplete — a feature whose assembly the host does not reference cannot be enabled at all, and the shell reports it only as a log line. Availability is `fragments ∩ shells.json ∩ hosts.json[host]`; the third term is published as `docs/contracts/hosts.json`.)*
...
- **FR-005a**: The merged directory MUST publish, per shipped host, the set of fragments that host actually contains, so a consumer can determine real availability without booting or introspecting an image. *(Added after consumer validation: fragments are host-agnostic by design, so without this term a consumer correctly reads a fragment and still wrongly concludes the feature is enableable. A runtime-kind classification cannot express this — two features sharing `elsa.server` can differ in whether a given host ships them.)*
