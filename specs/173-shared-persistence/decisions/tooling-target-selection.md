...
Resource name equality is neither necessary nor sufficient for physical target equality. For aliases that resolve to the same provider and connection reference in the same configuration snapshot, reference agreement can be established offline. When different references might resolve to the same database, report target affinity as unverified offline; do not declare an invalid layout merely because the reference or resource names differ.
