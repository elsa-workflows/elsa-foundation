...
- [ ] T031 [US2] Add legacy host regression journeys in `tests/essentials/Secrets/Persistence/EntityFrameworkCore/Tests/SecretsEntityFrameworkCoreShellReloadTests.cs`, `tests/essentials/Persistence/EntityFrameworkCore/Migrations/Tests/ModuleMigrationTests.cs`, and `e2e-tests/durability/Test-RestartRecovery.ps1` proving resource mode is absent and existing behavior remains intact.
  Legacy Secrets reload and module migration tests pass. The isolated Workbench restart harness confirms no resource selection and persisted suspended state, but its post-restart stimulus reproduces the existing `resumedCount=0` defect in [#1761](https://github.com/elsa-workflows/elsa-foundation/issues/1761); the no-restart control passes. Keep this task open until that defect is resolved or the acceptance boundary is explicitly revised.
