...
## R7. Configuration, presence and reviewed tooling refinements

The [authored configuration decision](decisions/authored-persistence.md) selects a root resource catalog/root default with shell-default and feature-binding overrides. It also records the exact null behavior: nested shell resource null leaves survive the published CShells blueprint, while null legacy feature target fields require a narrow raw IConfiguration presence read. An isolated published-package probe confirmed both and confirmed that public feature discovery/dependency expansion constructs no features. The probe is narrower than an Elsa runtime proof.

The [target selection decision](decisions/tooling-target-selection.md) defines deterministic provider/reference groups, alias handling, from-host filtering and exact explicit/all selections. Offline commands validate identities and report live prerequisites without looking up expected secret values. Live commands verify every distinct expected reference of every enabled owner of a selected context before database activity.
