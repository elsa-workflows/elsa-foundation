...
Use one explicit shell and one explicit shared resource. The CLI implements this invocation shape; the disposable PostgreSQL Workbench receipt above exercises the four-module sequence:
...
The legacy feature editor now runs mandatory resource applicability preparation after restoring masked secrets and validating the request, before ordinary activation guards, save, catalog refresh and shell reload. `FeatureManagementService.ApplyAsync` owns that order. Current-only, candidate-only, and invalid applicable resource selections return HTTP 409 with the `[resource-managed-configuration]` prefix in the existing `errors.generalErrors` message before mutation; wholly legacy requests retain the prior path. Integrated preparer tests assert zero ordinary-guard, save, refresh and reload calls on refusal. The live Workbench editor probe returned this prefix and left its feature revision unchanged. This establishes the observed guard boundary, not a transactional guarantee for all possible external feature stores.
