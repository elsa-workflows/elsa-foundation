...
3. Trigger the host's supported reload operation. The Foundation Host's optional module-management reload is `POST /_module-management/reload`, protected by its configured API-key header ([source](../../src/apps/Elsa.Foundation.Host/ModuleManagement/ModuleManagementEndpoints.cs)); use the actual enabled host surface and credential injection, not a guessed endpoint on Workbench.
