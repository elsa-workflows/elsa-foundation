...
**Created**: 2026-06-11
**Status**: Implemented
Implemented: #45, #46 (restatused 2026-09-10 from git evidence)
...
5. Given a different commit appends an incident key that already exists, then the writer rejects the second commit before recording it.
...
- **FR-006**: Incident append projection MUST be insert-only for the incident key; terminal or corrective replacements MUST use upsert.
- **FR-007**: Runtime.Core MUST keep incident history projection outside continuation state.
...
- Distinct duplicate incident appends are rejected before write recording.
