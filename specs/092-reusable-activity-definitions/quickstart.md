...
- Mandatory SQLite gate: `ActivityDraftTestRunTests.Groundwork_sqlite_graph_run_suspends_restarts_in_runtime_only_host_then_reopens_status` passed in Release. Generation 1 used a combined host to publish and suspend a real `GraphActivity`; all host/store state was disposed. Generation 2 used Runtime, Activities Runtime, Graph Runtime, and Groundwork SQLite only, resumed the same bookmark/workflow, preserved execution ids/sequences, captured the boundary input once, propagated the required output once, and read the executed Source Reference layout/hierarchy. Generation 3 reopened the full host, recovered the durable Test Run receipt by identity and idempotency key, reconciled completion plus the outer activity execution identity, and retained independent source-reference, Runtime Evidence, active-run, and receipt-expiry facts.
