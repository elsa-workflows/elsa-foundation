...
- [X] T040 [US2] Persist and read content-addressed executable activity templates plus closed Runtime requirements in `src/Elsa/Workflows/Runtime/Services/Executables/InMemoryExecutableActivityTemplateStore.cs` and `src/Elsa/Persistence/Groundwork/Stores/GroundworkExecutableActivityTemplateStore.cs`
...
- [X] T044 [US2] Fold graph completion/output capture/Done/outer terminalization/parent intent into one checkpoint in `src/Elsa/Activities/Runtime/Services/WorkflowParentActivityCompletionSchedulerWorkHandler.cs` and `src/Elsa/Workflows/Runtime/Services/WorkHandlers/WorkflowCompleteActivitySchedulerWorkHandler.cs`
...
- [X] T076 [US5] Classify unresolved consumers as recoverable deployment incidents outside ordinary activity retry in `src/Elsa/Workflows/Runtime/Services/Incidents/ActivityActivationFailureHandler.cs`
...
- [X] T085 [US6] Extend reference-derived cleanup to activity-template and test-run references in `src/Elsa/Workflows/Runtime/Services/Executables/WorkflowExecutableReferenceGarbageCollector.cs`
