...
> **Amendment 2026-09-16 (EF Core persistence program, issue #1738).** The Storage line, the "no new EF persistence work" constraint, the ADR 0042 constitution-check row, and principle 17 record this plan as ratified in July 2026, and are kept as that history. Accepted [ADR 0073](../../docs/adr/0073-ef-core-is-the-only-first-party-persistence-family.md) has since superseded ADR 0042's Groundwork-only policy. The durable stores this plan introduced are gaining opt-in EF Core implementations one storage unit at a time, and the [storage-unit register](../../docs/reports/ef-core-persistence/storage-unit-register.md) tracks which. The Elsa 3 import ledger (L01-L03: collections, receipts, and definition bindings) now has one; its import commits the receipt, the bindings, and the Activity and Workflow Design rows in one shared EF transaction. Groundwork remains the default composition, and EF migrations and the default flip are separate gates.
...
| Framework §2.21/§2.23 test preservation and branch coverage | PASS with implementation work | Existing non-legacy behavior tests remain; all new logic-bearing classes and feature registration paths require focused tests. Tests tied only to the intentionally removed workflow-as-activity surface are migrated into replacement/absence and explicit `ExecuteWorkflow` preservation coverage before the obsolete project is removed. |
