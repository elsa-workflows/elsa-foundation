...
- [X] T032 [P] [US3] `WellKnownTypeRegistryTests`: duplicate alias throws `DuplicateTypeAliasException`; duplicate type throws; bare non-primitive alias throws `ReservedAliasNamespaceException`; happy-path resolve; nullable auto-registration does not self-collide. File: `Unit/WellKnownTypeRegistryTests.cs`. **Done** in `tests/Elsa/Serialization/Tests/Unit/WellKnownTypeRegistryTests.cs` (9 tests incl. idempotent re-registration, dotted module alias, all reserved primitives seed clean).
- [X] T033 [P] [US3] Graceful-unknown test: a `VariableDefinition` with an unregistered alias round-trips the alias verbatim and `VariableMapper` resolves it to `object` with a warning (no throw). File: Expressions test project. **Done** in `tests/Elsa/Expressions/Tests/Unit/UnknownAliasRoundTripTests.cs` (also covers the T037 wrap).

### Implementation for User Story 3

- [X] T034 [US3] Harden `WellKnownTypeRegistry.RegisterType`: throw `DuplicateTypeAliasException` on repeat alias or repeat type (was silent last-writer-wins); keep nullable auto-registration collision-safe. File: `src/Elsa/Serialization/SystemText/Services/WellKnownTypeRegistry.cs`. **Done**: throws on repeat alias / repeat type-under-different-alias; identical (type, alias) re-registration is an idempotent no-op; nullable derivation extracted to `RegisterDerivedNullable` (only-add-when-absent, no throw, bypasses reserved check).
- [X] T035 [US3] Add the reserved-namespace guard: a bare (non-dotted) alias not in the framework-reserved primitive set throws `ReservedAliasNamespaceException`; primitives register through the trusted seed path. Same file. **Done**: `public static readonly IReadOnlySet<string> ReservedBareAliases` (BCL primitive names) gates only the incoming alias; dotted aliases always allowed.
- [X] T036 [US3] Audit existing `RegisterType` callers so the seed registers each alias exactly once under the new throw-on-duplicate behavior (grep `RegisterType`, `WellKnownTypeRegistry`). **Done**: `SeedWellKnownTypesStartupTask` is the single production registration site (with a `seen` dedupe); Elsa3 mappers and publishing/runtime consumers only read the registry; the `IDictionary` ctor + parameterless ctor are test-only. No pre-existing duplicate-registration bug found. Verified by full solution build + runtime (509) and publishing (46) suites green.
- [X] T037 [US3] Wrap `JsonException` at the argument (de)serialization boundary into a domain exception (§2.23.5), preserving inner exception. Files: serialization converters in the authored path. **Done**: `VariableConverter.Read` wraps `JsonException` → new `ArgumentDefinitionDeserializationException` (in `Elsa.Serialization.Core.Exceptions`), preserving `InnerException`.
- [X] T038 [US3] Document `IWellKnownTypeRegistry` fail-fast + frozen-alias contract in XML docs. File: `src/Elsa/Serialization/Core/IWellKnownTypeRegistry.cs`. **Done**: `RegisterType` XML docs now state the two `<exception>`s and the frozen-alias remark.
