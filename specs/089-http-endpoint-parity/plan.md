...
3. **Middleware mounting** (`HttpEndpointMiddlewareShellFeature : IMiddlewareShellFeature`): `Configure(app) → app.UseMiddleware<HttpEndpointMiddleware>()`, `Order` chosen after the auth middleware contribution (verify against CShells FastEndpoints feature ordering); registered from `ActivitiesHttpFeature.ConfigureServices`. Requests outside `HttpEndpointOptions.BasePath` pass through (existing behavior, now covered by a host-level test).
