...
The #2068 [test-owned receipt proof](../../docs/reports/runtime-composition/deployment-source-receipt-boundary.md) confirms that a deployer can refuse mismatched copies and overrides at its own launch boundary, but this does not make Workbench attest that receipt. The next design work must give Workbench a host-owned startup context for retained source and effective override winners, consumed by both root and shell composition, before an external receipt can be compared with a process and generation. Do not add a `verified` candidate-match branch to the current observer from a caller-provided label or later directory scan.
