...
**Current delivery boundary after #2060**: The host observer reports default-shell reload outcome, active generation and readiness with `candidateMatch=unverified`. It does not consume an external deployment receipt or attribute a reviewed candidate to the generation. A rebuilt-host probe showed that a fresh process can use one retained complete file copy, while file-at-a-time deployment can mix source revisions. Startup-bound changes therefore require a complete immutable artifact and process restart; a generation number alone cannot be compared across processes. Scenario 1's exact `candidate active` result still requires trusted deployment, process-start source/override, package-cohort and generation correlation.
