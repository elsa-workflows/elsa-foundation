...
3. **Given** the workflow declares inputs `{amount, approver}` and outputs `{decision}` plus outcomes `{Approved, Rejected}`, **When** the row is produced, **Then** the activity row's `Inputs`, `Outputs`, and `Ports` mirror them so a future consumer can bind to the shape.
...
- **A workflow is marked usable-as-activity but declares no inputs/outputs.** A valid catalog row is still produced with empty `Inputs`/`Outputs`/`Ports`. The marking, not the I/O, gates row production.
...
- **`WorkflowActivityOptions.UsableAsActivity`** *(existing, reused — `Elsa.Workflows.Design.Core.Models`)* — the `bool?` marking inside `WorkflowDefinitionState`; co-resident `ActivityCategory` and `Outcomes` feed the row's `Category` and `Ports`. No model change.
