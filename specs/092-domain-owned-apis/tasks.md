...
- [X] T011 Record in `specs/092-domain-owned-apis/migration-matrix.md` that every Runtime kind uses a current-only pre-GA boundary with exactly one fixture and no Elsa upcaster machinery, requiring a complete dependent Runtime/Publishing reset for any older generation; executable v4 retains its input contract and dependency snapshot, source-reference v4 retains tenant scope, workflow-execution v4 retains dispatch nesting depth, trigger/schedule current shapes are sole v2 fixtures, and Publishing v1 fixtures move with implemented T042 store shapes.
...
- [X] T034 [US2] Extend source references, trigger bindings, and recurring schedules with publication/slot identity in `src/Elsa/Workflows/Runtime/Core/Models/`; establish the complete source-reference shape, now including tenant scope, as the sole readable Groundwork v4 generation and the complete trigger/schedule shapes as their sole readable v2 generations, requiring a complete dependent Runtime/Publishing reset for older data with no Elsa upcasters or historical fixtures in `src/Elsa/Persistence/Groundwork/`
