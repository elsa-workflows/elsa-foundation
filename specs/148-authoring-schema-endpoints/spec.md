...
- **FR-003**: The schema MUST mark a member as required exactly when the submit operation cannot accept its absence: members whose absence the server tolerates (treated as null/unset) MUST be optional, and members whose absence the operation always rejects — such as the root activity — MUST be required, even where a payload type shared with more permissive operations declares them optional. *(Hardened twice after review/live verification: the initial version over-claimed nullable members as required; the pruned version under-claimed the submit-mandatory root activity.)*
