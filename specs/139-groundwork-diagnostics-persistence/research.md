...
Groundwork issue #71 is implemented on the consumed upstream line. Catalog identities and declared
diagnostic string fields therefore use its provider-neutral Unicode ordinal-ignore-case policy.
Queries that would require a catalog-to-record join still fail immediately with the stable
capability exception; the released identity policy is not misrepresented as a bounded join.
