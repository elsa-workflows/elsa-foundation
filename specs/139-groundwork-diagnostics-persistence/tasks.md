...
- [X] T027 [US2] Add real-provider execution-plan/index evidence assertions for every plan-exposed scale-bearing query and trim selection in `tests/Elsa/Diagnostics/Persistence/Tests/DiagnosticsBoundedExecutionTests.cs`
  ...
  - Preview.102 follow-up: all four providers passed the cursor-native catalog plan slice with an exact provider `Page` command, `Take + 1` continuation lookahead, and provider-applied order; the 1,000-row capacity probes use the production cursor first-page shape rather than the obsolete offset skip.
