...
- [X] T020 [US1] Implement idempotent normalized batch writes and durable restart reads in `src/Elsa/Diagnostics/OpenTelemetry/Persistence/Groundwork/GroundworkOpenTelemetryStore.cs`
  ...
  - Preview.102 follow-up: a provider-reported catalog `ConcurrencyConflict` is accepted only when a read-back proves byte-exact schema/content equivalence; divergent winners remain failures and are never overwritten. The two deterministic fault-injection regressions passed 2/2 and prove the reconciliation branch. The real four-provider concurrent-writer case passed 4/4 on exact implementation head `6e1b4e1a`, proving public concurrency behavior without claiming that every provider entered the branch. `evidence/preview102-provider-remediation-results.json` binds the source/configuration hashes, retained package/tool outputs, provider topology, exact commands, totals, skips, and sanitized per-test result artifacts.
...
- [X] T027 [US2] Add real-provider execution-plan/index evidence assertions for every plan-exposed scale-bearing query and trim selection in `tests/Elsa/Diagnostics/Persistence/Tests/DiagnosticsBoundedExecutionTests.cs`
  ...
  - Preview.102 follow-up: all four providers passed the cursor-native catalog plan slice with an exact provider `Page` command, `Take + 1` continuation lookahead, and a field/direction/identity-tie-break sequence equal to the admitted plan order; the 1,000-row capacity probes use the production cursor first-page shape rather than the obsolete offset skip. The exact-source receipt is `evidence/preview102-provider-remediation-results.json`.
