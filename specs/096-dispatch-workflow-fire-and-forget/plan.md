...
- **Contribution semantics**: PASS — executable enrichment uses generic `IExecutableNodeMetadataSource` registrations collected through `OnExecutableNodeMetadataCollecting` by Publishing's single `CollectExecutableNodeMetadata` handler; runtime delivery uses #675’s conflict-safe keyed handler mechanism.
