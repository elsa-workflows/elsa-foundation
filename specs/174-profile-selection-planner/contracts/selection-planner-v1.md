...
The public typed planner validates those pins and locks as well as the JSON import path. Digests are 64 lowercase hexadecimal characters. Package identity, version, evidence-source, and redacted resource-reference labels use ASCII letters/digits plus `.`, `_`, `-`, `/`, or `+`; connection-like strings containing `=` or `;` are invalid labels and cannot be echoed as package locks in a plan.
