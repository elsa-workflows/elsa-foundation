...
**Decision**: Each Groundwork implementation feature exposes one manifest source. A named startup contribution event in the Groundwork composition feature uses one aggregating handler over registered sources, producing an immutable, deterministically ordered composition snapshot. The selected provider validates and materializes that single union before stores resolve. `GroundworkUnifiedManifest` becomes a compatibility façade over the snapshot and loses its hard-coded domain references.

The composition root registers only sources belonging to selected features. Groundwork's `StorageManifestComposition.Union` remains the collision authority. The runtime and deployment CLI consume the same concrete `IPhysicalSchemaManifestSource` and target fingerprint.
