...
- Access to the package feed containing the pinned Groundwork `0.0.1-preview.90` release.
...
- `Groundwork.Tool` restored from the repository-local tool manifest at `0.0.1-preview.90`, matching all
  Groundwork packages.
...
The repository's current Groundwork family is `0.0.1-preview.90`; do not combine it with a different
...
versioned attachment by tuple as prior-generation provenance. The `0.0.1-preview.86` and
`0.0.1-preview.88` checkpoint/fence slices live under their versioned directories as immutable
prior-generation provenance. The current `0.0.1-preview.90` slice lives under
`versions/0.0.1-preview.90/` and is imported mechanically by tuple. All rows remain below
`evidence-complete`: this narrow 36-record slice cannot close the full provider-evidence gate.

**2026-07-25 preview.90 source alignment**: the seven Groundwork packages and `Groundwork.Tool` consume the
public `0.0.1-preview.90` release built from Groundwork merge
`d16e0f6c4a5c0c875d743820e63a7fab5b53e463`. Publication ran from exact Elsa source
`a95362f33ca272e353088e37263e8a310bc0fd9a` (tree
`e2148bc3e1665d1eac9489f00405efdc8fac8ed6`) and passed the four-provider publisher gate
(1/1, 3m44s). The importer retained 36 records with attachment SHA-256
`90e34d5a80a839d07f0e241ba594841e52e4546af562d2dd2f72f09e6c74abaf`.
The pre-import audit confirmed 9 records per provider, 9 equivalent four-provider correctness tuples,
exact source provenance, and no cross-provider result-hash mismatch. Root verification passed the exact-source
solution build (0 errors), the architecture/importer/performance-handoff guard slice (93/93), the 25-scenario
SQLite design baseline, and the OpenIddict public-capability gate (17/17 across all four providers).
The import advances no row status or performance verdict.
...
export ELSA_GROUNDWORK_RUN_IDENTITY="runtime-checkpoint-fence-preview90"
...
dotnet run --project tools/groundwork/Elsa.Groundwork.ProviderEvidenceImporter/Elsa.Groundwork.ProviderEvidenceImporter.csproj -- \
  ...
  --provider-version "0.0.1-preview.90" \
...
the opt-in SQL Server/PostgreSQL/MongoDB matrix against Groundwork `0.0.1-preview.90` and the current Identity
