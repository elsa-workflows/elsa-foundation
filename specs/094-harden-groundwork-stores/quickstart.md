...
- Access to the package feed containing the pinned Groundwork `0.0.1-preview.67` release.
...
- `Groundwork.Tool` restored from the repository-local tool manifest at `0.0.1-preview.67`, matching all
  Groundwork packages.

Groundwork PR #88 is the generic version-aware codec boundary in this release, and PR #101 admits sort-only
index fields as bounded residual predicates. Elsa-specific payload policies,
...
the opt-in SQL Server/PostgreSQL/MongoDB matrix against Groundwork `0.0.1-preview.67` and the current Identity
