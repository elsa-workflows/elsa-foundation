...
provenance. Current execution uses the repository-pinned `preview.102` family and current Identity manifest; no
...
is now immutable prior-generation provenance. Preview.102 has no imported generation until its exact clean-source
...
first consumed together by `preview.88` and retained by the current `preview.102` family. Elsa-specific per-kind version policies, legacy-stamp parsing, JSON
