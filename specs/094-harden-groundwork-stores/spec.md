...
1. **Given** the required runtime, IAM, secrets, distributed, and durable diagnostics workloads, **When** the current ledger reaches its evidence gate, **Then** every workload and verified outcome baseline has been supplied to #646.
...
- **FR-030**: This feature MUST supply #646 with verified workloads for checkpoint commit, bookmark lookup, trigger-binding stimulus lookup, recovery scan, queue drain, outbox drain, due timer selection, recurring-schedule selection, IAM normalized lookup and update, secret create and read, bounded secret-list query, placement takeover, command send/lease/acknowledgement, and durable Structured Logs/OpenTelemetry history; each workload MUST include a correctness baseline before timing is considered. Every coverage entry classified as a hot path MUST map to one of these workloads or to an explicitly accepted representative workload recorded in the ledger before measurements begin.
