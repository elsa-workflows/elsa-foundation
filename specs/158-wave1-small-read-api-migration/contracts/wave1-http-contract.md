...
| Elsa.Workflows.Runtime.JavaScript | POST | `/javascript/execute` | `ElsaWorkflowsRuntimeJavaScriptActivitiesRunJavaScriptEndpoint` | `workflows.runtime.javascript.execute` or wildcard | JSON `RequestModel` body; JSON success 200, ProblemDetails 400 for malformed/missing JSON, handler JSON 400 for blank script, JSON failure 500 |
...
The immutable FastEndpoints-before observations are committed in
`tests/Elsa/Architecture/Baselines/wave1-http-fastendpoints.json` and
`wave1-openapi-fastendpoints.json`. The real-host after capture compares all consumed HTTP facets
and canonical OpenAPI request/response/schema projections through the shared compatibility gate.
...
These two exact operation projections are recorded in the canonical
`rest-compatibility-approved-differences.json` registry and remain review-gated. They are not
treated as unapproved parity until accepted by the issue reviewer; all unrelated OpenAPI deltas
fail the Wave 1 comparison.
