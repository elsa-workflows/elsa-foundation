# Contract: `WorkflowVersionsReconciled` event
...
| Aspect | Value |
|---|---|
| Class | `Elsa.Workflows.Design.Reconciliation.Core.WorkflowVersionsReconciled : IEvent` (`sealed`) |
...
- `WorkflowVersionsReconciling.Claims` — new get-only `ICollection<WorkflowVersionSourceClaim>` populated by the aggregating handler beside `Versions`; consumed by the reconciler to assemble `WorkflowVersionsReconciled`.
