...
3. Idempotency pre-check (FR-007): `IPublicationSlotStore.ListByDefinitionAsync` → active `PublicationRecord`; when `record.Status == Active && record.WorkflowDefinitionVersionId == targetVersion.Id` ⇒ skip (log debug). `PublishWorkflow`'s internal `WasCreated=false` short-circuit remains the second net for races.
