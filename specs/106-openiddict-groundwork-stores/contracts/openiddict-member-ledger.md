...
## Reproducible source and current pin

The denominator is generated from the XML documentation restored with the
currently resolved `OpenIddict.Abstractions` **7.5.0** package. The package ships
its contract XML under `lib/netstandard2.0`, not `lib/net10.0`.
...
# Prove that the current preview.81 branch actually resolves that package.
rg -n 'OpenIddict.Abstractions/7.5.0' \
  src/Elsa/Foundation/Identity/OpenIddict/{Behavior,Groundwork}/obj/project.assets.json
```

Expected and observed on the T001 head: `Application=42`,
`Authorization=32`, `Scope=28`, `Token=43`, **total=145**. Central package
management pins `OpenIddict.Abstractions` at 7.5.0; both the EF-free Behavior
project and the new Groundwork project resolve that exact package. Groundwork is
at preview.81, but it does not change the OpenIddict interface denominator.
...
suite. The owner assignments above are obligations, not claims that those tests
exist or pass today. T042 later replaces each assignment with the exact passing
test identity and evidence artifact.
...
| `SetRequirementsAsync` | Set requirements collection. | L | T033/T037/T051 |
