...
This is the pre-migration retention baseline. An objective may move to a shared or
Groundwork fixture, but it may not disappear without recorded approval. The EF
fixture remains the behavior and #646 performance oracle until the exit gate.
...
That command returns 51 direct HTTP/token/shell objectives. Three additional
OpenIddict registration/provider objectives are retained from
`IdentityProviderModuleTests.cs`, for a total retention baseline of **54**.
...
## Shared token-endpoint host objectives — 9
...
The first objective must be renamed when the EF oracle is retired; its substantive
assertion becomes “Groundwork owns both framework-identity and OpenIddict stores.”
...
All 54 objectives remain present at this baseline. None is approved for deletion.
T061 must record the final destination and passing evidence for every row before EF
