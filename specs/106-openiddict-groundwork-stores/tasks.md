...
- [ ] T005 Add executable public-API probes for codec admission, physical entity definitions, naming/fingerprints, schema CLI/readiness, typed compound/range/multivalue routes, bounded mutations with native plans, CAS, and UoW in `tests/Elsa/Persistence/Groundwork/Conformance/Tests/OpenIddictGroundworkCapabilityProbeTests.cs` — Historical evidence: preview.88 and preview.90 passed an earlier 17-test probe. The 2026-07-29 review withdrew that completion claim because the mutation cases bypassed manifest admission and the canonical JSON/physical paths had drifted. The retained current probe covers only the non-mutation subset until an admitted fixed-assignment route and native-plan case are added.
- [ ] T006 Run T005 against SQLite, SQL Server, PostgreSQL, and transaction-capable MongoDB and record package hashes, tool output, provider topology, and capability verdicts in `specs/106-openiddict-groundwork-stores/quickstart.md` — Historical evidence: preview.88 and preview.90 each passed the earlier probe on all four providers. The exact currently configured family must pass the corrected T005 suite before this task can close; a future package import restarts this evidence gate.
- [X] T007 Create or link a Groundwork issue for every failed T005 capability and keep all dependent tasks blocked rather than adding provider-specific or client-side fallbacks in `specs/106-openiddict-groundwork-stores/research.md` — Evidence: implementation-oracle re-verification exposed two capabilities that the earlier synthetic probe did not cover; Groundwork #141 owns fenced cross-unit relationship guards and Groundwork #143 delivered fixed-value bounded assignment with matched-count semantics in Elsa's configured preview.95 family. Current-family recertification remains T006, and dependent application/authorization/token operations remain blocked instead of using client orchestration.
...
- [ ] T010 [P] Write red canonical JSON, descriptor round-trip, current/minimum-readable version, upcast, and corrupt/future payload tests for all four record kinds in `tests/Elsa/Foundation/Identity/OpenIddict/Groundwork/Tests/OpenIddictGroundworkCodecTests.cs` — Candidate source evidence: the 2026-07-29 post-review replay passed the expanded adapter project, and the codec cases now cover every descriptor field and all four canonical kinds across current/minimum-readable/upcast policy, camel-case output, and corrupt, future, wrong-kind, and identity-mismatch rejection. Completion is not claimed because the expanded matrix landed after its serializers and has no durable red-before-green record.
...
- [ ] T035 [P] [US2] Write red 28-member scope-store contract tests including name uniqueness, name-set/resource membership, generic delegates, CAS, pages, and descriptors in `tests/Elsa/Foundation/Identity/OpenIddict/Groundwork/Tests/GroundworkOpenIddictScopeStoreTests.cs` — Candidate source evidence: the 2026-07-25 direct Scope/manifest selection passed 23/23 on durable SQLite, but the test and implementation landed together without durable red-before-green evidence and T006 remains open. Completion is therefore not claimed.
...
- [ ] T040 [P] [US2] Implement all 28 OpenIddict scope-store members, descriptors, unique name, name-set/resource membership, CAS, pages, and generic-delegate rejection in `src/Elsa/Foundation/Identity/OpenIddict/Groundwork/Stores/GroundworkOpenIddictScopeStore.cs` — Candidate source evidence: the unregistered implementation uses bounded routes, fixed 256-row provider pages, a 900-name ceiling, expected-version CAS, and fail-closed generic delegates. It remains unchecked until T006 and the direct red-before-green evidence gate are satisfied.
