...
- [ ] T005 Add executable public-API probes for codec admission, physical entity definitions, naming/fingerprints, schema CLI/readiness, typed compound/range/multivalue routes, bounded mutations with native plans, CAS, and UoW in `tests/Elsa/Persistence/Groundwork/Conformance/Tests/OpenIddictGroundworkCapabilityProbeTests.cs` — Partial evidence: the current exact package family passed 6/6, including package/tool consistency, codec admission, four global physical-entity declarations, real SQLite manifest apply/reopen, and a provider-native bounded token-reference route. T005 remains open because naming/fingerprint transformation, multivalue routes, bounded mutation/count/cancellation/native plans, CAS, UoW, and CLI readiness are not yet executed.
