...
- [X] T003 [P] Add architecture tests that keep Identity abstractions and the retained OpenIddict behavior project free of Groundwork and new EF dependencies in `tests/Elsa/Architecture/OpenIddictPersistenceArchitectureTests.cs`
  - Evidence: after CI exposed a transitive EF edge, the focused guard passes against the extracted EF-free `OpenIddict/Behavior` assembly, requires Groundwork to reference Behavior rather than the oracle, and freezes the existing project's exact four-package EF oracle.
- [X] T004 Verify, without changing the central pins in this work unit, that the complete Groundwork package and tool family is the exact publicly restorable `0.0.1-preview.81` release in `Directory.Packages.props` and `.config/dotnet-tools.json`
  - Evidence: at integration head `86e3bbc9ef99918ab969a16aafd2b2d06711acab`, after merging `origin/main` `6458b39ad4a251c992a7384f1f325e164320d481`, central package management pins Core, DiagnosticRecords, Documents, MongoDb, PostgreSql, Sqlite, and SqlServer to `0.0.1-preview.81`; the manifest pins `groundwork.tool` to the same version. Empty-cache restores of the provider test-support and DiagnosticRecords consumer projects, plus manifest-tool restore and `groundwork --version`, succeeded against the configured Groundwork Preview feed. The 9 capability probes and 4 architecture guards passed again on that integration head. See `quickstart.md` for the exact commands and scope.
- [X] T005 Add executable public-API probes for codec admission, physical definitions, naming/fingerprints, schema CLI/readiness, typed compound/range routes, collection-element membership, bounded mutations with native plans, CAS, and UoW in `tests/Elsa/Persistence/Groundwork/Conformance/Tests/OpenIddictGroundworkCapabilityProbeTests.cs`
  - Evidence: the focused preview.81 probe passes 9/9 for the surfaces it actually inspects, but adversarial review proved it exercises only scalar projections and method/signature presence. It does not prove collection-element projection or contains-all membership. T007 records that failed capability honestly.
...
- [X] T007 Create or link a Groundwork issue for every failed T005 capability and keep all dependent tasks blocked rather than adding provider-specific or client-side fallbacks in `specs/106-openiddict-groundwork-stores/research.md`
  - Evidence: valence-works/Groundwork#128 records the missing bounded collection-element projection, automatic maintenance, exact membership/contains-all query, and four-provider evidence capability. The declaration-only prototype `e4e88c0` is explicitly non-runtime evidence.
- [X] T007A Reclassify the apparent physical-form choice when review proves that no public preview.81 form satisfies the required collection-membership routes
  - Evidence: `research.md`, `data-model.md`, and the independent review record reject both scalar linked sidecars and adapter-owned membership intersection. Production scaffolding remains blocked on Groundwork#128 before #646 can evaluate executable forms.
...
- [X] T008 Scaffold the concrete provider package and test project in `src/Elsa/Foundation/Identity/OpenIddict/Groundwork/Elsa.Foundation.Identity.OpenIddict.Groundwork.csproj` and `tests/Elsa/Foundation/Identity/OpenIddict/Groundwork/Tests/Elsa.Foundation.Identity.OpenIddict.Groundwork.Tests.csproj`
  - Evidence: both projects restore and compile; their resolved assets contain no EF package because the provider project references only the extracted OpenIddict Behavior and Groundwork provider-boundary projects.
- [X] T008A Exclude nested `Groundwork/**/*.cs`, resources, and project-local build artifacts from `src/Elsa/Foundation/Identity/OpenIddict/Elsa.Foundation.Identity.OpenIddict.csproj` before adding source below that directory
  - Evidence: the transitional EF oracle removes nested `Behavior/**/*.cs` and `Groundwork/**/*.cs` source/resources/build artifacts, while each nested project owns its assembly.
- [X] T009 Add both projects to `Elsa.Server.slnx` and add only provider-boundary project references in `src/Elsa/Foundation/Identity/OpenIddict/Groundwork/Elsa.Foundation.Identity.OpenIddict.Groundwork.csproj`
  - Evidence: Behavior, Groundwork, and its test project occupy canonical collapsed OpenIddict solution folders; Groundwork references Behavior plus the Groundwork boundary/composition projects, with no EF oracle or concrete provider leaf.
