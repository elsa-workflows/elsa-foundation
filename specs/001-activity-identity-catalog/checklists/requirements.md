...
- **Leaf models are sealed records (pinned at clarify).** `InputDefinition`, `OutputDefinition`, `ActivityDesignFacet`, `ArgumentDefinition` → `public sealed record`s. No separate read-only sibling interface required.
