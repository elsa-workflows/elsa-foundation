...
- **SC-004** *(amended 2026-07-22 by program-owner decision; decision record in the design persistence contract §Performance and Removal Gate)*: For every operation in the contract's Benchmark Acceptance Catalog at the required 100K catalog scale, the median of three independent measured process runs satisfies the ratified absolute operational budgets per row class (point reads p95 ≤ 0.8 ms / p99 ≤ 2.5 ms / ≥ 2,000 ops/s; batch reads p95 ≤ 5 ms / p99 ≤ 20 ms / ≥ 200 ops/s; catalog pages p95 ≤ 400 ms / p99 ≤ 800 ms / ≥ 4 ops/s; writes at c1 p95 ≤ 3 ms / p99 ≤ 25 ms / ≥ 400 ops/s; writes at c16 p95 ≤ 100 ms / p99 ≤ 500 ms with throughput at least the same row's c1 throughput); each run follows one untimed warm-up and contains at least 100 completed operations and 30 seconds of steady-state measurement, and same-provider EF measurements are recorded as evidence rather than gated. The superseded criterion (p95 ≤ 1.25x, throughput ≥ 80%, p99 ≤ 2x the same-provider EF oracle) compared semantically unequal work — the ratified Groundwork write path performs ledger, replay, scope, and atomic-staging work the temporary oracle does not.
