...
| `BoundedDocumentQueryPager` | Bounded query pagination | T020 | Current design-store adapter tests | `InMemoryDocumentStoreBoundedQueryTests` plus the named-store and scale-suite exhaustion scenarios (T039 page-crossing listing) cover offset page bounds, stable totals, deterministic exhaustion, and cancellation after the transitional reader's deletion (T047) | Covered |
