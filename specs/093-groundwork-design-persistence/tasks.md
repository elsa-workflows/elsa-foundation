...
- [ ] T015 Implement one serialized-path-aware `Query<TEntity>` to Groundwork `DocumentQuery` translator in `src/Elsa/Persistence/Groundwork/Querying/GroundworkQueryTranslator.cs`
- [ ] T016 Introduce one immutable design-store session bundle for scoped point reads, bounded queries, and UoW acquisition in `src/Elsa/Persistence/Groundwork/Querying/GroundworkDesignStoreSession.cs`
