...
- [ ] T062 [P] [US3] Complete every provider feature/registration/materialization row in the T020 coverage ledger with direct branch tests, then add one-provider-per-host conflict tests that compose the actual `src/Apps/Elsa.Server/` reference host in design-only, runtime-only, and combined deployment shapes within the owning provider-specific projects under `tests/Elsa/Persistence/Groundwork/DesignConformance/*.Tests/`; keep provider SDK references out of the shared `DesignConformance/Tests` assembly
