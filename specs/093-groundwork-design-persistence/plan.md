...
**Performance Goals**: At the required 100K acceptance scale, each operation in the contract's Benchmark Acceptance Catalog uses the median of three independent measured processes (one untimed warm-up, at least 100 operations and 30 seconds steady state per process) and must meet the absolute operational budget for its class (point-read: p95 ≤ 0.8 ms / p99 ≤ 2.5 ms / ≥ 2,000 ops/s; batch/projection: ≤ 5 ms / ≤ 20 ms / ≥ 200 ops/s; catalog page/count: ≤ 400 ms / ≤ 800 ms / ≥ 4 ops/s; writes @c1: ≤ 3 ms / ≤ 25 ms / ≥ 400 ops/s; writes @c16: ≤ 100 ms / ≤ 500 ms / throughput not below the same row's @c1). Same-provider EF measurements are recorded as evidence, not a gate. Every selected physical-entity form must improve median p95 or throughput by at least 10% over shared/linked and dedicated-document forms at both 100K and 1M, in the same direction in all three runs, with a 95% bootstrap confidence interval excluding zero.

> **Amendment 2026-07-22 (ratified by program-owner interactive decision; validated by the T079 three-axis independent review of 2026-07-22: performance-gate legitimacy PASS, test-objective preservation PASS, deletion completeness and core independence PASS, zero blockers).** The Performance Goals gate-5 criterion above was, until this date: *"must achieve p95 no worse than 1.25x the same-provider EF oracle, throughput at least 80% of the oracle, and p99 no worse than 2x."* That per-row EF ratio is **superseded** (history preserved here, not deleted) by the absolute-budget wording now inline above. Rationale: the EF ratio compared semantically unequal work — the Groundwork write path runs the ratified operation-ledger marker, replay preflight, scope-bound sessions, and atomic multi-document staging per operation, while the temporary EF oracle performs bare `SaveChanges` (its `LegacyEfOracle` conformance profile declares the ledger/replay/scope scenarios N/A). EF numbers remain recorded as evidence. The form-selection goal (gate 6) is unchanged. See the design persistence contract "Performance and Removal Gate" item 5 and `docs/reports/groundwork-design-persistence-performance.md`; the fair 2026-07-22 re-measurement passes 19/19.
