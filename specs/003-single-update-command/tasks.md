...
- [X] T006 [P] [US1] Migrated **activity** diff tests in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/ActivityDiffTests.cs` — driving `IUpdateDraftCommand`: add → `OnActivityAddedToDraft`; remove → `OnActivityRemovedFromDraft`; assert resulting State.
...
- [X] T011 [P] [US1] **Layout** diff test in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/LayoutDiffTests.cs` — changed `DesignMetadataRecord` (X/Y/W/H) for a `NodeId` → `OnActivityMovedInDraft` (FR-001a); confirms layout is diffed from the sibling, not from `WorkflowDefinitionState`.
...
- [X] T015 [P] [US1] **Rename vs id-change** test (SC-015, FR-013d) in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/IdentityMatchTests.cs` — same id + changed payload → single UPDATE (e.g. `OnVariableUpdatedInDraft`), zero REMOVE/ADD; differing id → REMOVE+ADD pair.
...
- [X] T017 [P] [US1] **Activity-removal cascade** test (FR-013f) in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/ActivityRemovalCascadeTests.cs` — removing an activity (NodeId absent from desired) prunes its connections, emitting `OnActivityRemovedFromDraft` + the connection removals.
...
- [X] T026 [P] [US3] Shell-ordering test (SC-006) in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/ShellOrderingTests.cs` — N events: one lock acquisition, `OnDraftValidating` published exactly once (Sequential) against post-diff state, validation sibling rewritten wholesale, State + sibling flushed in one transaction inside the lock, N mutation events + one `OnDraftValidated` published Background after release in cause-before-effect order.
