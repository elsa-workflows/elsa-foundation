...
- [X] T007 [P] [US1] Migrated **activity input/output** diff tests in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/ActivityIoDiffTests.cs` — add/update/remove by (`NodeId`,`ReferenceKey`) → `ActivityInput{Added,Updated,Removed}…` and `ActivityOutput{Added,Updated,Removed}…`.
- [X] T008 [P] [US1] Migrated **connection** diff tests in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/ConnectionDiffTests.cs` — add → `ConnectionAddedToDraft`; remove → `ConnectionRemovedFromDraft` (endpoint-tuple identity).
- [X] T009 [P] [US1] Migrated **variable** diff tests in `tests/Elsa.Workflows.Design.Tests/Unit/UpdateDraftCommand/VariableDiffTests.cs` — declare/update/remove by `ReferenceKey` → `Variable{Declared,Updated,Removed}…`.
