...
**Tests**: Required by spec 176's independent tests and success criteria. Write focused behavior tests before implementing each story. These tasks describe future implementation; this document does not claim that the commands exist.

## Phase 1: Setup and package-semantics prerequisite

- [ ] T001 Verify object-map, array, explicit-disabled, null/empty, and same-shape overlay behavior against the repository-pinned CShells `0.0.30-preview.159` package in `tests/essentials/Architecture/WorkbenchConfigurationTests.cs`; record any unsupported behavior or necessary contract narrowing in `specs/176-composition-file-bridge/research.md` before parser work.
- [ ] T002 Add the contract's disposable two-shell JSON source, Production/Staging overlays, local `primary` resource and canaries under `tests/essentials/Cli/Tests/Fixtures/CompositionBridge/`; keep actual Workbench files as a separate compatibility sample.
- [ ] T003 Validate the optional review-file shape and sample safe `/Flag` and `/Limit` declarations in `tests/essentials/Modularity/Planning/Tests/CompositionBridgeSettingReviewTests.cs` against `specs/176-composition-file-bridge/contracts/setting-review-v1.md`; revise the Draft contract before implementation if a safe scalar cannot be distinguished from unclassified source data.
...
- [ ] T004 Add invocation-local `SourceSelection`, `SourceSnapshot`, and selected-file identities in `src/essentials/Modularity/Planning/Bridge/SourceSnapshot.cs`; keep physical paths, bytes, and digests out of `AuthoredComposition` in `src/essentials/Modularity/Planning/Models/SelectionDocuments.cs`.
- [ ] T005 Implement a raw CShells JSON layer reader with exact feature IDs, disabled state, value types, and base/overlay provenance in `src/essentials/Modularity/Planning/Bridge/CshellsSourceReader.cs`; reject malformed/duplicate/case-equivalent IDs and unsupported cross-shape layers.
- [ ] T006 Implement strict, deny-by-default setting review parsing and logical-resource reference validation in `src/essentials/Modularity/Planning/Bridge/SettingReviewReader.cs`, including the stable unsafe/unresolved refusal classes from `specs/176-composition-file-bridge/contracts/file-bridge-v1.md`.
- [ ] T007 Implement supported-file discovery, symlink/path-escape refusal, all-copied-file content freezing, and recheck in `src/essentials/Cli/CompositionFileSource.cs`; read selected files for effective preview while retaining unselected sibling overlays for output preservation.
- [ ] T008 [P] Add parser/snapshot tests for object-map and array layers, false/zero/null/empty values, duplicates, changed selected and unselected files, and path escapes in `tests/essentials/Modularity/Planning/Tests/CompositionBridgeSourceTests.cs` and `tests/essentials/Cli/Tests/CompositionFileSourceTests.cs`.
...
- [ ] T009 [P] [US1] Add pure import-preview and authored-projection tests, including planner candidate/accepted equality and no invented profile/locks, in `tests/essentials/Modularity/Planning/Tests/CompositionImportTests.cs`.
- [ ] T010 [P] [US1] Add real-process import acceptance, non-TTY refusal, cancellation, catalog mismatch, and canary scans over stdout/stderr/authored/plan output in `tests/essentials/Cli/Tests/CompositionImportCliTests.cs`.

### Implementation for User Story 1

- [ ] T011 [US1] Build a redacted `ImportPreview` and accepted no-profile `AuthoredComposition` from the selected source, reviewed fields, and `SelectionPlanner.Plan` in `src/essentials/Modularity/Planning/Bridge/CompositionImporter.cs`; keep unknown values local and unresolved findings visible.
- [ ] T012 [US1] Register `composition import` without the persistence worker in `src/essentials/Cli/ElsaCli.cs` and implement its arguments, safe rendering, interactive one-preview decision, and stable refusal mapping in `src/essentials/Cli/CompositionImportCommand.cs`.
- [ ] T013 [US1] Write the authored v1 document only after acceptance and a fresh all-file snapshot check, using a private adjacent file and no-overwrite move in `src/essentials/Cli/CompositionFilePublisher.cs`; cancellation and I/O refusal leave no published file.
- [ ] T014 [US1] Run the independent import acceptance and redaction scenario in `specs/176-composition-file-bridge/quickstart.md` against the two-shell fixture and a disposable copy of `src/apps/Elsa.Workbench/` JSON; record selected-file versus running-host evidence limits.
