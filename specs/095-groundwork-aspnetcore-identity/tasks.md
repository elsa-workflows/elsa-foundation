...
- [X] T091 [P] [US5] Write failing offset-rejection, cursor lookup-tail, shared-index compatibility, manifest-version, and SQL Server 1,700-byte key-budget tests in `tests/Elsa/Foundation/Identity/Persistence/Groundwork/Tests/IdentityStorageManifestTests.cs` — Evidence: the 1.0.7 manifest suite asserts cursor paging and `id_lookup_key` tails on all twelve routes, preserves shared logical indexes, proves the 1,088-byte declared SQL Server key budget, and passed 27/27 in the root container-free run.
- [X] T092 [P] [US5] Write failing exact-page, page-plus-one, multi-page, cancellation, repeated-token, cross-route-token, and non-advancing-token tests for the reviewed finite traversal mechanism in `tests/Elsa/Foundation/Identity/AspNetCoreIdentity/Groundwork/Tests/BoundedIdentityContinuationTests.cs` — Evidence: 17/17 root-run cases cover exact/page-plus-one/eight-route/multi-page traversal, underfilled pages, cancellation, repeated/empty/cross-route continuations, duplicate identities, and non-progress failure.
- [X] T093 [P] [US5] Write failing complete-result/no-duplicate/no-omission tests for every affected framework and Elsa reader in `tests/Elsa/Foundation/Identity/AspNetCoreIdentity/Groundwork/Tests/AspNetCoreIdentityRelationshipContractTests.cs` and `tests/Elsa/Foundation/Identity/Persistence/Groundwork/Tests/IdentityGroundworkStoreTests.cs` — Evidence: page-plus-one public-reader regressions seed 513 records and prove exact/no-duplicate/no-omission results through all six framework relationship readers and all three Elsa list readers; both focused tests passed container-free.
...
- [X] T095 [US5] Convert the frozen twelve scale-bearing route/index declarations to cursor paging with the provider identity lookup-key tail and advance the Identity manifest/schema identity in `src/Elsa/Foundation/Identity/Persistence/Groundwork/IdentityStorageManifest.cs` — Evidence: manifest 1.0.7 declares ascending cursor paging and the provider `id_lookup_key` tail for the exact twelve-route inventory.
- [X] T096 [US5] Implement one guarded finite continuation traversal with page-count, token-progress, route/scope/order binding, and cancellation preservation in the owning Identity/Groundwork paging helper under `src/Elsa/Foundation/Identity/Persistence/Groundwork/Stores/` — Evidence: `IdentityBoundedDocumentQueryPager` follows only opaque provider continuations under the same route/clauses/order, bounds pages by distinct progress and total materialization, rejects repeated/empty/non-progress/duplicate results, and preserves cancellation.
- [X] T097 [US5] Migrate every exhaustive ASP.NET Core Identity claim/login/role reader to T096 without changing public ordering or materializing an unbounded provider result in `src/Elsa/Foundation/Identity/AspNetCoreIdentity/Groundwork/Stores/` — Evidence: both framework stores route every claims/logins/roles traversal through the shared pager while retaining their existing public mapping/order logic.
- [X] T098 [US5] Migrate affected Elsa IAM readers and all shared-host/test doubles to T096-compatible continuation behavior in `src/Elsa/Foundation/Identity/Persistence/Groundwork/Stores/` and their owning test fixtures — Evidence: Elsa role/claim-mapping/external-identity readers use the pager; both shared-host fakes advance `TestKeysetContinuations`; the conformance observation now requires a returned continuation rather than offset 512.
- [X] T099 [US5] Preserve and directly test normalized user-name/email/role first-page lookups, deterministic-ID loads, and ordered 64-record expired-receipt cleanup in the owning Identity store and manifest tests — Evidence: direct lookups remain one bounded request with null continuation; deterministic loads remain unchanged; cleanup retains one ascending take-64 request; the manifest and existing store/reconciliation contracts cover these shapes.
