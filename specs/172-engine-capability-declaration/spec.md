...
- `NuplaneRestoreResult.Refusals` (Nuplane `0.0.11-preview.92`) carries the stage and the message of every package the cycle failed, so the CLI classifies a failed package from the result alone. Until that release `FailedPackages` carried ids without stages and the CLI read the stage from the store state the same restore wrote, joined on that cycle's own failed ids; see "Known gap" below.
...
`NuplaneRestoreResult.FailedPackages` was a list of package ids with no stage, so a caller could not tell a capability refusal from an unreachable feed out of the restore result alone. E2 closed it in the CLI by reading `LastFailureById` from the store state that same restore wrote and joining on the cycle's own failed ids — correct, because the record carries the stage and the message, but one read more than a caller should need, and a coupling to the state-file shape.

Closed upstream on 2026-09-23 by [valence-works/nuplane#98](https://github.com/valence-works/nuplane/issues/98) ([PR 99](https://github.com/valence-works/nuplane/pull/99), published as `0.0.11-preview.92`): `NuplaneRestoreResult.Refusals` carries a `PackageRefusal` (`PackageId`, `Stage`, `Message`) for every package the cycle failed, built from the same failure records the cycle wrote, joined on the cycle's failed ids and its correlation id. Consumed here by [#1971](https://github.com/elsa-workflows/elsa-foundation/issues/1971): the pins moved to `0.0.11-preview.92` and `NuplaneRestoreRunner` reads `Refusals` instead of the state file. The `capability-` prefix match and the `id (stage): message` wording of the `restore-capability-unresolved` refusal are unchanged.
