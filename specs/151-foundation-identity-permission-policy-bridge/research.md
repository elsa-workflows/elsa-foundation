...
- **Rationale**: Identity already owns `IAuthorizationPolicyProvider`, evaluator, catalog, implication, normalization, and resource handlers. Implementation compilation proved the centrally pinned ASP.NET Core 2.3 packages do not expose .NET 10 endpoint-routing contracts such as `IEndpointConventionBuilder`. Replace those legacy direct ASP.NET package references with the standard `Microsoft.AspNetCore.App` framework reference so authorization, endpoint metadata, result handling, `IHttpContextAccessor`, and `AddHttpContextAccessor` come from one current runtime surface. This adds no FastEndpoints or Elsa project dependency.
...
- A marker alone is not trusted. Foundation Identity options contain an initially empty, ordinal set of normalized runtime authentication types. `AddNormalizedAuthenticationType` registers the exact post-authentication `ClaimsIdentity.AuthenticationType`, not merely an ASP.NET scheme registration name. The ASP.NET Core Identity package registers `"Elsa.Foundation.Identity"` for its normalized external principal factory and `AspNetCoreIdentityDefaults.CookieScheme` for reconstructed cookie requests. OpenIddict validation otherwise inherits the IdentityModel federation authentication type, so the package explicitly configures `TokenValidationParameters.AuthenticationType` and registers that same `OpenIddictValidationAspNetCoreDefaults.AuthenticationScheme` value for validated bearer requests; the token-creation identity type `"openiddict"` is not an endpoint principal and is not trusted. A custom provider explicitly registers its observed post-authentication type only after it guarantees the same strip-map-mark contract.
