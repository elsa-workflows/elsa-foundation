...
- [x] T004 Refactor `ActivityNode` to remove workflow-level start/terminal flags and carry optional activity-owned composition state.
- [x] T005 Update design projections, persistence commands, validators, and diffing for root activity traversal.
- [x] T006 Refactor `WorkflowExecutable` to expose `RootActivity` and derive `NodesById` from the root tree.
- [x] T007 Update publishing to compile the authored root activity recursively into the executable root activity.
- [x] T008 Update start scheduling to schedule only the executable root activity.
- [x] T009 Remove workflow-level executable edge traversal from completion scheduling.
