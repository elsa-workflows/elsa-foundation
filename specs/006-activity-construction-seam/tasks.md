...
- [x] T028 [US2] Implemented `WorkflowActivityReconciliationSource : IActivityReconciliationSource` at `src/Elsa/Activities/Composition/Design/Reconciliation/WorkflowActivityReconciliationSource.cs`. Discovery: full scan (`IWorkflowDefinitionStore.ListAsync` → `IWorkflowDefinitionVersionStore.ListByDefinitionAsync`) filtered on `WorkflowActivityOptions.UsableAsActivity`; one row per usable version (`ActivityTypeKey = definitionId`, `Version` = SemVer), `DescriptorType = typeof(WorkflowIdentity).FullName`, descriptor `WorkflowIdentity(defId, versionId, version)`, `Inputs`/`Outputs` mirrored directly off authored state. Behavioural tests in `WorkflowActivityReconciliationSourceTests`. (outcome/port visualization is future module-owned facet work, 005 FR-005/006)
- [x] T029 [US2] Wired `ActivitiesCompositionRuntimeFeature` (constructor + activity, pre-existing) and `ActivitiesCompositionDesignFeature` (registers `IActivityReconciliationSource`). Host wiring added: `Elsa.Server.slnx`, `Elsa.Server.csproj` project ref, `Program.cs` feature-assembly registration, and `shells.baseline.json` (`ActivitiesCompositionDesign`).
