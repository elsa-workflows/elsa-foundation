...
**Remaining open:** _none._ T004/T023/T025 landed on commit `70983a4a`. The remaining close-out (T018, T041, T042, T046, T050–T055) landed this pass: seam registration test; persistence round-trip + read-contract descriptor-surface tests; JSON reconciliation doc/sample fix; the four SC-proving architecture tests (T050–T053, in `tests/Elsa/Architecture/ArchitectureGuardTests.cs`); feature docs (G26) for Runtime/Primitives/Composition + a seam-walk note; and T055 validation (`dotnet build Elsa.Server.slnx` = 0 errors, affected tests green, quickstart walked). Per the T004 deviation, Primitives tests colocate in `Runtime.Tests` — no separate Primitives.Tests project. Two naming drifts from the tasks text confirmed against code: the reconciling handler is `CollectActivityVersions` (not `ActivityVersionsReconcilingHandler`), and the CLR descriptor is `ClrActivityDescriptor` (not `TypeInformation`, per spec 081).
...
- [x] T004 [P] Test projects exist: `tests/Elsa/Activities/Runtime/Tests`, `tests/Elsa/Activities/Composition/Tests`, `tests/Elsa/Activities/Design/Tests` (nested layout). **Deviation from spec:** no separate `Primitives.Tests` project — the Primitives constructor/binder/feature tests colocate in `Runtime.Tests` (which already references `Elsa.Activities.Primitives` and hosts `ClrActivityConstructorTests` + the `ClrConstruction` helper). Avoids an extra project for 3 test classes; matches the repo's evolved convention. xunit only, no FluentAssertions.
...
- [x] T018 [P] Registration test: `ActivitiesRuntimeFeature` registers and resolves every service (G27). Added `RegistersConstructionSeamServices` to `ActivitiesRuntimeFeatureTests` — asserts registry (singleton) + factory + `RegisterActivityConstructors` handler + `ActivityConstructorsStartupTask`, and resolves registry/factory (registry is `Assert.Same` across two resolves).
...
- [x] T023 [P] [US4] `ActivityArgumentBinderTests` (in `tests/Elsa/Activities/Runtime/Tests/`) covers match-and-set, case-insensitive match, missing-property throw, incompatible-type throw, no-public-setter throw, and the `InputArgument<int>`→`InputArgument<object>` widening rewrap (#313). 6 tests, green.
- [x] T024 [P] [US4] Unit test: `ClrActivityConstructor` round-trip — payload → loaded type → bound instance
- [x] T025 [P] Registration test: `ActivitiesPrimitivesFeatureTests` asserts the feature registers `IActivityConstructor` (ClrActivityConstructor) + `ActivityArgumentBinder` (G27). In `tests/Elsa/Activities/Runtime/Tests/`.
...
- [x] T041 [P] [US3] Unit test: save→load round-trips `(DescriptorType, JsonElement)` with no type resolution (`tests/Elsa/Activities/Design/Tests/Unit/DescriptorPersistenceRoundTripTests.cs`). Real SQLite round-trip via `ActivitiesDesignTestHost` + the saving/loading handlers; uses a deliberately non-resolvable `DescriptorType` FullName to prove the type is never loaded — payload survives as an equal `JsonElement`.
- [x] T042 [P] [US3] Read-contract surface test: `IActivityDefinitionVersion` exposes `DescriptorType` + `DescriptorPayload`, and NOT `ImplementationKind`/`IImplementationDescriptor`. Added two facts to `ReadContractSurfaceTests` (type-checked `DescriptorType`/`DescriptorPayload`; asserts no `ImplementationKind`/`ImplementationDescriptor` property and no property typed `IImplementationDescriptor`).
...
- [x] T046 [US3] Update `src/Elsa.Activities.Design.Reconciliation.Json/` — catalog field `implementationKind`→`descriptorType`; adjust `JsonActivityCatalogReader` doc + any sample catalog JSON. Code was already correct (model carries `DescriptorType`/`Descriptor`; `JsonActivityCatalogReaderTests` already uses `descriptorType`); updated the stale `README.md` (descriptor section, sample JSON → `Elsa.Primitives.Models.ClrActivityDescriptor` + `typeAlias`, handler name) + `EXTENSION_POINTS.md`.
...
- [x] T050 [US1] Reference/structural test: no project in the runtime construction path (`Elsa.Activities.Runtime`, `Elsa.Activities.Runtime.Core`, `Elsa.Activities.Primitives`, `Elsa.Activities.Composition.Runtime`) references any `Elsa.*.Design.*` project (SC-001). Added `Activity_construction_runtime_path_has_no_design_reference` to `tests/Elsa/Architecture/ArchitectureGuardTests.cs` (csproj `<ProjectReference>` scan — robust vs. compiler-elided assembly refs; complements the existing `Runtime_projects_do_not_add_design_references`, which did not cover `Composition.Runtime`/`Primitives`).
- [x] T051 [US6] Structural test: `ActivityFactory`, `ActivityConstructorRegistry`, and the reconciling handler contain no per-`DescriptorType`/per-kind branch (SC-004). Added `Construction_and_reconciliation_contain_no_per_descriptor_type_branch` — source-scan (reusing `StripCommentsAndStringLiterals`) asserting no `switch`/concrete-descriptor-type/kind token. **Drift:** the handler is `CollectActivityVersions`, not the tasks-file name `ActivityVersionsReconcilingHandler` (no such class exists).
- [x] T052 [P] [US1] Repo-wide sweep asserting zero production references to `IImplementationDescriptor`, `ClrImplementationDescriptor`, `WorkflowImplementationDescriptor`, the two registries, `IImplementationDescriptorSource`, and `IActivityImplementationResolver*` (SC-002). Added `No_production_code_references_deleted_implementation_descriptor_types` — scans `src/**/*.cs` with comments/strings stripped. Found + fixed one stale doc comment (`ActivityDefinition.cs`) en route.
- [x] T053 [P] Reference test: no feature project references another feature project — `Primitives`, `Composition.Runtime`, `Composition.Design`, `Reconciliation.Clr` (SC-006). Added `Activity_construction_feature_projects_do_not_reference_each_other` (csproj `<ProjectReference>` scan).
- [x] T054 [P] [US6] Feature documentation (G26): rewrote the stale `Runtime/README.md` (it still described the deleted resolver/descriptor registries) to the current seam + added an "Adding a new kind (seam walk)" note showing the 3-type move; added new `README.md` for `Primitives`, `Composition/Runtime`, `Composition/Design`. Also purged residual stale `ImplementationKind`/`ImplementationDescriptor`/`ActivityVersionsReconcilingHandler`/`ActivityDescriptorDeserialisationException` references from Persistence.EFCore + Reconciliation.Clr + Tasks/Events cross-domain catalogs.
- [x] T055 Ran `dotnet build Elsa.Server.slnx` → **0 errors** (4 pre-existing warnings). Affected test projects green: Architecture (23), Runtime feature/seam tests, Design (persistence round-trip + read-contract + json reader). Walked `quickstart.md` and corrected its CLR worked-example (was `TypeInformation`, now `ClrActivityDescriptor` per spec 081). SC-001…006 confirmed by T050/T053/T052/T051 + the existing round-trip/duplicate tests.
