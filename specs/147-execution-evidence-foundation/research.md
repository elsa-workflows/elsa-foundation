...
**Status:** The Path A recovery-authority decision and durable scheduler-continuation handoff are approved. T029e is complete as its bounded Runtime live-carrier resolution. The T029 construction-time logical-inspection-projection amendment passed independent and control-room plan review with no findings and is approved for task planning. Implementation remains paused until the amended tasks and RED packet pass their separate reviews; T029d remains the unfiltered spec-123 verification gate. These decisions implement the approved spec and are not ratification records.
...
**T024/T025 versus T028/T029 proof ownership:** the authority/accessor stage proves capture from an actual outer D1 durable `ScheduleActivity` dispatch plus stack-restoring ambient nesting semantics; it does not claim a real D2→D1 execution path. T028 owns the qualifying/nonqualifying Immediate-boundary matrix, live-overlay delayed acknowledgement, and crash-before/after-inline convergence through ordinary durable redrive. T029 owns only the minimal generic continuation implementation, expected primarily in `CoalescingRuntimeCheckpointCommitStore` and `RuntimeCoalescingSession`; it does not change the committer's override, the preparation replayer, source-domain progression, or fusion driver. Independent final review and control-room approval passed on 2026-08-06: root slice 38/38, Runtime 1830/1831 with only the clean-baseline T029e RED, no T027 regression, and Groundwork 844/853 with nine known pre-T029 REDs. T029e then resolved that separate live-carrier gap: after prepared finalization returns a new `Committed` result, the committer may publish only an exact raw candidate whose identity/association and intent/work-item payloads structurally equal the authoritative final durable intent; replay, skip, failure, exception, mismatch, and coalescing ownership never publish. Source-owner and independent RED reviews passed, control-room approved, implementation review corrected one P2 allocation finding, and final review passed with no open finding; `RuntimeInProcessHopFastPathTests` passed 19/19, the preparation/committer subset 50/50, and full Runtime 1838/1838. The current `ReplaySafeFusion` inventory is 11 total: 5 PASS (isolated pump authority, resumption-barrier isolation, provider-fold harness, ReplaySafe contract probe, deterministic fingerprint) and 6 RED (the canonical byte-identity guardrails, including External). It remains the Runtime/spec-123 reconciliation work unit (T029a amendment, reopened T029b review, paused T029c implementation, T029d unfiltered verification). That unit must preserve the canonical spec-123 guardrails and use no Evidence or provider branch. T030 cannot close before that unit completes.

**Amended T029a inventory:** The generic Runtime exact-materialized-source lifecycle filter has 8 cases: 7 protective green cases and 1 intentional new-`Committed` live-session RED. The current Activity filter is 11 total: 5 PASS / 6 RED, with the exact outcomes listed above. Its historical two-file T029c approval is superseded for this inspection reconciliation by the construction-time projection amendment below, whose plan review passed and which is approved for task planning; task and RED reviews still block production. It adds `CoalescingRuntimeStateStores` and still excludes fusion driver, committer, policy, providers, Evidence, and public contracts.

**Observer/source correction:** Internal provider folds use the captured `CoalescingInner<IRuntimeCheckpointCommitStore>`, not the outer decorated store previously observed by the Activity tests. The provider observer remains on that durable provider while retaining the outer boundary-crash wrapper. A later source review found the authority scenario also contained a queued resumption barrier. Its test-only split now proves exact nested pump ownership through `PendingConsume` and separately proves the ordered `RunSchedulerWork` fallback with a new durable authority and no leakage. The full filter is 11 total: 5 PASS / 6 RED. Its historical independent/control-room approval remains evidence for the authority split, not authorization to implement the construction-time projection.
...
The root cause is cadence, not a missed durable-baseline invalidation. The current store memoizes the durable baseline: a Deferred logical contribution is invisible to the next build, while an equivalent Immediate contribution becomes durable and is observed. Those different `FromState`/merge inputs produce the six spec-123 `FirstCheckpointId` divergences. Existing invalidation after successful durable finalization remains an FR-024 protective invariant, but does not make Deferred work visible.
...
**Alternatives rejected:** weakening the guardrails; normalizing/omitting durable inspection state; forcing all work Immediate; changing Fusion scheduling/output; exposing a new contract/model; or branching on Evidence, provider, recovery authority, source, or checkpoint name.

This supersedes the prior two-file T029c scope for this reconciliation only. T029a first creates `CoalescingInspectionReadTests` RED/protective coverage; T029b independently reviews it. Only then may T029c change `CoalescingRuntimeCheckpointCommitStore`, `RuntimeCoalescingSession`, and `CoalescingRuntimeStateStores`. Fusion driver, committer, policy, providers, Evidence, public contracts, and source-authority routing remain excluded. T029d reruns the unchanged six guardrails and full spec-123 gate before T030.
