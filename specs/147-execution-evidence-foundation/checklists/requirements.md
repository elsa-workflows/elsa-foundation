...
- **Spec-131/spec-123 composition amendment (2026-08-06):** FR-023 through FR-025, the User Story 2 scenarios, edge cases, logical-inspection entity, and SC-012 make the generic Runtime inspection projection invariant to equivalent immediate/deferred physical checkpoint disposition. They retain logical `FirstCheckpointId` and defined cumulative inspection semantics, durable-only outside reads, durable-truth replay, and no stale successful-commit baseline. The amendment does not add Evidence, fusion, provider, source-authority, or checkpoint-name branching and does not expand #1133 value capture.
- The full spec-123 checkpoint/state guardrails remain intact. Consistent with spec-123 research §8.2, the only comparison exclusion is transient post-commit intents/outbox rows; production replay fingerprints and every durable state family, including activity inspection state, remain strict. The spec-131 historical durable-baseline memo rationale and its original non-goal are retained as evidence and are superseded only for the new equivalent-logical-sequence composition contract.
- Checklist revalidation after the amendment: all mandatory sections remain complete; the new requirements have acceptance scenarios, edge cases, entities, and measurable success criteria; no clarification markers or implementation prescriptions were introduced. This specify-only change does not authorize plan, task, test, production, map, ADR, or commit changes.
- **Fusion-order amendment (2026-08-06):** User Story 2, FR-023, the logical-inspection entity, SC-012, and the edge case now make the canonical durable logical checkpoint order invariant across fusion on/off for semantically equivalent branch/fan-in proposal shapes. Durable inspection-provenance `WorkflowCheckpointOrder` is semantically meaningful evidence, not a fold count or transient physical-boundary detail; it cannot be reordered, normalized, omitted from comparison, or excluded. Independent branch proposals retain their canonical order. The amendment neither adds an Execution Evidence contract to Runtime nor changes the Evidence/Runtime dependency direction, and it does not widen lifecycle, value, UI, or J-Test scope.
