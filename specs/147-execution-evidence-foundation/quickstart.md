...
This is the implementation-validation guide for the approved Path A recovery-authority and durable scheduler-continuation plan amendments. T029e is complete as a separate bounded Runtime live-carrier resolution. The **historical pre-T029c** logical-inspection packet passed independent and control-room review: 24 tests, 14 protective PASS / 10 intentional RED, materialized-source lifecycle 8/8 PASS, and unchanged guardrails 1/7 PASS with six canonical `FirstCheckpointId` REDs. The current dirty candidate is distinct: full `ReplaySafeFusion` is 9/11 (guardrails 5/7, with two canonical-order failures: right `ScheduleActivity` 16 ON versus 13 OFF), and full Runtime is 1869/1871 with the D1 `Missing` and focused successor-materialization REDs. T029c is authorized in exactly the three planned production files, while implementation and T029d remain pending. A separately planned D2 fusion-routing canonical-order repair requires its own focused RED/review gate and is outside that authorization. This guide does not ratify the Draft constitution or proposed ADRs and does not claim that the projects, endpoints, or feature IDs already exist.
...
5. **Amended generic materialized-source lifecycle RED:** The historical reviewed packet's `RuntimeCheckpointCoalescingTests` 8-case filter passed **8/8**. It is not a current-candidate result and travels with the reviewed 24-test packet above; the corrected current focused status is stated in step 4. The former two-file T029c authorization is superseded for this inspection reconciliation; after independent RED review PASS with no P0–P3 finding and control-room approval, T029c is authorized exactly in `CoalescingRuntimeCheckpointCommitStore`, `RuntimeCoalescingSession`, and `CoalescingRuntimeStateStores`. Fusion driver, committer, policy, providers, Evidence, and public contracts remain excluded.
   ...
   **T029f corrected D1 successor-materialization RED (complete, 2026-08-06):** The focused command was:
   ...
   remains a separate RED and is not replaced by these focused tests. Fresh independent review found P0/P1/P2/P3 all zero and passed the corrected RED for implementation gating. T029c and T029g onward remain incomplete.
