...
This is the implementation-validation guide for the approved Path A recovery-authority and durable scheduler-continuation plan amendments. Both amendments passed independent review and control-room approval; their numbered RED artifacts and production implementations retain the separate review gates below. This guide does not ratify the Draft constitution or proposed ADRs and does not claim that the projects, endpoints, or feature IDs already exist.
...
4. **Spec 123 continuity:** run the original guardrails without replacing or weakening them. All five ON/OFF shapes must remain byte-identical: straight-line hot loop, multi-outcome branch, fan-in join fallback, suspend/resume, and External activity. The ON run must prove non-vacuous D1 `FusedSpans > 0` and D2 `InlineCascadeDispatches > 0`; D1-only must have fused spans with zero inline cascades; the join shape must increment its fallback counter; OFF/External must not claim leaf fusion. Preserve all original Groundwork kill ordinals `2, 3, 4, 7, 9, 10, 11, 12`, including the D2→D1 recursion boundary, plus the original join, External-parent, and suspend fallback convergence. The additive three-member identity test is not a substitute for any original guardrail.

5. **Durable scheduler-continuation boundary (T028/T029):** first add RED coverage in `tests/Elsa/Workflows/Runtime/Tests/{RuntimeCheckpointCoalescingPolicyTests.cs,RuntimeCheckpointCoalescingTests.cs}` and `tests/Elsa/Activities/Runtime/Tests/ReplaySafeFusionAuthorityScopeTests.cs`, review it independently, and only then implement. Prove the generic after-enrichment rule still forces Immediate for any post-commit work, context snapshot, or context mutation. For a successful context-empty/unmutated commit containing only nonempty `EnqueueSchedulerWork` rows, assert that the exact committed Pending rows/IDs are imported and marked durably persisted, the committed boundary state enters the overlay, the current source advances/consumes exactly as at a cap flush, and the session stays active. Drive the real shipped D2 completion pump into D1 and observe every nested preparation retaining the original outer durable `ScheduleActivity` authority. Assert the existing outbox processor/decorator/scheduler overlay/driver/queue creates no duplicate intent or work item and invokes no handler directly. Crash once before inline dispatch and once after inline dispatch but before the later checkpoint/fold: in both cases the original row remains durable Pending and normal redrive converges idempotently. Only after that later checkpoint/fold incorporates the inline effect and reconciliation runs may the durable row become Delivered. Mixed/arbitrary/external outbox, context-only or context-mutating commits, delivery failure, and terminal/no-continuation state must deactivate the session and use ordinary durable processing. T029 owns `src/Elsa/Workflows/Runtime/Services/Coalescing/{CoalescingRuntimeCheckpointCommitStore.cs,RuntimeCoalescingSession.cs}` and reuses the existing `RuntimePostCommitOutboxProcessor`, `CoalescingRuntimePostCommitOutboxStore`, `CoalescingWorkflowSchedulerWorkQueue`, `ReplaySafeFusionDriver`, and scheduler queue path. It must not weaken the generic Immediate override, invoke a handler directly, duplicate an intent/work item, acknowledge delivery early, or add D1/D2/Evidence/fusion/provider branches. Change `RuntimeCheckpointCommitter.cs` or `CoalescingRuntimeCheckpointPersistencePolicy.cs` only if the reviewed RED proves a defect in their existing override/skip behavior.

The eight unfiltered test projects and `Elsa.Server.slnx` build above are the exact spec 123 SC-008 gate. Record their pass counts and the five-shape/counter/kill-point evidence; a filtered Runtime/Groundwork run alone cannot close the gate.
