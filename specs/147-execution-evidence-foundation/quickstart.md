...
This is the implementation-validation guide for the approved Path A recovery-authority and durable scheduler-continuation plan amendments. T029e is complete as a separate bounded Runtime live-carrier resolution. The later T029 construction-time logical-inspection-projection amendment passed independent and control-room plan review with no findings and is approved for task planning. Production remains paused until its amended tasks and RED packet pass separate reviews. It supersedes the former two-file T029c scope only for the six spec-123 inspection-state REDs; T029d remains the unfiltered verification gate. This guide does not ratify the Draft constitution or proposed ADRs and does not claim that the projects, endpoints, or feature IDs already exist.
...
4. **Spec 123 continuity (T029a–T029d):** retain the original guardrails without replacing or weakening them. All five ON/OFF shapes, counters, and Groundwork kill ordinals remain owned by the canonical [spec 123](../123-replaysafe-hop-fusion/spec.md) reconciliation, including the D2→D1 recursion boundary. The current `ReplaySafeFusion` inventory is **11 total: 5 PASS / 6 RED**. PASS are the isolated pump authority proof, resumption-barrier authority isolation, provider-fold harness, ReplaySafe contract probe, and deterministic fingerprint. RED are the six guardrail byte-identity failures, including External. The construction-time projection plan review passed and it is approved for task planning; task and RED reviews still block production, and unfiltered verification follows only after its implementation review. The additive three-member identity test is not a substitute for any original guardrail.

5. **Amended generic materialized-source lifecycle RED:** `RuntimeCheckpointCoalescingTests` owns an 8-case Runtime filter: 7 protective green cases and 1 intentional new-`Committed` exact-source RED requiring the live session to remain active. It travels with the current Activity inventory above: `ReplaySafeFusion` is 5 PASS / 6 RED, with both authority scenarios and the provider-fold harness green. The former two-file T029c authorization is superseded for this inspection reconciliation by the approved-for-task-planning scope `CoalescingRuntimeCheckpointCommitStore`, `RuntimeCoalescingSession`, and `CoalescingRuntimeStateStores`; task and RED reviews still block production, and fusion driver, committer, policy, providers, Evidence, and public contracts remain excluded.

   **Reopened source review:** Internal provider folds run through `CoalescingInner<IRuntimeCheckpointCommitStore>`, so the provider observer stays on that captured durable store while preserving the outer boundary-crash wrapper. The former authority scenario also ran a queued `RunSchedulerWork` barrier. Its split uses the scoped pending-consume claim to prove exact nested pump ownership and separately proves ordered resumption fallback with a distinct later authority and no leakage. The full filter is 11 total: 5 PASS / 6 RED. That historical approval proves authority isolation; the construction-time projection plan review has now passed and authorizes task planning only, while task and RED reviews still block production.
...
The eight unfiltered test projects and `Elsa.Server.slnx` build above are the exact spec 123 SC-008 gate. T029e is complete; T029a–T029d remain blocked on the amended task and RED reviews, then must record pass counts and canonical shape/counter/kill-point evidence. The known full Groundwork baseline remains 844/853 with nine pre-T029 REDs until T029d runs the unfiltered/kill gate; do not claim it green. A filtered Runtime/Groundwork run alone cannot close the gate.
