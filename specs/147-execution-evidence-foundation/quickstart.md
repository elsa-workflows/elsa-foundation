...
This is the implementation-validation guide for the approved Path A recovery-authority and durable scheduler-continuation plan amendments. Both amendments passed independent review and control-room approval; their numbered RED artifacts and production implementations retain the separate review gates below. The approved T029 scope allocation keeps generic continuation in T029; T029e is complete as a separate bounded Runtime live-carrier resolution, T029a/T029b are independently PASSed and control-room APPROVED, and the remaining Activity fusion/fold follow-up gates are T029c implementation and T029d verification. This guide does not ratify the Draft constitution or proposed ADRs and does not claim that the projects, endpoints, or feature IDs already exist.
...
4. **Spec 123 continuity (T029a–T029d):** retain the original guardrails without replacing or weakening them. All five ON/OFF shapes, counters, and Groundwork kill ordinals remain owned by the canonical [spec 123](../123-replaysafe-hop-fusion/spec.md) reconciliation, including the D2→D1 recursion boundary. The inventory is five guardrail REDs, one authority-scope RED, and one provider-fold harness prerequisite that must be green before the reconciliation gate can pass. The seven REDs were reviewed as RED; T029a and T029b have independent-review PASS and control-room APPROVED dispositions; T029c remains pending implementation without Evidence/provider branches, followed by unfiltered verification. The additive three-member identity test is not a substitute for any original guardrail.

5. **Amended generic materialized-source lifecycle RED:** `RuntimeCheckpointCoalescingTests` owns an 8-case Runtime filter: 7 protective green cases and 1 intentional new-`Committed` exact-source RED requiring the live session to remain active. This travels with the Activity 5+1+1 RED inventory above, all currently RED including the provider-fold harness. T029a/T029b independent-review PASS and control-room APPROVED dispositions authorize pending T029c implementation only in `src/Elsa/Workflows/Runtime/Services/Coalescing/{CoalescingRuntimeCheckpointCommitStore.cs,RuntimeCoalescingSession.cs}`; fusion driver, committer, policy, providers, Evidence, and public contracts remain excluded.
