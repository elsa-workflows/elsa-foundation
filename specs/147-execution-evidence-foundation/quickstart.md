...
This is the implementation-validation guide for the approved Path A recovery-authority and durable scheduler-continuation plan amendments. T029e is complete as a separate bounded Runtime live-carrier resolution. T029c/T029g/T029h are also complete: the three-file construction-time inspection projection and Session-only distinct-source correction passed independent implementation review after closing one public-surface and one DRY finding. Current evidence is 37/37 combined inspection/materialized-source/D1 tests, 48/48 `RuntimeCheckpointCoalescingTests`, and 2/2 authority-scope tests. The separate D2 T029i–T029l lane is complete: full `ReplaySafeFusion` is 12/12, the unchanged guardrail class is 8/8, and the first independent right `ScheduleActivity` is canonical order 13 in both modes. The unfiltered T029d gate remains pending. This guide does not ratify the Draft constitution or proposed ADRs and does not claim that the projects, endpoints, or feature IDs already exist.
...
4. **Spec 123 continuity (T029a–T029d):** retain the original guardrails without replacing or weakening them. All five ON/OFF shapes, counters, and Groundwork kill ordinals remain owned by the canonical [spec 123](../123-replaysafe-hop-fusion/spec.md) reconciliation, including the D2→D1 recursion boundary. The **historical pre-T029c** packet is **24 total: 14 protective PASS / 10 intentional RED**, materialized-source lifecycle **8/8 PASS**, and unchanged guardrail lane **1/7 PASS** with six canonical `FirstCheckpointId` REDs. After the independently reviewed D1 and D2 corrections, full `ReplaySafeFusion` is **12/12** and the unchanged guardrail class is **8/8**; both modes now retain 24 unique durable commits, complete equal membership, complete byte-identical proposal traces, and right-`ScheduleActivity` order **13 ON / 13 OFF**. The last full Runtime snapshot before those corrections was **1869/1871**; T029d owns the new unfiltered result and the real D1 recovery reconciliation. Only the existing `ReplaySafeFusionDriver` received the narrow D2 locality correction; generic checkpoint/provenance/inspection/coalescing code does not inspect fusion, Evidence, provider, recovery, source, or checkpoint-name concepts, and no durable fusion model or order normalization was added. The additive three-member identity test is not a substitute for any original guardrail.

5. **Amended generic materialized-source lifecycle RED:** The historical reviewed packet's `RuntimeCheckpointCoalescingTests` 8-case filter passed **8/8**. It is not a current-candidate result and travels with the reviewed 24-test packet above; the corrected current focused status is stated in step 4. The former two-file T029c authorization is superseded for this inspection reconciliation; after independent RED review PASS with no P0–P3 finding and control-room approval, T029c is authorized exactly in `CoalescingRuntimeCheckpointCommitStore`, `RuntimeCoalescingSession`, and `CoalescingRuntimeStateStores`. Fusion driver, committer, policy, providers, Evidence, and public contracts remain excluded.

   **T029f corrected D1 successor-materialization RED (complete, 2026-08-06):** The focused command was:

   ```bash
   dotnet test tests/Elsa/Workflows/Runtime/Tests/Elsa.Workflows.Runtime.Tests.csproj --no-restore --filter 'FullyQualifiedName~PreparedMaterializedRecoverySource_DistinctSuccessor|FullyQualifiedName~CompletedMaterializedRecoverySource_UnclaimedDistinctSuccessor|FullyQualifiedName~CompletedMaterializedRecoverySource_LiveClaimedDistinctSuccessor' --verbosity minimal
   ```

   The pending-source case is now **1/1 PASS** and proves the exact two durable identities without relying on provider queue order. Its former direct commit tail was removed because the later preparation advances the provider order revision, so committing the earlier token must conflict by design and does not test source materialization. The unclaimed and live-claimed post-completion cases are **0/2 intentional RED**: each retains Deferred requested/token modes and exact authority but finds no durable successor. The related materialized/nonexact filter is **10 PASS / 2 intentional RED**. The real D1 recovery/source-redrive integration
   `RuntimeCheckpointCoalescingRecoveryTests.D1_schedule_start_invoke_reservations_retain_exact_identity_and_normal_source_redelivery`
   remains a separate RED and is not replaced by these focused tests. Fresh independent review found P0/P1/P2/P3 all zero and passed the corrected RED for implementation gating.

   **T029c/T029g/T029h implementation result (complete, 2026-08-06):** `RuntimeCoalescingSession` now records one completed materialized-source identity, keeps same-authority retries Deferred without rematerializing, and idempotently copies later distinct exact sources to the durable queue without binding or advancing a second Immediate frontier. The active matching session exposes its logical inspection projection only through an internal implementation surface; Deferred contributions publish after buffer acceptance, and Immediate/fold trailing contributions publish only after a new committed finalization. The final combined filter passed **37/37**, `RuntimeCheckpointCoalescingTests` passed **48/48**, and `ReplaySafeFusionAuthorityScopeTests` passed **2/2**. Independent review first blocked a public helper and repeated queue-seeding code; the corrected internal helper/private seed function then passed re-review with P0/P1/P2/P3 all zero. `git diff --check` passed, the scope remained exactly the three authorized coalescing production files, and the separate D2 proposal-order failures were not treated as part of this gate.

   **T029i/T029j D2 proposal-trace RED (complete, 2026-08-06):** The new focused branch trace proves **24 unique durable commits per mode**, equal cardinality, identical complete `CommitId` membership, and equal right-schedule `CommitId`, `CheckpointId`, and checkpoint name. It then records the exact right `ScheduleActivity` order **16 ON / 13 OFF** and intentionally fails only on complete trace equality including `WorkflowCheckpointOrder`; the first divergence is zero-based position 12. The full guardrail class is **5 PASS / 3 expected RED**: the two pre-existing branch/diamond byte-identity failures plus this focused trace. Independent review passed with P0/P1/P2/P3 all zero, one test-file scope, no normalization/exclusion, and no D1/recovery or existing-protection change. T029k may now change only `ReplaySafeFusionDriver`.

   **T029k/T029l D2 locality result (complete, 2026-08-06):** `ReplaySafeFusionDriver` now retains a contiguous nested-schedule cohort and executes all derived Starts before any derived Invoke, preserving the discrete FIFO proposal order without rewriting durable documents. If a stage fails or is cancelled, the exact remaining Starts are enqueued first and already-derived Invokes second; entries remain retained until successful enqueue/dispatch and the original durable Schedule remains the crash backstop. The focused complete trace is green with **24 unique commits per mode**, byte-identical membership and proposal traces, and right `ScheduleActivity` order **13 ON / 13 OFF**. Root reruns passed guardrails **8/8** and full `ReplaySafeFusion` **12/12**; accepted authority/harness evidence is **4/4**. Independent review initially blocked two continuation-loss paths and duplicate recovery handling; after correction, re-review passed with P0/P1/P2/P3 all zero. The Groundwork kill filter is **0/8** on both exact pre-driver commit `69551c45f` and this candidate with the same ordinal failure matrix (non-convergence at 2, 3, 7, 9, 11, 12; missing durable execution at 4; expected injection absent at 10). This proves no T029k regression but is not a green kill-gate claim; T029d owns baseline reconciliation.

   **Reopened source review:** Internal provider folds run through `CoalescingInner<IRuntimeCheckpointCommitStore>`, so the provider observer stays on that captured durable store while preserving the outer boundary-crash wrapper. The former authority scenario also ran a queued `RunSchedulerWork` barrier. Its split uses the scoped pending-consume claim to prove exact nested pump ownership and separately proves ordered resumption fallback with a distinct later authority and no leakage. That historical approval proves authority isolation; the amended construction-time packet has now passed its task and RED reviews and authorizes T029c only in the reviewed three-file boundary.

6. **Durable scheduler-continuation boundary (T028/T029):** first add/review the Runtime RED coverage in `tests/Elsa/Workflows/Runtime/Tests/{RuntimeCheckpointCoalescingPolicyTests.cs,RuntimeCheckpointCoalescingTests.cs}`, then implement only the generic contract. Prove the generic after-enrichment rule still forces Immediate for any post-commit work, context snapshot, or context mutation. For a new successful `Committed` context-empty/unmutated commit containing only nonempty `EnqueueSchedulerWork` rows, assert that the exact committed Pending rows/IDs are imported and marked durably persisted and the committed boundary state enters the overlay. The exact materialization/consume transition requires T029 implementation-and-review evidence; eligibility remains generic and no durable recovery input may depend on memory-only scheduler work. `Replay` uses ordinary deactivation/advancement. Assert the existing outbox processor/decorator/scheduler overlay/queue creates no duplicate intent or work item and invokes no handler directly. While that overlay remains live, inline delivery keeps the original row Pending until a later checkpoint/fold incorporates the effect and reconciliation succeeds. Crash once before inline dispatch and once after inline dispatch: both begin with that original Pending row; after session loss ordinary durable redrive idempotently enqueues the exact durable scheduler item and may mark Delivered immediately, while a crash between enqueue and the mark repeats without a duplicate. Mixed/arbitrary/external outbox, context-only or context-mutating commits, delivery failure, and terminal/no-continuation state must deactivate the session and use ordinary durable processing. T029 owns `src/Elsa/Workflows/Runtime/Services/Coalescing/{CoalescingRuntimeCheckpointCommitStore.cs,RuntimeCoalescingSession.cs}` and reuses existing generic outbox/overlay/queue seams. Historical final review and control-room approval passed on 2026-08-06: root slice 38/38, Runtime 1830/1831 with only the clean-baseline T029e RED, no T027 regression, and Groundwork 844/853 with nine known pre-T029 REDs. T029e then separately resolved `RuntimeInProcessHopFastPathTests.Committer_PublishesContinuation_ToOwningLiveDrainCarrier`: a raw candidate is captured only for the owning live drain and published only after a new `Committed` prepared finalization has one exact final durable intent with matching identity/association plus structural equality of the candidate intent payload and independently serialized candidate work-item payload. The durable payload remains authoritative; replay, skip, failure, exception, mismatch, and coalescing ownership do not publish. Source-owner/independent RED reviews passed, control-room approved, corrected implementation review passed with no open finding, and historical verification was 19/19 focused tests, 50/50 preparation/committer subset, and 1838/1838 full Runtime. The current dirty candidate's full Runtime 1869/1871 status, including D1 `Missing` and focused successor-materialization REDs, is stated in step 4. T029a/T029b are complete and T029c is authorized only in the reviewed three-file boundary. This resolution must not weaken the generic Immediate override, invoke a handler directly, duplicate an intent/work item, acknowledge memory-only delivery, or add D1/D2/Evidence/fusion/provider branches.

7. **Construction-time inspection projection (amended T029a–T029d):** retain and amend `CoalescingInspectionReadTests`; do not delete its current byte-identity, durable-pass-through, or read-observability objectives. First prove the RED that accepted Deferred state is visible to the next in-session inspection build, producing the same `FromState`/merge input as the equivalent Immediate path. Then prove active projection precedence for both `CoalesceInspectionReads` modes; no-projection ON memoization versus OFF per-call reads; Immediate/fold trailing update only after new `Committed`; no double-application of folded buffered contributions; baseline-memo invalidation after successful finalization; no candidate update on failure/conflict/ownership loss/replay/skip; cap, quiescence, deactivation, and crash/replay behavior. The scope is exactly `CoalescingRuntimeCheckpointCommitStore`, `RuntimeCoalescingSession`, and `CoalescingRuntimeStateStores`; generic checkpoint/provenance/inspection/coalescing code may not inspect fusion, Evidence, provider, recovery, source, or checkpoint-name concepts. The six existing spec-123 guardrails remain unchanged and must become green without normalization or weakening.

The eight unfiltered test projects and `Elsa.Server.slnx` build above are the exact spec 123 SC-008 gate. T029a–T029c, T029e–T029l are complete; T029d remains pending. T029d must record pass counts and canonical shape/counter/kill-point evidence. The Groundwork kill filter is currently 0/8 on both the exact pre-driver baseline and candidate; T029d must reconcile that known baseline rather than claim either the historical 844/853 snapshot or the kill gate is green. A filtered Runtime/Groundwork run alone cannot close the gate.
