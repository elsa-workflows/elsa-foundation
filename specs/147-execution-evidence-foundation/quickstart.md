...
4. **Spec 123 continuity (T029a–T029d):** retain the original guardrails without replacing or weakening them. All five ON/OFF shapes, counters, and Groundwork kill ordinals remain owned by the canonical [spec 123](../123-replaysafe-hop-fusion/spec.md) reconciliation, including the D2→D1 recursion boundary. The **historical pre-T029c** packet is **24 total: 14 protective PASS / 10 intentional RED**, materialized-source lifecycle **8/8 PASS**, and unchanged guardrail lane **1/7 PASS** with six canonical `FirstCheckpointId` REDs. The current dirty candidate is full `ReplaySafeFusion` **9/11**: guardrails **5/7**, with two right-`ScheduleActivity` canonical-order failures (16 ON, 13 OFF). The last full Runtime snapshot before the corrected focused RED was **1869/1871**; the corrected focused materialized-source lane is **1 PASS / 2 intentional RED**, and the real D1 `Missing` remains a separate RED. T029c is authorized only in the three planned production files, and unfiltered T029d verification follows only after implementation review. Separately, the D2 routing RED must prove equal ON/OFF durable commit membership/cardinality while recording that order divergence, then prove equal canonical branch proposal order and byte-identical durable documents after repair. Only the existing `ReplaySafeFusionDriver` may receive the narrow locality correction after that RED/review; generic checkpoint/provenance/inspection/coalescing code may not inspect fusion, Evidence, provider, recovery, source, or checkpoint-name concepts, and no durable fusion model is permitted. The repair must not normalize/exclude durable order, rewrite provenance, add an Execution Evidence dependency, or use the fusion toggle as its correctness solution. The additive three-member identity test is not a substitute for any original guardrail.
...
The eight unfiltered test projects and `Elsa.Server.slnx` build above are the exact spec 123 SC-008 gate. T029a–T029c, T029e–T029h are complete; the D2 T029i–T029l lane and T029d remain pending. T029d must record pass counts and canonical shape/counter/kill-point evidence. The known full Groundwork baseline remains 844/853 with nine pre-T029 REDs until T029d runs the unfiltered/kill gate; do not claim it green. A filtered Runtime/Groundwork run alone cannot close the gate.
