...
| Testing | xUnit project tests, TestServer/FastEndpoints API integration, backend PowerShell e2e, deterministic CAS/replay/coalescing/concurrency tests, actual D2→D1 authority and Immediate-boundary crash/reconciliation RED, spec 123 D1/D2 crash-convergence guardrails, source-independent current/older-fence recovery tests, and benchmark observations. |
...
| Refactoring continuity | Preserve spec 123's byte-identical and crash-convergence objectives, including D1, D2, join/External/suspend fallback, fusion counters, and the existing idempotency ladder. A qualifying Immediate boundary reuses the durable outbox authority and existing overlay/D2 machinery; it does not replace the source authority or rewrite the fusion driver. The partial fusion fixture rewrite is not accepted as a reduction in guardrail coverage. |
...
T028/T029 own the real shipped D2→D1 authority path plus Immediate-boundary crash and delayed-reconciliation
proof/implementation. This approval authorizes the amended task contract; it does not approve a RED artifact or production
...
| Qualifying durable scheduler continuation | An internal coalescing handoff after a successful Immediate commit whose context snapshot is empty, context mutation is absent, and nonempty committed outbox consists only of `EnqueueSchedulerWork`. It imports the exact committed Pending rows/IDs into the active overlay, never creates a second row or work item, and delays durable `Delivered` until a later successful checkpoint/fold incorporates the inline effect and reconciliation runs. |
...
5a. **Continue only across a qualifying durable scheduler boundary.** After the Immediate CAS succeeds, a still-active session examines only generic committed state. If context is empty/unmutated and every nonempty outbox row is `EnqueueSchedulerWork`, it imports those exact committed Pending rows/IDs, marks them durably persisted, applies the durable boundary state to the overlay, advances/consumes the current source as a cap flush does, and remains active. The existing outbox processor/overlay/queue and D2 pump consume them. Durable rows remain Pending until a later successful checkpoint/fold incorporates the inline effect and existing reconciliation records `Delivered`. A crash before or after inline dispatch therefore redrives the same idempotent durable row. Mixed/arbitrary/external outbox, context-only/context-mutating, delivery-failure, and terminal/no-continuation boundaries deactivate normally.
...
| Successful Immediate commit with empty/unmutated context and only `EnqueueSchedulerWork` rows | Import exactly the committed Pending rows/IDs into the active overlay, mark them durably persisted, apply the boundary state, advance/consume the source as at a cap flush, and keep the session active. No direct dispatch, duplicate intent, or duplicate work item. |
| Crash before or after inline scheduler-continuation dispatch | The original durable row remains Pending until a later successful checkpoint/fold and reconciliation. Normal outbox redrive reuses the same intent/work-item identity and Path A source authority; no early `Delivered` acknowledgement or memory-only authority. |
...
| Runtime seams | `dotnet test tests/Elsa/Workflows/Runtime/Tests/Elsa.Workflows.Runtime.Tests.csproj` and Groundwork tests — v1 authority canonicalization/bounds; actual outer D1 dispatch authority plus ambient nesting; all six router results including claimed-but-durable exactness; immutable original versus mutable current fence/revision; exact-set adoption success, idempotent replay, every rejection, and injected all-member rollback/no-dispatch for source-bound and source-free routes; exact three-member D1 Prepared identity/token/provenance/order/source assertions; 28-caller matrix; fingerprint/fold/compaction; explicit skipped/failed orders; unchanged after-enrichment Immediate override; qualifying scheduler-only import/continued session; nonqualifying deactivation; crash-before/after-inline dispatch with delayed reconciliation; skip-with-work; six-status paging. |
| Fusion continuity | Preserve the original spec 123 tests as guardrails, not rewritten substitutes: all five ON/OFF byte-identical shapes (straight line, multi-outcome branch, fan-in join fallback, suspend/resume, External); non-vacuous D1 `FusedSpans` and D2 `InlineCascadeDispatches` counters plus join-fallback counter; D1-only counter separation; and all eight original Groundwork kill ordinals `2,3,4,7,9,10,11,12`, including D2→D1 recursion. T028/T029 must additionally prove that the real shipped D2 pump re-enters D1 under the original outer authority across the qualifying durable scheduler boundary. The additive three-member Prepared assertions must not replace original convergence, join, External-parent, suspend, or no-fusion coverage. |
...
The final regression gate also runs every project named by spec 123 SC-008, without filters or substitutions:
...
| Durable scheduler-continuation handoff | A post-commit-bearing D2 edge must remain an honest Immediate commit without ending safe same-drain locality. Exact Pending rows plus delayed acknowledgement retain crash authority. | Direct handler invocation, duplicate intent/work, early `Delivered`, or disabling the override loses durability or changes accepted source semantics. |
