...
the Runtime coalescing handoff. The current `ReplaySafeFusion` inventory is **10 total: 3 PASS / 7 RED**. PASS are the
provider-fold harness prerequisite, ReplaySafe contract probe, and deterministic fingerprint. RED are the outer/nested
authority mismatch plus six guardrail byte-identity failures, including External. These checks remain required by the
...
independent review passed and the control room approved it; T029c is authorized to resume. The amended T029a inventory also owns a generic Runtime exact-materialized-source
...
**Reopened T029b review (2026-08-06):** T029c source inspection showed that the two test observers were attached to
the outer checkpoint-store services, while provider-atomic internal folds use the captured
`CoalescingInner<IRuntimeCheckpointCommitStore>`. The test-only correction decorates that durable provider without
changing the outer boundary-crash behavior. The resulting `ReplaySafeFusion` inventory is **10 total: 3 PASS / 7 RED**:
the provider-fold harness, ReplaySafe contract probe, and deterministic fingerprint pass; the authority outer/nested
mismatch and six guardrail byte-identity checks, including External, remain RED. Independent re-review passed and the
control room approved T029c to resume; the two-file production boundary remains unchanged.
