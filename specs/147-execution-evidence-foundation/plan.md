...
**Status:** T029 construction-time logical-inspection-projection plan, tasks, and RED packet approved (2026-08-06). The **historical pre-T029c** packet produced 24 tests: 14 protective PASS / 10 intentional RED; materialized-source lifecycle was 8/8 PASS, and unchanged guardrails were 1/7 PASS with six canonical `FirstCheckpointId` REDs. Independent RED review passed with no P0–P3 finding and the control room authorized T029c in exactly the three planned production files. The current dirty candidate is not that historical baseline or an approval result: full `ReplaySafeFusion` is 9/11 (guardrails 5/7, with two canonical-order failures: right `ScheduleActivity` 16 ON versus 13 OFF), and full Runtime is 1869/1871 with the D1 `Missing` and focused successor-materialization REDs. The separate D2 canonical-order repair requires its own focused RED/review gate and is not included in the three-file T029c authorization. The 2026-08-05 plan, Path A recovery-authority amendment, and qualifying Immediate-boundary amendment also passed their applicable reviews. The constitution remains Draft and the referenced ADRs remain proposed until their separate governance review.
...
| Hard constraints | Generic checkpoint, provenance, inspection, and coalescing paths MUST NOT inspect or branch on fusion, Execution Evidence, provider, recovery, source, or checkpoint-name concepts, and no fusion-specific durable Runtime branch, identifier, setting, or model is permitted. The sole narrow locality exception is the existing `ReplaySafeFusionDriver`, and only after its dedicated D2 RED/review gate; it may preserve generic proposal sequencing but may not create a durable fusion model. Preserve accepted ADR 0047/spec 123 source redrive and the generic after-enrichment Immediate override; no direct handler invocation, duplicate continuation intent/work item, or acknowledgement of memory-only overlay delivery; deterministic intent identity; checkpoint-atomic recording; at-least-once idempotent materialization; no synthetic order, inferred terminal disposition, or premature nonterminal compaction; no #1134 barriers/gap-free claims, #1136 values, #1137 Evidence durability, UI, shared fixtures, or J-Test. |
...
| Refactoring continuity | Preserve spec 123's byte-identical and crash-convergence objectives, including D1, D2, join/External/suspend fallback, fusion counters, and the existing idempotency ladder. The generic qualifying-Immediate handoff reuses durable outbox authority and does not replace source authority or rewrite the fusion driver. The historical pre-T029c `ReplaySafeFusion` result was 5 PASS / 6 RED, with six unchanged `FirstCheckpointId` guardrails. The current dirty candidate is 9/11: guardrails 5/7, with two durable canonical-order failures (right `ScheduleActivity` 16 ON versus 13 OFF), not transient transport drift. Construction-time projection remains one reviewed generic reconciliation path, but cannot replace the separately proven D2 routing repair. Only the existing `ReplaySafeFusionDriver` may receive a narrowly reviewed locality correction; generic checkpoint/provenance/inspection/coalescing remains unconditional isolation. |
...
The generic scheduler-continuation handoff is complete and remains separate from this work. The **historical pre-T029c**
`ReplaySafeFusion` inventory was **11 total: 5 PASS / 6 RED**. The five PASS were the isolated D2-pump authority proof,
...
The construction-time concern is generic coalescing state: the existing inspection store treats a session-local read as a
durable-baseline memo, so a Deferred logical contribution can remain invisible to the next inspection build whereas an
equivalent Immediate contribution has already become durable and is observed. That cadence can select different
`FromState`/merge inputs and must remain covered by T029c. Successful durable finalization already invalidates its
baseline memo; retaining that FR-024 invariant is protective but is not a substitute for canonical routing order. This
amendment deliberately and only supersedes the prior spec-131 **control-read semantics**: it retains the tests'
byte-identity, durable-pass-through, and read-observability objectives, but no longer treats an active session's logical
projection as ineligible for `FindAsync`.

T029a added the reviewed RED/protective packet and T029b independently reviewed that historical baseline: 24 tests = 14 protective PASS /
...
provider and the outer boundary-crash behavior is unchanged. The historical completed amended packet is **24 total: 14 protective
...
### Canonical D2 fusion-routing order amendment (planned; separate RED/review gate)

The committed spec amendment makes durable `WorkflowCheckpointOrder` and the relative order of independent
branch proposals part of the strict FR-023/SC-012 comparison. Fresh source tracing isolates a separate first divergence
before inspection composition: a semantically equivalent branch/fan-in run retains the same durable commit set and
cardinality with fusion on and off, but the right `ScheduleActivity` receives workflow checkpoint order **16** with
fusion on versus **13** with fusion off. In the ON path, the generic D2 pump in
`src/Elsa/Workflows/Runtime/Services/ReplaySafeFusionDriver.cs` dispatches a fusable left-side completion and its D1
re-entry before all sibling schedule proposals have been made. The OFF path proposes the siblings in the canonical
logical order. This is routing locality changing durable proposal order, not a provenance, inspection, Evidence, or
provider defect.

The repair belongs only at that existing D2 fusion-routing seam and its focused test seam is
`tests/Elsa/Activities/Runtime/Tests/ReplaySafeFusionGuardrailTests.cs`, with the existing spec-123 shape builders and
the ordinary Runtime checkpoint/inspection comparisons. It must preserve the D2 pump's FIFO and fallback semantics,
the D1/D2 source-redrive contract, and all existing generic coalescing behavior, while ensuring equivalent
branch/fan-in proposal shapes present the same canonical logical proposal order before durable preparation regardless
of fusion mode. This is the sole reviewed locality exception: generic checkpoint, provenance, inspection, and coalescing
code remains unable to inspect fusion, Evidence, provider, recovery, source, or checkpoint-name concepts, and the driver
must not introduce a durable fusion model. The plan intentionally does not prescribe a particular queueing or scheduling algorithm.

The first implementation step is a focused RED which records the complete ON/OFF proposal trace for the branch/fan-in
shape, proves the same durable commit identities/set and cardinality, and pinpoints the right schedule's `16`/`13`
order divergence. The subsequent generic repair must make the full durable checkpoint and state documents
byte-identical, including inspection provenance `WorkflowCheckpointOrder`, `FirstCheckpointId`, and independent
branch order; it must preserve the non-vacuous D1/D2 and join-fallback counters. Verify the repaired shape with
fusion on/off and the applicable coalescing-read controls, then rerun all unchanged spec-123 guards and its eight
Groundwork kill ordinals before the unfiltered SC-008 gate.

Reject and roll back any candidate that changes durable commit membership/cardinality, recovers correctness by
rewriting provenance after proposal, normalizes or omits durable order from comparison, creates a fusion-specific
durable model, adds an Execution Evidence dependency or reverse dependency, or uses disabling fusion as the repair.
If the generic seam cannot retain the canonical order, keep the strict RED/guardrail open rather than weaken it. This
is distinct from the focused D1 missing-source RED already scoped to `RuntimeCoalescingSession`: that source-materialization
case remains a separate coalescing/recovery concern and must not be folded into D2 routing work.
...
failure, exception, mismatch, coalescing ownership, and unrelated commits do not publish. Historical verification: focused
...
| D2 branch/fan-in routing with fusion on versus off | The existing `ReplaySafeFusionDriver` may, after its dedicated RED/review gate, preserve the same canonical logical proposal order before durable preparation, including each independent branch's `WorkflowCheckpointOrder`; the durable commit set/cardinality, checkpoint/state documents, `FirstCheckpointId`, and existing D1/D2/join-fallback observability remain equivalent. No generic checkpoint/provenance/inspection/coalescing path may inspect fusion or a forbidden domain concept. A failed parity probe leaves the strict guardrail RED open; it is never repaired by provenance rewrite, durable-field exclusion/normalization, a fusion-specific durable model, an Evidence dependency, or disabling fusion. |
...
| Fusion continuity (T029c–T029d pending) | Preserve the original spec-123 tests as unchanged guardrails, not rewritten substitutes: all five ON/OFF byte-identical shapes (straight line, multi-outcome branch, fan-in join fallback, suspend/resume, External); non-vacuous D1 `FusedSpans` and D2 `InlineCascadeDispatches` counters plus join-fallback counter; D1-only counter separation; and all eight original Groundwork kill ordinals `2,3,4,7,9,10,11,12`, including D2→D1 recursion. The 5 PASS / 6 RED Activity result is historical pre-T029c evidence; the current dirty candidate is full `ReplaySafeFusion` 9/11, with guardrails 5/7 and two canonical-order failures (right `ScheduleActivity`: ON `16`, OFF `13`). The focused D2 RED first proves equivalent durable commit set/cardinality and that divergence, then proves canonical independent-branch order and complete byte identity after the narrow existing-driver locality repair. The six guardrails continue to compare durable checkpoint/state documents while excluding only transient intent/outbox transport. The two authority PASS cases prove exact pump ownership and resumption-barrier isolation; they do not replace original convergence, join, External-parent, suspend, or no-fusion coverage. |
...
| Canonical D2 proposal routing | Only the existing `ReplaySafeFusionDriver` locality seam may, after focused RED/review, prevent an eligible left-side D1 re-entry from allocating durable order before independent sibling schedules are proposed; it retains D1/D2 fallback/counter semantics. Generic checkpoint/provenance/inspection/coalescing remains unconditional isolation and no durable fusion model is allowed. | Post-hoc provenance rewrite, durable-order normalization/exclusion, a fusion-specific durable model, Evidence coupling, or treating the fusion toggle as the correctness fix violates FR-023/SC-012 and ADR 0047 continuity. |
