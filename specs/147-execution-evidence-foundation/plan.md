...
| Durable scheduler-continuation handoff | A post-commit-bearing D2 edge must remain an honest Immediate commit without ending safe same-drain locality. Exact Pending rows plus live-overlay delayed acknowledgement retain crash authority; after loss, durable redrive enqueues idempotently then acknowledges. | Direct handler invocation, duplicate intent/work, memory-only early `Delivered`, or disabling the override loses durability or changes accepted source semantics. |
