...
| Refactoring continuity | Preserve spec 123's byte-identical and crash-convergence objectives, including D1, D2, join/External/suspend fallback, fusion counters, and the existing idempotency ladder. The generic qualifying-Immediate handoff reuses durable outbox authority and does not replace source authority or rewrite the fusion driver. The seven Activity fusion/fold REDs remain a separately blocked spec 123 reconciliation gate; the partial fixture rewrite is not accepted as a reduction in guardrail coverage. |
