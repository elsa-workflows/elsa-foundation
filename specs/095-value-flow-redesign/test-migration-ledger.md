...
| `tests/Elsa/Activities/Http/Tests/WriteHttpResponseLiveWriteTests.cs` and adjacent HTTP execution fixtures: use wrapper-backed HTTP activity contracts | Replace | Hydrate request/response activity inputs from immutable snapshots and expose typed results without live block writes. | `Elsa.Activities.Http.Tests`; HTTP activity contract fixtures | Implemented/passing for `WriteHttpResponse`: immutable input snapshot, atomic `HttpResponseInstruction` result, and sync/async/resume delivery are covered by 190 unit and 29 integration tests (2026-07-16). `HttpEndpoint` remains on the legacy start-trigger contract pending typed start-stimulus identity and registration metadata. |
