...
- [ ] T126 **Add an e2e test that proves the headline claim against a running server (Joey, 2026-08-18).** Spec 151 has **no automated e2e test at all** — T114 is a manual walkthrough of a doc, and every proof that an artifact built elsewhere runs on a runtime-only engine is currently **in-process**. The feature's central claim is untested against a real server.
  ...
  - Runtime-only composition is the point: no design, no publishing, no compiler. If the server-from-source shape cannot express that, say so — that is itself a finding about the deployment shape this feature exists to enable.
