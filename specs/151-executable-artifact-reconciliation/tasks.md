...
5. **Pre-existing, out of scope:** the activation slot is keyed `(DefinitionId, SlotName)` with no tenant axis, while tenancy lives on the source reference. Two tenants publishing one definition therefore contend on one slot. That is true of publishing's slot store today and is not introduced by this feature — noted so it is not mistaken for a spec-151 regression.
