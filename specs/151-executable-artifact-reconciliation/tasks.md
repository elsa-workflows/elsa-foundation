...
> **Root-cause update (2026-08-18) — the number is not a quality signal.** The set is now **42 deterministic** (was 43 before the nested worktrees were deleted) (plus `Wave5OpenTelemetryMinimalApiCollectibilityTests`, a GC-collectibility flake that passes in isolation). Decomposed:
...
- [x] T124 **Make the foreign-owner outcome a replaceable policy, defaulting to skip-and-log (Joey, 2026-08-18).** [[T118]] hardcodes what happens when a foreign source owns the slot: named skip, warning log, never reclaim. That default is right, *"but we should not close this behaviour fully — everything depends on what the user expects here."*
  ...
  - **§2.6.2 replacement contract** (§2.24.2 #5, sanctioned — no new pattern). `TryAdd` first-wins per ADR 0033; declare it in the Reconciliation extension-point catalog's *Overridable contracts* section. Keep it minimal: one member, one default, no configuration surface.
