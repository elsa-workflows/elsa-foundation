...
- **Named contracts/services in FRs are intentional, not leakage.** This spec transcribes GitHub
  ...
  types (`IRuntimeRequirementChecker`, `IWorkflowTriggerBindingStore`, the neutral runtime-owned
  activation authority + coordinator superseding publishing's slot store, the prohibition on the
  publishing-only `IPublicationRecordStore`/Publishing-assembly machinery entering the runtime, …)
  are **reviewed architectural decisions the spec is required to carry faithfully** — they define the design/runtime boundary the feature
