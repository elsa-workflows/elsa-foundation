...
3. **Given** a mixed batch, **When** reconciled, **Then** satisfiable artifacts activate and unsatisfiable ones are rejected individually (one bad artifact does not fail the batch).
...
- **Requirements checker (shared runtime service)**: the runtime-layer capability check (requirements + storage drivers, + possibly CLR-type availability) reused by both publishing's deployment preflight and the importer's import gate — moved out of `Publishing.Api` into `Workflows.Runtime.Core`.
...
- Reusing the **publishing slot/record stores** on the runtime — explicitly avoided (FR-B-006).
...
- Artifact **signing / supply-chain trust** — resolved by clarification as a follow-up: v1 relies on declared-hash closure validation within an operator-controlled source. Two named follow-up steps, in order of weight: (1) recomputed-hash tamper-evidence, which requires extracting the executable hasher from Publishing into the runtime layer (analogous to the FR-B-005 requirements-checker extraction); (2) artifact signing/verification.
