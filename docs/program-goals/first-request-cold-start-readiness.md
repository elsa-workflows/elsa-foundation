...
3. **Schema batch / skip-if-current.** Reduce the current 922-operation fresh-DB admission and skip it entirely on an
   ...
   **Skip-if-current: implemented (spec 133, SQLite), opt-in** — records an Elsa-owned applied-plan fingerprint
   ...
   but not out-of-band drift. **Batching: deferred to Groundwork** — the 922-op apply is 922 package-internal
