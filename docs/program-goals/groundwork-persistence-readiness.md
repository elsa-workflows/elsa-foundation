...
- Update (universal provider, Option 2): the design lanes were taken off `IQueryable`/LINQ and rebuilt on provider-neutral, domain-specific read ports implemented by Groundwork. The obsolete generic Elsa command/query kernel and its EF Core adapters have since been removed; host decision remains that every Elsa lane runs on a relational or document provider selected once at the host. See the [Groundwork design provider implementation plan](../reports/groundwork-design-provider-implementation-plan.md) for the remaining provider work.
