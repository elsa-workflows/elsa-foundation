...
- **ADR 0031 fast path — items (a)/(c)/(d) implemented, [spec 109](../../specs/109-runtime-inprocess-hop-fast-path/spec.md) (WU-3).** The in-process-hop payload short-circuit lands at the WU-2 live-drain seam: the checkpoint committer publishes the materialized continuation work item onto a drain-scoped carrier on `RuntimeLiveDrainDeliveryScope` and the scheduler intent dispatcher takes it instead of deserializing the durable intent payload, with the serialized payload authoritative at every persistence boundary. Guardrail (c) proves byte-identical committed state ENABLED-vs-DISABLED; (d) burst-affinity/single-writer/scale-out documented in the spec. **Step-0 evidence** ([research.md](../../specs/109-runtime-inprocess-hop-fast-path/research.md)) found serialization is ~2% of an in-memory hop / ~0.36% of a durable hop, so this is a small byte-identical cut; the dominant hot-loop costs (hop count, checkpoint-commit fsync, claim/outbox round-trips) are owned by other dials (ADR 0032 shipped, spec 106 shipped, a future hop-count unit). Item **(b)** — the burst-scoped reconstructible heavy-object cache — is the next unit.
