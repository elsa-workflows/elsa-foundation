...
15. **Planned — reusable activity definitions.** [Spec 092](../../specs/092-reusable-activity-definitions/spec.md) and its [initial plan](../../specs/092-reusable-activity-definitions/plan.md) replace the construct-only workflow-as-activity path with first-class activity drafts/versions, graph-backed inline composite execution, exact template pinning, durable restart/resume, and hierarchical inspection. API, diagnostic, version-diff, dependency/upgrade, provider/runtime, and inspection contracts are captured before Studio UX grilling and implementation task generation.
