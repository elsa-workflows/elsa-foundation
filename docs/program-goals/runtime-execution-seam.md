...
14. 📝 **Planned — not implemented** — [spec 090 (trigger publication contract hardening)](../../specs/090-trigger-contract-hardening/spec.md) establishes exact-one provider recognition, stable provider identity, artifact-only preflight, and fail-before-mutation Timer/Cron schedule materialization for Event, Timer, Cron, and HttpEndpoint. The approved boundary excludes diagnostics (future Unit B), shell composition/startup health and connected-host expansion (future Unit C), Studio, and publication-wide transactionality. See the [implementation plan](../../specs/090-trigger-contract-hardening/plan.md) and [decision map](../reports/trigger-system-hardening-decision-map.md).
