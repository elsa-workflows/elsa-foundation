...
## Per-drain-turn durable round-trip characterization (2026-07-20)

- **The "collapse per-drain-turn durable commit round-trips" engine-performance unit was characterized empirically and re-aimed** — see [spec 110](../../specs/110-drain-turn-durable-roundtrips/spec.md) + [research.md](../../specs/110-drain-turn-durable-roundtrips/research.md), and the permanent instrument `benchmarks/Elsa/Workflows/Runtime/Benchmarks/DurableRoundTripDiagnostics.cs`. Finding: under the shipped **Coalesced** cadence (ADR 0032, delivered across specs 105 + 107 + 108) the per-turn durable **commit** collapse is **already done** — deferred turns are overlay-only (zero fsync), and a 10-activity hot loop goes from **66 checkpoint commits + 194 durable scheduler-queue ops (Immediate)** to **1 commit + 2 queue ops (Coalesced)**; the 2-node from **12 + 41** to **2 + 4**. The execution-liveness fence touch, the spec-105 consumed-item delete, the overlay bookkeeping, and the commit markers are all folded into the single per-flush unit-of-work or served from the in-memory overlay; the root-write lease is workflow-level (~2/run), not per-turn. The premise that "each `CommitAsync` still fsyncs ~11ms even under coalescing" is refuted: the `elsa.runtime.checkpoint.commit` span wraps the committer call, not a guaranteed durable round-trip.
- **Residual, re-aimed target:** the redundant per-drain durable **executable-artifact reads** — `IWorkflowExecutableStore.FindAsync` runs ~5×/activity (46 reads for the 10-activity hot loop), every call resolving the same immutable content-addressed pinned artifact (ADR 0038), and survives coalescing (which batches writes, not reads). Spec 110 scopes a drain-scoped, reconstructible, immutable-by-`ArtifactId` read cache on the dispatch hot path (lease/GC reads excluded), reusing the drain DI scope so it composes with — rather than duplicates — [ADR 0031](../adr/0031-runtime-burst-execution-sticky-single-writer-drain-with-in-process-fast-path.md)'s burst-scoped user-object cache (WU-2). No commit/fence/queue-path change is warranted (the measurement shows that path already at one atomic durable commit per flush); the cache implementation is left as a distinct reviewed unit because its risk is DI lifetime/captivity and GC-read correctness, not the commit path.
