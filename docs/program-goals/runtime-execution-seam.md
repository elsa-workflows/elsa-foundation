...
- **New work units surfaced.** (a) **Route runtime execution through the workflow + activity pipelines** — a distinct, larger unit (own ADR + spec) to wire the scaffolded pipeline into the scheduler handlers so the module system can extend runtime execution; arguably more foundational than the expression-context edge. Sizing via an inlined-phase → slot mapping across the handlers is in progress. (b) The sensitive-value payload-capture default (Slice 7) is unverified — confirm or specify.
