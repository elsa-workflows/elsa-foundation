...
4. Complete diagnostics #642's remaining preview.85 provider-evidence, performance, and EF-removal gates; replay checkpoints #1048 and #1072 are on `main`, and stale PR #660 is closed as historical reference.
...
- ASP.NET Core Identity #644 is closed. Its `preview.60` artifacts and Spec 094's `preview.76`/`preview.77`/`preview.80`/`preview.81` attachments are immutable historical provenance; the active repository consumes Groundwork `0.0.1-preview.85` from exact upstream source `f4450175a853cc6eb1a6ef2e7c81802a6ae5dc18`, but has no imported preview.85 provider evidence yet. Only a fresh exact-source publication and mechanical import can advance the current ledger. The checked-in EF artifact remains a non-executed contract baseline; #646 owns real same-provider EF execution, equality, timing, and physical-shape verdicts. Identity completion does not by itself complete OpenIddict, diagnostics #642, host switching, or #647 EF-family deletion.
- Groundwork PR #88 owns the generic version-aware codec contract, and the current package family is `0.0.1-preview.85`. Elsa owns marker-gated per-kind version policies, legacy-stamp parsing, JSON options, and concrete upcasters in its provider packages; no such policy belongs in Groundwork or an Elsa core module.
