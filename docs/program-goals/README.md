...
| [Groundwork Persistence Readiness](groundwork-persistence-readiness.md) | Completed | Provider-neutral persistence framework / Elsa validation bridge | Joey plus active architects/agents | Foundation extracted and validated; ADR 0073 moved remaining persistence replacement work to the EF Core Persistence goal |
| [EF Core Persistence](ef-core-persistence.md) | Active | Elsa persistence-family consolidation / four-provider EF Core delivery | Sipke plus the EF Core persistence control room | Record ADR 0073 and the initial ledger boundary, complete #1671's entry-level inventory, then resolve the four architecture spikes before broad implementation |
