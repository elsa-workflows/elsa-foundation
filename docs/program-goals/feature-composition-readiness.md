...
3. Deliver the first file-only plan command in [#2001](https://github.com/elsa-workflows/elsa-foundation/issues/2001), using the same selection result as the future builder and the reviewed [contract from #1999](../reports/runtime-composition/developer-plan-command-contract.md). Keep export/import and live host checking separate until their source and secret contracts are reviewable. Keep one active integration lane and synchronize issue comments, labels, and project readiness when work changes state.
