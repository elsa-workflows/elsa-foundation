...
3. ▶ **Next** — Speckit slice: `Elsa.Diagnostics.OpenTelemetry` OTLP ingestion + live feed + in-memory store + query API + live streaming. Source-grounded notes (elsa-core pin): the module is **already CShells-shaped** (`ShellFeatures/OpenTelemetryFeature.cs` uses `[ShellFeature]` + `IFastEndpointsShellFeature`/`IWebShellFeature` + `[ManifestSetting]`) and **already decomposed** via `TryAddSingleton` (separate `IOpenTelemetryStore`/`IOpenTelemetryLiveFeed`/`IOpenTelemetryIngestor`/`IOpenTelemetryRedactor`/`IOpenTelemetrySourceRegistry`/`IOpenTelemetryProvider`/`ICollectorConfigurationProvider`) — no decomposition refactor needed. OTLP HTTP/protobuf ingestion is **self-contained** (hand-rolled `ProtobufReader`; only `Microsoft.Extensions.Options` + `Elsa.Api.Common` deps) so it ports near-verbatim with no new protobuf package. Surface is much larger than Structured Logs (traces/spans/metrics/logs/resources, redaction of sensitive attrs + regex text, storage diagnostics, collector config). **Improvements to apply on port:** (a) replace the SignalR hub + `OpenTelemetrySubscriptionManager` with the foundation SSE transport for consistency with Structured Logs (one-way streaming; native `EventSource` reconnect); (b) defer gRPC (`EnableGrpc=false`, host-specific stub in source); (c) keep redaction. See `diagnostics-port-plan.md` (session artifact) for the file-by-file port map.
4. Speckit slice: OpenTelemetry EFCore persistence (enhancement beyond source, which was in-memory only). Reuse the Structured Logs persistence pattern: non-blocking channel-buffered writes + async batch-draining writer, `NullLoggerFactory` feedback-loop break, durable auto-increment cursor (entity not derived from `Entity`), retention pruning, migration/draining startup-task ordering. Note the OTEL store is **async** (`ValueTask WriteAsync`/`Query*Async`) vs. the synchronous Structured Logs store, so the EF override implements the async contract directly.
5. ⛔ Studio slice (elsa-foundation-studio): Structured Logs bottom-panel tab. **Cross-repo — cannot build from this worktree.** Handoff: add a bottom-panel tab via `api.panels.add` (sibling of the existing Console tab in `Elsa.Studio.ConsoleStream`) that consumes the foundation SSE `stream` endpoint + `recent`/`sources` HTTP, using native `EventSource` (no `@microsoft/signalr`). Matches the user's bottom-panel mockup (Structured Logs + OTEL as tabs alongside Console).
