...
The validation compares `.claude/skills/elsa-*/SKILL.md` against this catalog and `.specify/integrations/claude.manifest.json`, including manifest hashes, thin-wrapper metadata, and selected work-tracking / program-goal-selection lookup guardrails.
...
**Workflow:** read the work tracking and drift guard in [../../AGENTS.md](../../AGENTS.md#work-tracking-and-drift-guard), read `.agent-prefs/work-tracking-model.md` when it exists, and use [../reference/agent-preferences.md](../reference/agent-preferences.md#work-tracking-models) when the tracking model is missing and durable planning is needed. Read `.agent-prefs/program-goal-selection.md` when ranking depends on program-goal state, and use [../reference/agent-preferences.md](../reference/agent-preferences.md#program-goal-selection-models) when the program goal selection model is missing and the current state is unclear. Use [../reports/unfinished-work.md](../reports/unfinished-work.md) as an inventory of findings and candidate concerns, not as the active queue. If ranking depends on program-goal state, inspect the program-goal bucket model in [../program-goals/](../program-goals/) and identify whether a named bucket or `none/free-flow` applies according to the selected program-goal-selection model; do not invent a bucket just because one is missing. Check whether any short-term roadmap notes in the active program-goal file have already been implemented or captured in their normal source-of-truth layers; if so, retire, replace, or mark completed objectives before relying on them for ranking. Refresh with searches for `TODO`, `DEFERRED`, `pending`, `stub`, `placeholder`, and specs marked superseded or retained-for-intent. Rank candidates by the selected work tracking model, current program goal state when applicable, user intent, local severity, and unblock value. Before turning a report finding into substantial planning, implementation, or a multi-session handoff, route it through the selected work tracking model; under the default program-goal-buckets model, add or move the item to the relevant bucket first. Also read `.agent-prefs/session-execution-model.md` when it exists; if it prefers fresh workers or ask-each-time, ask whether to plan/execute here or prepare a reviewed handoff prompt for a fresh agent/thread.

**Output:** prioritized list of next candidates, with whether each is architecture, docs, tests, or code, the work tracking route for any selected item, the program goal state when it was used for ranking, any completed short-term objective that was removed or superseded, and the recommended execution route. If the recommended next unit is substantial and the local session execution model calls for fresh-worker control-room behavior, stop at the route question or produce the requested handoff prompt instead of silently planning in the current session.
...
**Workflow:** read the work tracking and drift guard in [../../AGENTS.md](../../AGENTS.md#work-tracking-and-drift-guard), read `.agent-prefs/work-tracking-model.md` when it exists, and read `.agent-prefs/program-goal-selection.md` when the current program goal state is unclear. Use [../reference/agent-preferences.md](../reference/agent-preferences.md#program-goal-selection-models) when the program goal selection model is missing and the drift review needs an explicit state. Inspect the program goal state model in [../program-goals/](../program-goals/) when the selected tracking model or user request involves program-goal buckets. Identify the current program goal state, recent short-term objectives, and the next proposed objective; decide whether the work is aligned, knowingly specialized, over-invested, under-invested elsewhere, free-flow by intent, or evidence that the program goal should be updated. When a short-term objective has been implemented, verify that its result still advances the program goal and remove or replace the completed objective in the goal file once the result is captured in reports, specs, maps, skills, glossary, constitution, or code. If multiple architects or domains are involved, treat each valid mid-term bucket separately instead of forcing all work into one global priority list.
...
**Workflow:** apply the work tracking and drift guard in [../../AGENTS.md](../../AGENTS.md#work-tracking-and-drift-guard) when a trigger is present; read `.agent-prefs/work-tracking-model.md` when durable planning needs a selected tracking route, and read `.agent-prefs/program-goal-selection.md` when program-goal state affects planning or is unclear. Use [../program-goals/](../program-goals/) as the goal registry and program goal state model only when the selected tracking model or user request involves program-goal buckets. Define goal, success criteria, in/out of scope, affected gates/docs/maps, tests, and review points; choose whether it is feature development or architecture development.

**Output:** a concise work-unit plan ready for `speckit-specify` or architecture review, including the work tracking route, any program milestone the work advances, and the reason it is not merely local polish.
