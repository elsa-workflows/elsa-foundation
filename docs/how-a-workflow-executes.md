...
point; every library under `src/essentials` is composed by this file or by a host you write yourself.
...
`src/essentials/Workflows/Runtime/Api/Endpoints/Executables/Execute/Endpoint.cs` declares the route with
...
`src/essentials/Workflows/Runtime/Api/Handlers/WorkflowExecutionStartService.cs` implements that service. It reads the
...
`src/essentials/Workflows/Runtime/Core/Models/WorkflowExecutionCommand.cs`.

### 6. Writing state: the checkpoint commit

Every handler that changes state produces a `RuntimeCheckpointCommit`
(`src/essentials/Workflows/Runtime/Core/Models/RuntimeCheckpointCommit.cs`): a named `RuntimeCheckpoint`, a
...
sweep runs. `WorkflowsRuntimeResumptionFeature` (`src/essentials/Workflows/Runtime/Resumption/WorkflowsRuntimeResumptionFeature.cs`)
...
- **Definition**: the named, logical workflow authored on the Design side; `WorkflowDefinition` in
  `src/essentials/Workflows/Design/Persistence/Core/Entities/WorkflowDefinition.cs` owns many drafts and many versions.
...
- **Draft**: the mutable authored state of one definition between versions
  (`IWorkflowDefinitionDraft` in `src/essentials/Workflows/Design/Core/Contracts`).
...
All of these models live under `src/essentials/Workflows/Runtime/Core/Models` unless a path says otherwise.
