...
| Escalation (escalation throw/end events fire-and-continue via seam C code `bpmn.escalation`; dormant code-matched boundaries on subProcess hosts, interrupting + non-interrupting + catch-all; consumer-side bubbling; late races pinned; root `<escalation>` interchange index; zero runtime changes, zero new state records) | `specs/127-bpmn-escalation` | #975 | merged |
| Event subprocesses tier 1 (`TriggeredByEvent` flow-less catchers; escalation triggers fully live — own-scope check + specificity ladder + `StopOtherLiveWork` extraction; scheduled-start seeding hint closes the body-seeding gap for all future triggers; error trigger = validated stated cut pending runtime deferred-seam-B fix #989; `ResolveTokenId` hardened for inline nested-body completions) | `specs/128-bpmn-event-subprocesses` | #992 | merged |
| Deferred seam-B fault-absorption fix (#989: fault-marker CommandMetadata leak onto Defer-derived work items — `WithoutFaultEvaluationMetadata` strips at both derivation sites; error event subprocesses UN-GATED; spec-120 error-boundary→task latent bug pinned green) | `specs/132-deferred-fault-absorption` | #996 | merged |
| Call activity (`callActivity` task-family element binding `Elsa.DispatchWorkflow`; waited execution rides shipped dispatch machinery; engine-owned failure-outcome ladder — error boundary > error event subprocess > per-outcome composite fault, no seam-B; interchange bound/unbound with `bpmn.calledElement` passthrough; seam-A cancellation reach = stated cut #998) | `specs/133-bpmn-call-activity` | #999 | merged |
| Event subprocesses tier 2 (message/signal/timer scope listeners: additive `BpmnToken.Kind` Listener|Activation discriminator + `BpmnElement.ListenerNodeId` second binding channel; liveness filter in exactly the two FinishEvaluation predicates; teardown-then-complete; non-interrupting re-arm gives timer repetition; two-phase arming) | `specs/134-bpmn-event-subprocess-tier2` | #1000 | merged |
| Message send surface (`Elsa.PublishEvent` durable-first primitive — publish-typed stager + `PublishStimulusIntentKind` post-commit handler → `RouteAsync(StartAndResume)`, dispatch path byte-identical; BPMN message throw/end as bound-child shapes; sendTask/receiveTask synthesis; correlation narrowing cut → #1001) | `specs/135-bpmn-message-send` | #1002 | merged |
| Collaboration/pool import (N-process documents via additive `BpmnImportResult.ProcessNodes` — silent multi-pool drop ended; `BpmnPool`/lanes populated; `BpmnMessageFlow` wiring metadata with loud name-mismatch degrades; per-definition participant-wrapper export; interchange-only, zero engine changes) — **PHASE 3 CONSTRUCT LIST COMPLETE** | `specs/136-bpmn-collaboration-import` | #1003 | merged |
