...
6. **Never mutate an existing catalog version in place.** Classification corrections for existing CLR activities belong in the compiler projection so same-version catalog hashes remain stable; authored contract changes require a new activity version. At Unit A delivery, the then-current executable shape remained readable and runtime stayed artifact-only; a corrected classification took effect through republish. That historical result is superseded by the pre-GA Groundwork clean baseline: every Runtime kind admits only its current version and fixture, with no Elsa compatibility upcasters. Executable, source-reference, and workflow-execution documents are version 4 and reject versions 1 through 3. Executable v4 includes the reusable-activity input contract and dependency snapshot; source-reference v4 includes tenant scope; workflow-execution v4 includes dispatch nesting depth. Upgrading an installation with any older Runtime persistence requires atomically resetting the complete Runtime and Publishing Groundwork persistence sets while preserving Design and Activities data, then republishing workflows before serving traffic. After a released shape exists, compatible in-place or rolling evolution may deliberately add Groundwork `IDocumentJsonUpcaster` contributions and retain supported historical fixtures; Groundwork's codec remains the generic version-policy engine.
