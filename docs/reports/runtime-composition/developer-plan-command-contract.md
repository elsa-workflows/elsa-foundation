...
Status: proposed command contract for [#1999](https://github.com/elsa-workflows/elsa-foundation/issues/1999), under [#1962](https://github.com/elsa-workflows/elsa-foundation/issues/1962). Source baseline: `47c3ff92641ec3063499d15f27f6b6328e5cf8e6`. This decides a first offline inspection task; it does not implement a command, publish profiles, or prove runtime readiness.
...
One focused implementation story can add the CLI front-end command, strict inventory and resource-hint adapters, additive dependency evidence in the shared planner result, a safe redacted plan projection, deterministic text/JSON renderers, and command tests for the examples above. It should reference the existing planner assembly and leave `persistence plan` and its worker untouched. A separate export/import story needs a lossless authored-document serializer, secret-reference policy, unknown-content round-trip tests, and comparison to the actual CShells output contract. A later host-check story needs trusted effective-persistence evidence, source-precedence/drift context, and no false runtime/EF-tooling parity claim. None of these follow-on stories should release the proposed Embedded, Authoring, or Worker planning fixtures as profiles without their remaining product and host gates.
