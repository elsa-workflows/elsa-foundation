...
| `WorkflowDesignContext` throws for both exposed members. | Incomplete design-context service / likely code drift | `WorkflowDesignContext` implements `IWorkflowDesignContext`, but no registration or factory implementation was found. Workflow design JavaScript contributors depend on `IWorkflowDesignContext`. | Design-time JavaScript declaration contributors can fail at DI or fail when reading draft/activity context unless an external host supplies the context. | Plan a focused Workflows Design JavaScript/context unit: decide who creates design context, how it is scoped, and whether contributors should depend on a factory instead of a direct context. |
...
2. Plan the Workflows Design JavaScript/context unit, because consumers depend on `IWorkflowDesignContext` but no local creation path is visible.
