...
1. Plan the Workflows Design JavaScript/context unit, because consumers depend on `IWorkflowDesignContext` but no local creation path is visible; preserve the ambient design-context idea as input, not as a settled decision.
