...
2. **Fresh-database schema admission applies a deterministic 922 DDL operations** for the reference SQLite
...
The trustworthy, reproducible evidence is (a) the deterministic 922-operation count and (b) the *phase ratios*
...
|---|---:|---|
| Fresh SQLite database (first request, `AutoApplyOnStartup=true`) | **922** | `ColdStartSchemaOperationCountTests` |
| Warm database (already admitted) | **0** | same test — idempotent restart |

922 = the current physicalized `CREATE`/index operation set for the 8 reference deployment families (Runtime, Secrets,
...
units and indexes, increasing the prior 873-operation snapshot by 49.
...
2. **Unit 3 — schema skip-if-current** removes the 922-operation admission on warm boots (the common
