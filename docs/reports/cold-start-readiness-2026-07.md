...
The trustworthy, reproducible evidence is (a) the deterministic 963-operation count and (b) the *phase ratios*
...
|---|---:|---|
| Fresh SQLite database (first request, `AutoApplyOnStartup=true`) | **963** | `ColdStartSchemaOperationCountTests` |
| Warm database (already admitted) | **0** | same test — idempotent restart |

963 = the current physicalized `CREATE`/index operation set for the 8 reference deployment families (Runtime, Secrets,
...
shape, increasing the immediately preceding 930-operation snapshot by 3. The preview.95→preview.102
additive-upgrade remediation then retained those historical index identities and added 30 versioned
replacement indexes for the reference composition, raising 933 to 963. The test derives the count from a
fresh SQLite target and keeps the warm restart at 0; this report does not infer wall-clock performance from
that structural delta.
...
2. **Unit 3 — schema skip-if-current** removes the 963-operation admission on warm boots (the common
