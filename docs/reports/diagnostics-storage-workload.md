...
| OpenTelemetry batch | Up to 64 normalized OTLP batches. Queue capacity is `max(SubscriberChannelCapacity, 64) * 4` (4,000 batches by default). On a provider that does not serialize writers, the batches of one drain pass are committed in groups of up to `CaptureRecordsPerCommit` records (4,096 by default), one provider transaction per group; on a provider that advertises Groundwork's `serialized-writer` capability (SQLite) every batch commits on its own so foreground commits never wait for a grouped transaction. Every OTLP batch keeps its own capture-ledger identity and fingerprint either way. |
