...
| Tenant isolation | Both contexts snapshot tenant identity and enforce tenant/resource separation before authorization. Design and runtime tests cover same-tenant, cross-tenant, and audit subject behavior. |
| Stable request profile | Profiles are derived from effective async decisions and cached through a thread-safe lazy request snapshot. Concurrent callers do not duplicate evaluator operations. |
...
| First-party caller migration | Activity Design management, authoring, dependency, fork, upgrade, diff, and recommendation callers, plus Workflow Runtime inspection, incident, instance, hierarchy, layout, and value-payload callers use async siblings. |
