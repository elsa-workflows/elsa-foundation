...
| H22 | `src/Elsa/Workflows/Runtime/Persistence/EntityFrameworkCore/**` and its focused tests | #1723/#1725 | Provider-neutral EF Core + Relational R01 bookmark state plus R04-R06 executable artifact/template/source-reference implementation; exact scoped identity, lossless projections, CAS save/delete, bounded ordinal keyset paging, authenticated continuations, rollback/restart, DI ownership and four-provider model/smoke proof; no migrations in this slice | | | | R01 and R04-R06 implementations and focused proof are prepared under #1723/#1725; R04-R06 are fresh-schema-only, with migration lifecycle and native-provider proof deferred; Groundwork remains default |
