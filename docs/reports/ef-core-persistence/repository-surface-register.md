...
| H13 | Secrets provider contexts `SecretsSqliteDbContext`, `SecretsSqlServerDbContext`, `SecretsPostgreSqlDbContext`, `SecretsMySqlDbContext` and base `SecretsDbContext` | #1679/#1678 | Correct provider/artifact pairing, context ownership/disposal and provider-neutral repository boundary | | | N/A | Existing opt-in; revalidation pending |
...
| H22 | `src/Elsa/Workflows/Runtime/Persistence/EntityFrameworkCore/**` and its focused tests | #1723 | Provider-neutral EF Core + Relational R01 bookmark state and raw stimulus-index implementation; exact scoped identity, lossless projections, CAS save/delete, bounded ordinal keyset paging, rollback/restart, DI ownership and four-provider model/smoke proof; no migrations in this slice | | | | Implementation and focused proof prepared under #1723, pending review/merge; Groundwork remains default |
