...
| H21 | `src/Elsa/Workflows/Runtime/Distributed/Persistence/EntityFrameworkCore/**` and its focused tests | #1717 | Provider-neutral EF Core + Relational D01 placement implementation; provider packages remain host/test-only; scoped CAS claim/renew/logical-release/list, durable fencing high-water marks, startup ownership validation, restart, SQLite behavior, and four-provider model/smoke proof; no migrations in this slice | #1718 | | | Opt-in implementation present; Groundwork remains default for transport and migration/default-flip/deletion gates remain pending |
