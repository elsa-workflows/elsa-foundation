...
Status: closed (2026-09-16) for Program [#1665](https://github.com/elsa-workflows/elsa-foundation/issues/1665), initially delivered by [#1671](https://github.com/elsa-workflows/elsa-foundation/issues/1671).

The `Open`/`Closed` column is the issue's state as of 2026-09-16 and goes stale as the tracker moves;
re-check it rather than trusting it. Rows marked `Closed (obsolete)` describe Groundwork or MongoDB
behavior that ceased to exist with #1764 and were closed as not planned, not fixed: if the same
behavior matters for EF Core it needs a fresh issue written against the EF implementation. Rows still
marked `Open` are general backlog that outlives this program; they constrained it but were never
owned by it.
...
| [#1657](https://github.com/elsa-workflows/elsa-foundation/issues/1657) Concurrent `dotnet-ef` BuildHost race | Closed | Safe concurrent tooling using current compiled artifacts | #1669 prerequisite | Preserve and prove |
...
| [#1668](https://github.com/elsa-workflows/elsa-foundation/issues/1668) Retire persistence performance measurement and preserve correctness evidence | Closed | Remove all performance infrastructure without losing timing-independent safety or correctness | #1668 with module owners | Active feature |
| [#1669](https://github.com/elsa-workflows/elsa-foundation/issues/1669) Four-provider migration lifecycle and isolation | Closed | Provider/artifact pairing, histories, runtime/out-of-process apply, locks, pending model, fresh install and failure-before-activation | #1669 after #1657 | Active spike |
...
| [#1674](https://github.com/elsa-workflows/elsa-foundation/issues/1674) Cross-module EF transaction topology | Closed | All transaction acceptance rows A01-A13 in the surface register | #1674 | Active spike |
...
| [#1676](https://github.com/elsa-workflows/elsa-foundation/issues/1676) Hard EF Runtime proving slice | Closed | Claims/leases/fencing/idempotency/checkpoint-outbox/crash/concurrency/SQLite contention | #1676 | Active spike |
...
| [#1678](https://github.com/elsa-workflows/elsa-foundation/issues/1678) General EF lifecycle policy and four-provider test kit | Closed | Reusable lifecycle without a universal domain abstraction | #1678 after spikes | Active feature |
| [#1679](https://github.com/elsa-workflows/elsa-foundation/issues/1679) Four-provider Secrets cutover | Closed | Add MySQL, revalidate three providers, flip default, remove adapter | #1679 after spikes | Active feature |
| [#1680](https://github.com/elsa-workflows/elsa-foundation/issues/1680) Studio Preferences EF | Closed | Scope/read/write/concurrency, migrations, four providers, host/e2e | #1680 after foundation | Active feature |
| [#1681](https://github.com/elsa-workflows/elsa-foundation/issues/1681) Diagnostics EF | Closed | Structured Logs and OpenTelemetry lifecycle, ordering, retention, failure and bounded-query behavior | #1681 after foundation | Active feature |
...
| [#1449](https://github.com/elsa-workflows/elsa-foundation/issues/1449) Shared cached sessions reject concurrent commands | Closed (obsolete) | Context/connection ownership, per-caller concurrency, lifetime/disposal and no leaks | #1674/#1676/#1678 | Preserve as spike acceptance |
...
| [#1526](https://github.com/elsa-workflows/elsa-foundation/issues/1526) Groundwork Runtime duplication/validation drift | Closed (obsolete) | Exact validation behavior must not silently diverge; Groundwork refactor itself becomes obsolete after deletion | #1672/#1670 | Supersede after EF guards prove parity |
...
| [#1297](https://github.com/elsa-workflows/elsa-foundation/issues/1297) Groundwork SQLite HTTP endpoint starts no workflow | Closed (obsolete) | Real HTTP route starts and persists workflow, returns correct response, survives restart | #1672/#1670; T37 and host e2e | Preserve |
...
| [#1422](https://github.com/elsa-workflows/elsa-foundation/issues/1422) Container-backed tests hang locally | Closed (obsolete) | Reliable, bounded Testcontainers lifecycle and cleanup for relational EF providers | #1678; Mongo portions retire | Preserve relational fixture requirement |
...
| [#1594](https://github.com/elsa-workflows/elsa-foundation/issues/1594) Typed native-plan evidence/parser deletion | Closed (obsolete) | Provider-neutral functional evidence may move to normal contract tests | #1668/#1670; native-plan admission and parser migration retire with performance policy | Retire after correctness audit |
| [#1611](https://github.com/elsa-workflows/elsa-foundation/issues/1611) Mongo diagnostics measurement child records no commands | Closed (obsolete) | Structured-log reopen/read/high-water correctness remains covered relationally | #1681; Mongo measurement/topology retires | Retire Mongo/measurement after mapping |
...
| [#1661](https://github.com/elsa-workflows/elsa-foundation/issues/1661) Workbench fails activation after first boot | Closed | Second-start payload/options/materialization and shell activation correctness | #1670/#1677; restart test | Preserve |
