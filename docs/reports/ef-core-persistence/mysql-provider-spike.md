...
| MySQL server requirement | MySQL **8.0 or later** | Stated in the official [Connector/NET EF Core requirements](https://dev.mysql.com/doc/connector-net/en/connector-net-entityframework-core.html). This is the provider's documented floor, not a floor this repository has exercised: Elsa's scripted-artifact path has only been run against the digest-pinned `mysql:8.4.11` image the MySQL suites use (see the pinned test image row below and `tools/ef/README.md`). |
...
| 5 | Collations and normalized keys | `Binary_collation_and_Elsa_normalized_keys_preserve_ordinal_identity_and_unicode_equivalence` starts from a conflicting `utf8mb4_0900_ai_ci` database default, verifies seven live columns use NO PAD `utf8mb4_0900_bin`, stores ASCII-, Unicode-, and trailing-space-distinct identities, and exercises ASCII/Unicode normalization. | **Passed** |
...
| Tables and keys | `elsa_secrets`; composite primary key `(TenantId, NormalizedName)`; index `IX_elsa_secrets_tenantId_status_normalizedName`; seven identity/lookup columns verified live as NO PAD `utf8mb4_0900_bin` despite a conflicting database default. Case-, Unicode-, and trailing-space-distinct primary-key values coexist. | Verified |
...
- Oracle-specific `ForMySQLHasCollation` metadata is required for the migration SQL to emit explicit NO PAD `COLLATE utf8mb4_0900_bin`; legacy `utf8mb4_bin` is PAD SPACE and the database default is case-insensitive, so neither preserves Elsa's ordinal identity contract.
