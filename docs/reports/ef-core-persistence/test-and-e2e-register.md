...
| T66 | `tests/Elsa/Workflows/Runtime/Persistence/EntityFrameworkCore/ProviderTests/Elsa.Workflows.Runtime.Persistence.EntityFrameworkCore.ProviderTests.csproj` | #1723 | Four-provider model plus live PostgreSQL/SQL Server/MySQL CRUD, keyset query, transaction rollback, optimistic concurrency and reopen durability; no hidden skips under the required-provider gate | PostgreSQL/SQL Server/MySQL focused provider smoke; implementation prepared under #1723 | | |
