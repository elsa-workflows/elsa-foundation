...
| ID | Groundwork unit / physical name | Domain contract or semantic role | Evidence | Blockers | Replacement PR | Default-flip PR | Deletion PR | Disposition |
|---|---|---|---|---|---|---|---|---|
| W01 | `workflowDefinition` / `elsa_workflow_definitions_v2` | `IWorkflowDefinitionStore`; definition read authority | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store and focused SQLite scope/query proof are prepared; native-provider smoke, migrations, and default flip remain pending |
| W02 | `workflowDefinitionVersion` / Groundwork v2: `elsa_workflow_definition_versions_v2`; EF: `elsa_workflow_definition_versions` | `IWorkflowDefinitionVersionStore` | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store and focused SQLite version/order proof are prepared; native-provider smoke, migrations, and default flip remain pending |
| W03 | `workflowDefinitionDraft` / Groundwork v2: `elsa_workflow_definition_drafts_v2`; EF: `elsa_workflow_definition_drafts` | `IWorkflowDefinitionDraftStore` and draft commands | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store/commands and focused SQLite lifecycle/rollback proof are prepared; native-provider smoke, migrations, and default flip remain pending |
| W04 | `workflowDefinitionVersionLayout` / `elsa_workflow_definition_version_layouts` | `IWorkflowDefinitionVersionLayoutStore` | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store and focused SQLite write-once/layout proof are prepared; native-provider smoke, migrations, and default flip remain pending |
| W05 | `workflowDesignOperation` / `elsa_design_operations` | Idempotent design-operation ledger and atomic commands | E-CLAIM | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF operation ledger and focused SQLite replay/conflict/reconciliation proof are prepared; native-provider smoke, migrations, and default flip remain pending |
