...
| R07 | `activityExecutionState` / `runtime_activity_execution_state` | `IActivityExecutionStateStore` | E-RUNTIME | S1-S4 | | | | EF implementation is complete locally with focused SQLite proof and native SQL Server/PostgreSQL/MySQL smoke for CRUD, bounded ordering/cursors, tenant/execution-scope isolation, parent queries, create-only writes, and optimistic concurrency; review/merge, migration lifecycle, default flip, and deletion remain pending |
| R08 | `activityExecutionInspection` / `runtime_activity_execution_inspection` | `IActivityExecutionInspectionStore`, writer | E-RUNTIME | S1-S4 | | | | EF implementation is complete locally with focused SQLite proof and native SQL Server/PostgreSQL/MySQL smoke for inspection writes, summaries, bounded deterministic cursors, tenant/execution-scope isolation, create-only writes, and optimistic concurrency; review/merge, migration lifecycle, default flip, and deletion remain pending |
| R09 | `activityExecutionHierarchy` / `runtime_activity_execution_hierarchy` | `IActivityExecutionHierarchyStore`, reader, writer | E-RUNTIME | S1-S4 | | | | EF implementation is complete locally with focused SQLite proof and native SQL Server/PostgreSQL/MySQL smoke for hierarchy projection, root/watermark/cursor binding, descendants, boundary and attempt navigation, tenant/execution-scope isolation, and optimistic concurrency; review/merge, migration lifecycle, default flip, and deletion remain pending |
