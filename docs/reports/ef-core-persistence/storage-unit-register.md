...
| A11 | `activityDependencyProjection` / `elsa_activity_dependency_projection` | Dependency projection and reconciliation | E-DESIGN | S1-S3 | | | | Opt-in EF implementation tracked by issue #1731; focused SQLite and live 6/6 native-provider smoke. Incremental upgrade updates are staged in the upgrade's own transaction, and `EfActivityDependencyProjectionRebuildCoordinator` converges the view after an ordinary Design edit under an explicit privileged-across-scopes context. Groundwork remains default |
| A12 | `activityUpgradePlan` / `elsa_activity_upgrade_plans` | `IActivityUpgradePlanStore`; cross-design upgrade staging | E-PUBLISH | S1-S3 | | | | Opt-in EF implementation tracked by issue #1731; focused SQLite and live 6/6 native-provider smoke. Its Publishing-side bridge — `IActivityUpgradeDiscoverySource`, `IActivityUpgradePlanMutationStore`, `IActivityUpgradePublishedDraftResolver` — is `EfActivityUpgradePlanStore`, which commits Activities Design and Workflows Design through one `EfSharedTransaction`; focused SQLite plus a live 3/3 apply-and-rollback smoke. Groundwork remains default |
| A13 | `activityUpgradeApplyReceipt` / `elsa_activity_upgrade_apply_receipts` | Idempotent upgrade-apply receipt | E-PUBLISH | S1-S3 | | | | Opt-in EF implementation tracked by issue #1731; focused SQLite and live 6/6 native-provider smoke. The receipt's Applied transition now commits inside the EF upgrade apply, so a rejected apply leaves it Preparing and writes nothing; covered by the Publishing EF idempotency and rollback suites. Groundwork remains default |
...
| ID | Groundwork unit / physical name | Domain contract or semantic role | Evidence | Blockers | Replacement PR | Default-flip PR | Deletion PR | Disposition |
|---|---|---|---|---|---|---|---|---|
| W01 | `workflowDefinition` / `elsa_workflow_definitions_v2` | `IWorkflowDefinitionStore`; definition read authority | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store and focused SQLite scope/query proof plus native-provider smoke (3/3 passed) are prepared; migrations and default flip remain pending |
| W02 | `workflowDefinitionVersion` / Groundwork v2: `elsa_workflow_definition_versions_v2`; EF: `elsa_workflow_definition_versions` | `IWorkflowDefinitionVersionStore` | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store and focused SQLite version/order proof plus native-provider smoke (3/3 passed) are prepared; migrations and default flip remain pending |
| W03 | `workflowDefinitionDraft` / Groundwork v2: `elsa_workflow_definition_drafts_v2`; EF: `elsa_workflow_definition_drafts` | `IWorkflowDefinitionDraftStore` and draft commands | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store/commands and focused SQLite lifecycle/rollback proof plus native-provider smoke (3/3 passed) are prepared; migrations and default flip remain pending |
| W04 | `workflowDefinitionVersionLayout` / `elsa_workflow_definition_version_layouts` | `IWorkflowDefinitionVersionLayoutStore` | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store and focused SQLite write-once/layout proof plus native-provider smoke (3/3 passed) are prepared; migrations and default flip remain pending |
| W05 | `workflowDesignOperation` / `elsa_design_operations` | Idempotent design-operation ledger and atomic commands | E-CLAIM | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF operation ledger and focused SQLite replay/conflict/reconciliation proof plus native-provider smoke (3/3 passed) are prepared; migrations and default flip remain pending |
