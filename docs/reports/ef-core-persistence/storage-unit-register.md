...
| R20 | `postCommitOutbox` / `runtime_post_commit_outbox` | Runtime outbox store, lookup, claim, completion, and redrive | E-CLAIM | S1-S4 | | | | Thin EF outbox slice committed locally on `codex/ef-runtime-r19-r24`: create-only pending replay/conflict handling, scope isolation, bounded deterministic deliverable queries, claims/reclaims with fencing and stale-owner protection, retry/finalization, outbox-only atomic completion, and explicit fail-closed dispatch/redrive until R21; focused SQLite proof 10/10 and native PostgreSQL/SQL Server/MySQL smoke 3/3. Candidate-row projection drift fails closed; a tampered eligibility value that hides a row cannot be detected without an unbounded/provider-specific scan and remains an explicit repair concern. Complete R20/R21 composition remains pending; preview registration does not replace runtime contracts and Groundwork remains default |
