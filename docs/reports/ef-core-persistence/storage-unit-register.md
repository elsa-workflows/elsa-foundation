...
| W02 | `workflowDefinitionVersion` / `elsa_workflow_definition_versions_v2` | `IWorkflowDefinitionVersionStore` | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store and focused SQLite version/order proof are prepared; native-provider smoke, migrations, and default flip remain pending |
| W03 | `workflowDefinitionDraft` / `elsa_workflow_definition_drafts_v2` | `IWorkflowDefinitionDraftStore` and draft commands | E-DESIGN | S1-S3 | #1727 (opt-in EF) | | | Opt-in EF store/commands and focused SQLite lifecycle/rollback proof are prepared; native-provider smoke, migrations, and default flip remain pending |
