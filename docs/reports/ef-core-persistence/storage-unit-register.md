...
| R04 | `executableActivityTemplate` / `runtime_executable_activity_template` | `IExecutableActivityTemplateStore`, reader, writer | E-RUNTIME | S1-S4 | | | | EF parity wave implemented on the current branch for fresh schemas only; focused SQLite proof is green; migration lifecycle, native-provider proof and default flip remain pending |
| R05 | `executableActivityTemplateHashClaim` / `runtime_executable_activity_template_hash_claim` | Unique template-hash claim used by R04 | E-CLAIM | S1-S4 | | | | EF parity wave implemented on the current branch for fresh schemas only; focused SQLite proof is green; migration lifecycle, native-provider proof and default flip remain pending |
