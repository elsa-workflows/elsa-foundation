...
Adopt **coexistence with explicit boundaries**:

1. Keep the existing ASP.NET Core Minimal API implementation for root CShells/Nuplane management
   endpoints. It meets the requirements actually used by that surface; replacing it with
   FastEndpoints would add framework coupling without demonstrated value.
2. Do not migrate Elsa's existing feature APIs merely to standardize endpoint authoring.
   FastEndpoints remains acceptable for currently static, non-collectible Elsa modules while their
   contracts are migrated onto shared ASP.NET Core authorization policies.
3. Make Foundation Identity's policy provider, permission evaluator, catalog contributions, and
   resource handlers the single authorization path for every first-party endpoint adapter.
   Endpoint frameworks declare requirements; they do not own permission semantics.
4. Treat dynamically unloadable endpoint assemblies as **not FastEndpoints-compatible until
   collectible-context retention is resolved and guarded by a repeatable test**. The spike observed
   FastEndpoints retaining all three collectible contexts.
5. Replace CShells' remove-then-add route publication with a validated candidate manifest and one
...
## Minimal API parity for shell management
...
The relevant comparison is therefore the management surface's actual needs, not the union of every
feature FastEndpoints offers:
...
| Problem details | Elsa uses a FastEndpoints ProblemDetails configurator and several typed problem models. | ASP.NET Core `AddProblemDetails`, `Results.Problem`, and typed results cover root management needs. | No blocker for management; feature APIs keep existing behavior. |
| OpenAPI | Twenty-seven first-party source files contain summary, description, Swagger, or OpenAPI declarations. | Route groups support `WithOpenApi`, `Produces`, descriptions, tags, and operation transformers. | No blocker; add only metadata the management client consumes. |
...
| Discovery | Assembly scanning is central to the current Elsa FastEndpoints modules. Root management routes are six explicit mappings. | Explicit mapping is preferable for this small host-owned surface. | Keep discovery inside feature adapters, not the CShells boundary. |
...
This closes the endpoint-authoring question for the current scope: **keep Minimal APIs for root
management; coexist with feature adapters**.
...
surface was found in `src/Elsa`, `src/Elsa3`, or `src/Apps`.
...
The prototype registered Foundation Identity's real dynamic policy provider, permission
authorization handler, composite catalog, and a replaceable evaluator. A Minimal API endpoint used
...
static root and does not attribute it to CShells. It is nevertheless sufficient to reject the
assumption that a FastEndpoints feature assembly is collectible today. Follow-up work should
produce a minimal upstream reproducer, identify/reset the static cache or redesign discovery, and
retain this weak-reference test as a release gate.
...
|---:|---|---|---|
| 1 | Actual FastEndpoints use compared with Minimal APIs | Complete | Root management ready for Minimal APIs. |
...
| 4 | Representative adapters use one evaluator | Complete for Minimal API + FastEndpoints; MVC N/A | Not implemented in first-party endpoints. |
...
| 8 | Management Minimal APIs coexist with feature FastEndpoints | Executable proof complete | Supported. |
...
| 16 | Repeated collectible-context evidence | Complete: 0/3 collected | FastEndpoints retention blocks unloadability. |
| 17 | Keep/migrate/coexist recommendation and follow-ups | Complete | Review gate pending. |
...
1. **Shared permission policy contract**
   ...
   - Add permanent Minimal API and FastEndpoints adapter contract tests.

2. **Elsa endpoint-base migration**
   - Change the six Elsa FastEndpoints bases from direct `Permissions(...)` matching to the shared
     policy contract without changing public routes.
   - Preserve existing wildcard and action-scoped permission behavior during the transition.

3. **Complete permission ownership and endpoint disposition inventory**
...
4. **Runtime endpoint authorization guard**
   ...
   - Add a separate publication-time rule for workflow-authored HTTP endpoints.

5. **Atomic CShells endpoint publication**
...
6. **FastEndpoints collectible-context investigation**
   - Publish the 0/3 minimal reproducer upstream or in CShells.
   - Identify framework-held roots and establish whether a supported reset/discovery boundary is
     possible.
   - Until green, forbid FastEndpoints for assemblies promised to be dynamically unloadable.

7. **Architecture decision record**
   - After review, record endpoint-framework scope, authorization ownership, route ownership,
     collision policy, public endpoint classification, and the collectible-framework constraint.

The spike itself remains `none/free-flow`. If review approves units 1–6 as a coordinated migration
program, move them under Feature Composition Readiness (or create a narrower security/composition
program) rather than silently expanding this issue.
