...
|---:|---|---|---|---|
| 1 | Codebase reality / test maturity follow-up | Elsa Brain Operating Model: codebase reality checks | Docs, maps, skills, glossary, and program-goal routing are now balanced enough that the next high-value unit should reconnect the brain to implementation reality. | Use [NotImplemented classification](notimplemented-classification.md) to select a focused fix or planning unit that belongs to the current program bucket. Do not pull the Workflows Design JavaScript/context deferral into this bucket; it is safeguarded by source comments and the classification report until full design-suite implementation starts. |
