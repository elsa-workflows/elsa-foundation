...
| Typed argument model — studio (Phase 2) + two backend follow-ups | Backend Phase 1 landed (`specs/081-typed-argument-model/`, PR #330): `TypeReference {Alias, CollectionKind}`, descriptor catalog + `descriptors/variables` endpoint, fail-fast registry. Open: (a) studio must consume the wire contract (`contracts/wire-contract.md`) — type dropdown, collection-kind dropdown, type-aware default editor, editable name/description; (b) `ElsaWorkflowManagementApi.GetUiSpecifications` enum-option inference was dropped (alias-only model can't `LoadType`); (c) runtime persistence/resolution of workflow variables/inputs at execution is a separate gap (see `runtime-variables-inputs-not-persisted`). | Keep `none/free-flow` until the studio Phase-2 effort is planned; restore enum default-editor options via the descriptor catalog (`defaultEditor`/metadata) when Phase 2 needs them; track runtime persistence under its own unit |
