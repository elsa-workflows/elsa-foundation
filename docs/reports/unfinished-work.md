...
|---:|---|---|---|---|
| 1 | Codebase reality / test maturity follow-up | Elsa Brain Operating Model: codebase reality checks | Docs, maps, skills, glossary, and program-goal routing are now balanced enough that the next high-value unit should reconnect the brain to implementation reality. | Use [NotImplemented classification](notimplemented-classification.md) to select a focused fix or planning unit; next best candidate is `VariableExpressionDescriptor` or Workflows Design JavaScript/context. |
...
| NotImplemented/code placeholder classification is complete | [NotImplemented classification](notimplemented-classification.md) separates intentional runtime deferrals, active expression code drift, incomplete design context, unregistered HTTP placeholder, JavaScript endpoint fake context, and test-only stubs | Pick a focused implementation or planning unit; prefer `VariableExpressionDescriptor` fix/defer decision before broader runtime work |
