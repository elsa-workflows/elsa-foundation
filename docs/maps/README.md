...
- [Dependency map](dependency-map.json) - the canonical project-graph dataset (spec 149) the project-graph maps below are projections of; see [Dependency map dataset](#dependency-map-dataset).
...
`dependency-map.json` is the one scan of the project files. The project-reference, package, test, domain,
architecture-reference and feature-dependency maps take every project and package fact from it rather than reading
`.csproj` files themselves, and publishing reads it (spec 150). The `dependency-map` layer writes it; `-- all`, the
v1 refresh below and the freshness check all include it.

- `schema_version` changes when the shape changes in a way a consumer could misread. A consumer checks it when it reads the file; generation does not gate on it.
- `nodes` holds one node per project under `src/` and `tests/`, ordered by `path`, the node's identity. Each records `name`, `kind` (`source` or `test`), `domain`, `sub_domain`, `role`, `declared_target_framework` (null when inherited), and `packable`, with MSBuild's defaults applied. `package_id` and `line` (`A` or `B`, from `VersionLines.props`) are set on packable nodes and null on every other node.
- Each node's `edges` are typed. An `internal` edge is a `ProjectReference`, with the target's package `id` and project `path`; that path can name a project outside `src/` and `tests/` that has no node. An `external` edge is a `PackageReference`, with the package `id` and the `version` that central package management declares.
- A file belongs to the node whose directory is the longest prefix of its path. When no node's directory contains the file, it has no owner.
- It holds no publish state and no list of source files, so it changes only when the project graph does.

## Refresh Maps v1

The v1 refresh also rewrites the dependency map, because the v1 maps are projections of it.
...
- It deliberately does not gate on a fingerprint over the inputs. Such a fingerprint moves on every source edit, so it would oblige every code PR to regenerate and commit thirteen map files even though most source edits change no map at all.
