...
Narrowed on 2026-09-22 (Decision record, "U5 delivered"): when the host's package closure selects an
engine that is not `Sqlite` (`Nuplane:Capabilities:ef-provider`), the D4 provider-disagreement refusal
— exit 3 — comes first, because such a host was never a SQLite host and the idempotency rule would
answer a question the operator should not have asked. With no selection configured, or one containing
`Sqlite`, this refusal is unchanged.
