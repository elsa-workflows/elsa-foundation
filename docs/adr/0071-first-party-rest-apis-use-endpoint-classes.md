...
- **Reflective module-local scanning is the current registration path.** A source generator that
  emits per-assembly registration — removing the scan from the default path entirely — is a planned
  follow-up, not something any composition root uses today; until it lands there is one path, and
  once it lands there are two while it rolls out.
...
- `OpenApiLifetimeBoundaryTests` — 25 facts and a theory covering nested, private, generic-argument,
