<#
.SYNOPSIS
    Incident querying: poisoned scheduler work records a blocking incident that is retrievable and counted.
.DESCRIPTION
    Poisons scheduler work (a WriteLine whose `text` is a JavaScript binding that throws at evaluation), then asserts:
      - the non-terminal instance remains `Running` and its summary `incidentCount` is 1;
      - `GET runtime/workflows/instances/{id}/incidents` returns `{ workflowExists, incidents:[...], count }`
        with one incident that is `Blocking`, has a system-authored `WaitForIntervention` resolution outcome,
        and carries a `failureType` + `message`.
...
Write-Host "== Incident query (poisoned scheduler work) ==  -> $BaseUrl" -ForegroundColor Cyan
...
if ($one) { Write-Host ("incident: severity={0} status={1} resolution={2} failureType={3}" -f $one.severity, $one.status, $one.resolutionOutcome.actionKind, $one.failureType) }

$fail = @()
if ($inst.instance.status -ne 'Running') { $fail += "instance not Running (got $($inst.instance.status))" }
...
if ($one) {
    ...
    if ($one.resolutionOutcome.actionKind -ne 'WaitForIntervention') { $fail += "resolutionOutcome.actionKind=$($one.resolutionOutcome.actionKind) (expected WaitForIntervention)" }
    if ($one.resolutionOutcome.systemSource -ne 'PoisonedSchedulerWork') { $fail += "resolutionOutcome.systemSource=$($one.resolutionOutcome.systemSource) (expected PoisonedSchedulerWork)" }
...
Write-Host ""
if ($fail.Count -eq 0) {
    Write-Host "SUCCESS - poisoned scheduler work recorded a blocking incident, retrievable and counted correctly." -ForegroundColor Green
} else {
