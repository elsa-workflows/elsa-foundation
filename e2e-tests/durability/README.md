...
| `Test-RestartRecovery.ps1` | Set a variable, suspend, then **kill the `Elsa.Workbench` process and relaunch it** against the same SQLite DB. Asserts the instance is still `Suspended` with identical state (the pre-suspend node ran **exactly once** — no duplicate replay), then resumes it to completion with the variable intact. The full black-box durability proof. |
