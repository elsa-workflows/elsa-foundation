...
internal sealed class SqlServerDesignSchemaEvolutionFixture : IDesignSchemaEvolutionFixture
{
    private readonly string _connectionString;
    private ServiceProvider? _host;

    private SqlServerDesignSchemaEvolutionFixture(string connectionString) => _connectionString = connectionString;

    public string Provider => "groundwork-sqlserver";

    public static async Task<SqlServerDesignSchemaEvolutionFixture> CreateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var fixture = new SqlServerDesignSchemaEvolutionFixture(connectionString);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var executor = new InterruptingPhysicalSchemaExecutor(new SqlServerPhysicalSchemaExecutor(_connectionString), failAfterOperations);
        try
        {
    ...
    public async Task<DesignEvolutionDriftReport> PerturbAppliedObjectAndInspectAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using (var connection = new SqlConnection(_connectionString))
        {
            ...
            {
                find.CommandText =
                    "SELECT TOP 1 i.name AS indexname, t.name AS tablename FROM sys.indexes i " +
                    "JOIN sys.tables t ON i.object_id = t.object_id " +
                    "WHERE i.is_primary_key = 0 AND i.is_unique_constraint = 0 AND i.name IS NOT NULL " +
                    "ORDER BY LEN(i.name) DESC, i.name";
                await using var reader = await find.ExecuteReaderAsync(cancellationToken);
                if (!await reader.ReadAsync(cancellationToken))
                    throw new InvalidOperationException("No applied design index was found to drift.");
                droppedIndex = reader.GetString(0);
                droppedTable = reader.GetString(1);
            }

            await using var drop = connection.CreateCommand();
            drop.CommandText = $"DROP INDEX [{droppedIndex}] ON [{droppedTable}]";
    ...
    private ServiceProvider BuildHost(bool evolved)
    {
        ...
        if (evolved)
            services.AddGroundworkSqlServerUnifiedPersistence<SqlServerDesignEvolutionDeploymentSchema>(_connectionString, autoApplyOnStartup: false);
        else
    ...
    private ServiceProvider BuildCompositionServices(bool evolved = false, bool collision = false)
    {
        ...
        services.AddSingleton(typeof(ILogger<>), typeof(NullLogger<>));
        if (collision)
            services.AddGroundworkStorageComposition<SqlServerDesignEvolutionCollisionDeploymentSchema>();
        else if (evolved)
            services.AddGroundworkStorageComposition<SqlServerDesignEvolutionDeploymentSchema>();
        else
...
internal sealed class InterruptingPhysicalSchemaExecutor(
    SqlServerPhysicalSchemaExecutor inner,
