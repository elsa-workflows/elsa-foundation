...
internal sealed class MongoDbDesignPersistenceContractFixture : IDesignPersistenceContractFixture
{
    ...
    private MongoDbDesignPersistenceContractFixture(
    ...
    public string Provider => "groundwork-mongodb";
    public int RestartCount { get; private set; }
    public int BoundScopeCount { get; private set; }
    public GroundworkSchemaEvidence SchemaEvidence { get; private set; } = null!;

    /// <summary>The shared replica-set connection string, for leaf-local plan capture.</summary>
    internal string MongoDbConnectionString => _connectionString;

    /// <summary>The applied per-fixture isolated database name, for leaf-local plan capture.</summary>
    internal string MongoDbDatabaseName => _databaseName;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var fixture = new MongoDbDesignPersistenceContractFixture(telemetry, connectionString, databaseName);
        fixture.SchemaEvidence = await GroundworkSchemaCli.ApplyFreshAsync(connectionString, databaseName, cancellationToken);
    ...
    private static SaveDocumentRequest DefinitionSave(AtomicityDocumentIdentities identities, string storageScope)
    {
        var definition = new WorkflowDefinition
        {
            ...
            Description = "MongoDB design-conformance atomicity probe",
...
internal sealed class GroundworkTargetAtomicityFaultController
{
    ...
    public IDesignAtomicityFaultLease Arm(DesignAtomicityFaultPlan plan, CancellationToken cancellationToken)
    {
        ...
        if (_armed is not null)
            throw new InvalidOperationException("Only one MongoDB atomicity fault may be armed at a time.");
        ...
        GroundworkTargetAtomicityOperationCancellation operation)
    {
        ...
        switch (fault.Plan.Action)
        {
            case DesignAtomicityFaultAction.Throw:
                throw new InvalidOperationException($"Injected MongoDB atomicity fault at '{phase}'.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        switch (fault.Plan.Action)
        {
            ...
            case DesignAtomicityFaultAction.Cancel:
                ...
                return false;
            case DesignAtomicityFaultAction.Throw:
                throw new InvalidOperationException("Injected MongoDB atomicity provider-decision fault.");
            default:
                throw new ArgumentOutOfRangeException(nameof(fault.Plan), fault.Plan.Action, null);
        }
    }
...
{
    private const string ConnectionEnvironmentVariable = "ELSA_DESIGN_GROUNDWORK_MONGODB_CONNECTION";
    ...
    public static async Task<GroundworkSchemaEvidence> ApplyFreshAsync(
        ...
        CancellationToken cancellationToken)
    {
        var offline = await RunAsync("validate", connectionString: null, databaseName: null, cancellationToken, "--offline");
        if (offline.ExitCode != 0)
            throw offline.ToException("Groundwork offline schema validation failed");

        var plan = await RunAsync("plan", connectionString, databaseName, cancellationToken);
        if (plan.ExitCode != 2)
            throw plan.ToException("A fresh Groundwork MongoDB target did not report a pending plan");

        var apply = await RunAsync("apply", connectionString, databaseName, cancellationToken, "--safe");
        if (apply.ExitCode != 0)
        ...
        params string[] extraArguments)
    {
        ...
        // MongoDB isolates one shared server by database; the tool takes the database name explicitly
        // because it does not ride the shared replica-set connection URI.
        if (databaseName is not null)
        {
            start.ArgumentList.Add("--database");
            start.ArgumentList.Add(databaseName);
        }
