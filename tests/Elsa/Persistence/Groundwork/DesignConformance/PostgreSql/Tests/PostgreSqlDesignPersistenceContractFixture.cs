...
internal sealed class PostgreSqlDesignPersistenceContractFixture : IDesignPersistenceContractFixture
{
    ...
    private PostgreSqlDesignPersistenceContractFixture(GroundworkBaselineTelemetry telemetry, string connectionString)
    {
    ...
    internal string PostgreSqlConnectionString => _connectionString;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var fixture = new PostgreSqlDesignPersistenceContractFixture(telemetry, connectionString);
    ...
    private static SaveDocumentRequest DefinitionSave(AtomicityDocumentIdentities identities, string storageScope)
    {
        var definition = new WorkflowDefinition
        {
            ...
            Description = "PostgreSQL design-conformance atomicity probe",
...
internal sealed class GroundworkTargetAtomicityFaultController
{
    ...
    public IDesignAtomicityFaultLease Arm(DesignAtomicityFaultPlan plan, CancellationToken cancellationToken)
    {
        ...
        if (_armed is not null)
            throw new InvalidOperationException("Only one PostgreSQL atomicity fault may be armed at a time.");
        ...
        GroundworkTargetAtomicityOperationCancellation operation)
    {
        ...
        switch (fault.Plan.Action)
        {
            case DesignAtomicityFaultAction.Throw:
                throw new InvalidOperationException($"Injected PostgreSQL atomicity fault at '{phase}'.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        switch (fault.Plan.Action)
        {
            ...
            case DesignAtomicityFaultAction.Cancel:
                ...
                return false;
            case DesignAtomicityFaultAction.Throw:
                throw new InvalidOperationException("Injected PostgreSQL atomicity provider-decision fault.");
            default:
                throw new ArgumentOutOfRangeException(nameof(fault.Plan), fault.Plan.Action, null);
        }
    }
...
{
    private const string ConnectionEnvironmentVariable = "ELSA_DESIGN_GROUNDWORK_POSTGRESQL_CONNECTION";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (plan.ExitCode != 2)
            throw plan.ToException("A fresh Groundwork PostgreSQL target did not report a pending plan");
