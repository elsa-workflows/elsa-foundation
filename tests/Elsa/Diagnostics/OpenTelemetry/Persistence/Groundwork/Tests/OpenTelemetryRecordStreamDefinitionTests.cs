...
public sealed class OpenTelemetryRecordStreamDefinitionTests
{
    ...
    public void Every_immutable_signal_has_a_valid_bounded_distinct_stream_definition()
    {
        ...
        foreach (var definition in definitions)
        {
            ...
            Assert.All(
                definition.Fields.Where(x => x.Type == DiagnosticFieldType.String),
                field => Assert.Equal(DiagnosticStringCasePolicy.Ordinal, field.CasePolicy));
        }
    }

    [Fact]
    public void Definitions_declare_the_portable_ordinal_range_order_membership_and_latest_query_contract()
    {
        ...
        Assert.Equal(DiagnosticStringCasePolicy.Ordinal, traceId.CasePolicy);
