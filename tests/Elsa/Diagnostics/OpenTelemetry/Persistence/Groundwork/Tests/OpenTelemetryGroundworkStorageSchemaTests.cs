...
{
    [Fact]
    public void Schema_declares_explicit_streams_physical_catalog_entities_indexes_and_capture_ledger()
    {
        ...
        Assert.All(streams, stream => Assert.InRange(stream.Stream.Value.Length, 1, 64));
        Assert.All(streams, stream => Assert.Equal(128, stream.Limits.MaxRecordIdBytes));
        Assert.Equal(
            new[]
            {
        ...
        Assert.Equal(PhysicalStorageForm.PhysicalEntityTable, ExplicitDefinition(CatalogDocuments.ResourceKind).Form);
        Assert.Equal(PhysicalStorageForm.PhysicalEntityTable, ExplicitDefinition(CatalogDocuments.InstrumentKind).Form);
        Assert.Equal(PhysicalStorageForm.DedicatedDocumentTable,
            ExplicitDefinition(OpenTelemetryGroundworkStorageSchema.OperationLedgerKind).Form);
        ...
        PhysicalTableDefinition ExplicitDefinition(string kind) => Assert.IsType<PhysicalStoragePolicy.ExplicitPolicy>(
            manifest.StorageUnits.Single(unit => unit.Identity.Value == kind).PhysicalStorage!.Policy).Definition;
    }
