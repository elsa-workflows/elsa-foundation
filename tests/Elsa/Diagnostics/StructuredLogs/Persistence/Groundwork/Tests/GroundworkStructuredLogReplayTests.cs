...
public abstract class GroundworkStructuredLogStoreTestBase : IAsyncLifetime
{
    ...
    {
        var provider = await CreateProviderAsync(binding);
        return Start(new(provider, Options.Create(new StructuredLogsOptions()), binding));
    }

    protected static GroundworkStructuredLogStore Start(GroundworkStructuredLogStore store)
    {
        store.Start();
        return store;
    }
