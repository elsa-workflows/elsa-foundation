...
/// <summary>
/// Covers <see cref="GroundworkOpenIddictTokenStore"/>: CRUD/CAS, every scalar accessor, the two declared
/// lookups (reference-id point, subject cursor-paged), the rejected shapes (compound/application-id/
/// authorization-id/count-all/list-all/prune), and the revoke family's exactly-once behaviour through
/// <see cref="OpenIddictGroundworkAtomicWrite"/>.
/// </summary>
public sealed class GroundworkOpenIddictTokenStoreTests
{
    private readonly FakeGroundworkDocumentStore _documentStore = new();
    private readonly GroundworkOpenIddictTokenStore _store;

    public GroundworkOpenIddictTokenStoreTests()
    {
        _store = new GroundworkOpenIddictTokenStore(_documentStore, AtomicWrite(_documentStore), _documentStore);
    }

    [Fact]
    public async Task Create_then_find_by_id_round_trips_the_token()
    {
        var token = new OpenIddictGroundworkToken { Subject = "bob" };

        await _store.CreateAsync(token, CancellationToken.None);
        var found = await _store.FindByIdAsync(token.Id, CancellationToken.None);
    ...
    public async Task Scalar_accessors_round_trip_through_get_and_set()
    {
        ...
        var expirationDate = new DateTimeOffset(2026, 2, 1, 0, 0, 0, TimeSpan.Zero);
        var redemptionDate = new DateTimeOffset(2026, 1, 15, 0, 0, 0, TimeSpan.Zero);

        await _store.SetApplicationIdAsync(token, "app-1", CancellationToken.None);
        await _store.SetAuthorizationIdAsync(token, "authz-1", CancellationToken.None);
        await _store.SetSubjectAsync(token, "bob", CancellationToken.None);
        await _store.SetStatusAsync(token, OpenIddictConstants.Statuses.Valid, CancellationToken.None);
        await _store.SetTypeAsync(token, OpenIddictConstants.TokenTypes.Bearer, CancellationToken.None);
        await _store.SetCreationDateAsync(token, creationDate, CancellationToken.None);
        await _store.SetExpirationDateAsync(token, expirationDate, CancellationToken.None);
        await _store.SetRedemptionDateAsync(token, redemptionDate, CancellationToken.None);
        await _store.SetPayloadAsync(token, "payload-bytes", CancellationToken.None);
        await _store.SetReferenceIdAsync(token, "ref-1", CancellationToken.None);

        Assert.Equal(token.Id, await _store.GetIdAsync(token, CancellationToken.None));
        Assert.Equal("app-1", await _store.GetApplicationIdAsync(token, CancellationToken.None));
        Assert.Equal("authz-1", await _store.GetAuthorizationIdAsync(token, CancellationToken.None));
        Assert.Equal("bob", await _store.GetSubjectAsync(token, CancellationToken.None));
        Assert.Equal(OpenIddictConstants.Statuses.Valid, await _store.GetStatusAsync(token, CancellationToken.None));
        Assert.Equal(OpenIddictConstants.TokenTypes.Bearer, await _store.GetTypeAsync(token, CancellationToken.None));
        Assert.Equal(creationDate, await _store.GetCreationDateAsync(token, CancellationToken.None));
        Assert.Equal(expirationDate, await _store.GetExpirationDateAsync(token, CancellationToken.None));
        Assert.Equal(redemptionDate, await _store.GetRedemptionDateAsync(token, CancellationToken.None));
        Assert.Equal("payload-bytes", await _store.GetPayloadAsync(token, CancellationToken.None));
        Assert.Equal("ref-1", await _store.GetReferenceIdAsync(token, CancellationToken.None));
    }

    [Fact]
    public async Task Properties_round_trip_through_get_and_set()
    {
        var token = new OpenIddictGroundworkToken();
        ...
        var readProperties = await _store.GetPropertiesAsync(token, CancellationToken.None);
        Assert.Equal("gold", readProperties["feature"].GetProperty("tier").GetString());
    }

    [Fact]
    public async Task Empty_properties_round_trip_to_an_empty_immutable_dictionary()
    {
        var token = new OpenIddictGroundworkToken();

        Assert.Empty(await _store.GetPropertiesAsync(token, CancellationToken.None));
    }
    ...
    {
        await _store.CreateAsync(new OpenIddictGroundworkToken { Subject = "bob" }, CancellationToken.None);
        await _store.CreateAsync(new OpenIddictGroundworkToken { Subject = "bob" }, CancellationToken.None);
        await _store.CreateAsync(new OpenIddictGroundworkToken { Subject = "alice" }, CancellationToken.None);

        var found = new List<OpenIddictGroundworkToken>();
        await foreach (var token in _store.FindBySubjectAsync("bob", CancellationToken.None))
            found.Add(token);

        Assert.Equal(2, found.Count);
        Assert.All(found, token => Assert.Equal("bob", token.Subject));
    }

    [Fact]
    public async Task FindByReferenceIdAsync_resolves_the_single_matching_token_through_the_declared_unique_route()
    {
        await _store.CreateAsync(new OpenIddictGroundworkToken { Subject = "bob", ReferenceId = "ref-1" }, CancellationToken.None);
        await _store.CreateAsync(new OpenIddictGroundworkToken { Subject = "alice", ReferenceId = "ref-2" }, CancellationToken.None);

        var found = await _store.FindByReferenceIdAsync("ref-1", CancellationToken.None);
    ...
    [Fact]
    public async Task FindByReferenceIdAsync_returns_null_when_no_token_matches()
    {
        var found = await _store.FindByReferenceIdAsync("missing", CancellationToken.None);
    ...
    public async Task FindAsync_subject_only_serves_the_same_result_as_FindBySubjectAsync()
    {
        ...
        var found = new List<OpenIddictGroundworkToken>();
        await foreach (var token in _store.FindAsync("bob", null, null, null, CancellationToken.None))
            found.Add(token);
        ...
        string? client, string? status, string? type)
    {
        ...
        {
            await foreach (var token in _store.FindAsync("bob", client, status, type, CancellationToken.None))
                _ = token;
        });

        Assert.Equal("token.FindAsync", exception.Operation);
    }
    ...
    public async Task FindAsync_rejects_an_entirely_unfiltered_predicate()
    {
        var exception = await Assert.ThrowsAsync<OpenIddictGroundworkCapabilityException>(async () =>
        {
            await foreach (var token in _store.FindAsync(null, null, null, null, CancellationToken.None))
                _ = token;
        });
    ...
    public async Task FindByApplicationIdAsync_is_always_rejected_because_no_route_is_declared()
    {
        var exception = await Assert.ThrowsAsync<OpenIddictGroundworkCapabilityException>(async () =>
        {
            await foreach (var token in _store.FindByApplicationIdAsync("app-1", CancellationToken.None))
                _ = token;
        });

        Assert.Equal("token.FindByApplicationIdAsync", exception.Operation);
    }

    [Fact]
    public async Task FindByAuthorizationIdAsync_is_always_rejected_because_no_route_is_declared()
    {
        var exception = await Assert.ThrowsAsync<OpenIddictGroundworkCapabilityException>(async () =>
        {
            await foreach (var token in _store.FindByAuthorizationIdAsync("authz-1", CancellationToken.None))
                _ = token;
        });

        Assert.Equal("token.FindByAuthorizationIdAsync", exception.Operation);
    }
    ...
    public async Task Update_is_a_successful_compare_and_swap_that_rotates_the_concurrency_token()
    {
        ...
        Assert.NotEqual(originalToken, token.ConcurrencyToken);
        var reloaded = await _store.FindByIdAsync(token.Id, CancellationToken.None);
        Assert.Equal(OpenIddictConstants.Statuses.Valid, reloaded!.Status);
        Assert.Equal(token.ConcurrencyToken, reloaded.ConcurrencyToken);
    }

    [Fact]
    public async Task Update_with_a_stale_persistence_version_raises_a_concurrency_exception()
    {
        var token = new OpenIddictGroundworkToken { Subject = "contested" };
        await _store.CreateAsync(token, CancellationToken.None);

        var staleCopy = await _store.FindByIdAsync(token.Id, CancellationToken.None);
        var freshCopy = await _store.FindByIdAsync(token.Id, CancellationToken.None);
    ...
    [Fact]
    public async Task DeleteAsync_removes_the_token()
    {
        var token = new OpenIddictGroundworkToken { Subject = "removable" };
        await _store.CreateAsync(token, CancellationToken.None);

        await _store.DeleteAsync(token, CancellationToken.None);

        Assert.Null(await _store.FindByIdAsync(token.Id, CancellationToken.None));
    }

    /// <summary>
    /// Count-all and list-all are rejected, not degraded, for the same reason as the other three stores: no
    /// bounded route is declared and the portable query surface is forbidden in production Groundwork reads
    /// by <c>ArchitectureGuardTests.Groundwork_production_reads_use_only_admitted_bounded_query_APIs</c>.
    /// </summary>
    [Fact]
    public async Task CountAsync_is_rejected_because_no_bounded_count_all_route_is_declared()
    {
        await _store.CreateAsync(new OpenIddictGroundworkToken { Subject = "one" }, CancellationToken.None);
        ...
        Assert.Equal("token.CountAsync", exception.Operation);
    }

    [Fact]
    public async Task ListAsync_is_rejected_because_no_bounded_list_all_route_is_declared()
    {
        await _store.CreateAsync(new OpenIddictGroundworkToken { Subject = "list-one" }, CancellationToken.None);

        var exception = await Assert.ThrowsAsync<OpenIddictGroundworkCapabilityException>(async () =>
        {
            await foreach (var token in _store.ListAsync(null, null, CancellationToken.None))
                _ = token;
        });

        Assert.Equal("token.ListAsync", exception.Operation);
    }
    ...
    {
        var exception = await Assert.ThrowsAsync<OpenIddictGroundworkCapabilityException>(() =>
            _store.CountAsync(tokens => tokens.Select(token => token.Id), CancellationToken.None).AsTask());
    ...
    public async Task GetAsync_generic_overload_fails_before_any_provider_work()
    {
        var exception = await Assert.ThrowsAsync<OpenIddictGroundworkCapabilityException>(() =>
            _store.GetAsync(
                    (tokens, state) => tokens.Where(token => token.Subject == state).Select(token => token.Id),
                    "irrelevant",
    ...
    /// <summary>
    /// No standalone creation-date or expiration-date route is declared for tokens, so prune has nothing to
    /// bound its selection with; this is rejected outright rather than scanning every document in memory.
    /// </summary>
    [Fact]
    public async Task PruneAsync_is_rejected_because_no_date_route_is_declared()
    {
        ...
        Assert.Equal("token.PruneAsync", exception.Operation);
    }
    ...
    public async Task RevokeByApplicationIdAsync_is_rejected_because_no_route_is_declared()
    {
        ...
        Assert.Equal("token.RevokeByApplicationIdAsync", exception.Operation);
    }

    [Fact]
    public async Task RevokeByAuthorizationIdAsync_is_rejected_because_no_route_is_declared()
    {
        var exception = await Assert.ThrowsAsync<OpenIddictGroundworkCapabilityException>(
            async () => await _store.RevokeByAuthorizationIdAsync("authz-1", CancellationToken.None));

        Assert.Equal("token.RevokeByAuthorizationIdAsync", exception.Operation);
    }
        ...
        string? client, string? status, string? type)
    {
        ...
        Assert.Equal("token.RevokeAsync", exception.Operation);
    }
    ...
    [Fact]
    public async Task RevokeBySubjectAsync_revokes_every_matching_token_and_returns_the_exact_count()
    {
        var first = new OpenIddictGroundworkToken { Subject = "bob", Status = OpenIddictConstants.Statuses.Valid };
        var second = new OpenIddictGroundworkToken { Subject = "bob", Status = OpenIddictConstants.Statuses.Valid };
        var other = new OpenIddictGroundworkToken { Subject = "alice", Status = OpenIddictConstants.Statuses.Valid };
    ...
    {
        var token = new OpenIddictGroundworkToken { Subject = "bob", Status = OpenIddictConstants.Statuses.Valid };
        await _store.CreateAsync(token, CancellationToken.None);

        var revoked = await _store.RevokeAsync("bob", null, null, null, CancellationToken.None);

        Assert.Equal(1, revoked);
        Assert.Equal(OpenIddictConstants.Statuses.Revoked, (await _store.FindByIdAsync(token.Id, CancellationToken.None))!.Status);
    }

    /// <summary>
    /// Proves the exactly-once property end to end through the store's own surface (spec 106 T030): a second
    /// call for the SAME subject, after the one matching token is already revoked, must not perform a
    /// second underlying save for it - the atomic wrapper replays the first call's receipt for that token id
    /// instead of re-executing the load/save.
    /// </summary>
    [Fact]
    public async Task RevokeBySubjectAsync_replay_returns_the_same_outcome_without_a_second_underlying_write()
    {
        var token = new OpenIddictGroundworkToken { Subject = "bob", Status = OpenIddictConstants.Statuses.Valid };
        await _store.CreateAsync(token, CancellationToken.None);
        var saveCountBeforeRevoke = _documentStore.TokenSaveCount;

        var first = await _store.RevokeBySubjectAsync("bob", CancellationToken.None);
        var saveCountAfterFirstRevoke = _documentStore.TokenSaveCount;
        var replay = await _store.RevokeBySubjectAsync("bob", CancellationToken.None);
        var saveCountAfterReplay = _documentStore.TokenSaveCount;
    ...
    /// <c>OpenIddictGroundworkAtomicWriteTests.AtomicWrite</c> and
    /// <c>GroundworkOpenIddictAuthorizationStoreTests.AtomicWrite</c> so this suite exercises the revoke
    /// family exactly as production wiring would.
    /// </summary>
    ...
    private sealed class StubGroundworkStoreSessionFactory(FakeGroundworkDocumentStore store) : IGroundworkStoreSessionFactory
    {
        ...
        public ValueTask<TResult> ExecutePrivilegedAsync<TResult>(
            ...
            CancellationToken cancellationToken = default) =>
            throw new NotSupportedException("Not exercised by the token-store suite.");
    ...
    /// unit-of-work commit/rollback semantics, mirroring
    /// <c>GroundworkOpenIddictAuthorizationStoreTests.FakeGroundworkDocumentStore</c>: point load/save/
    /// delete, and bounded query with <c>Equal</c> (both the reference-id point route and the subject cursor
    /// route resolve through the same <c>Equal</c> comparison).
    /// </summary>
    private sealed class FakeGroundworkDocumentStore : IDocumentStore, IBoundedDocumentStore
    {
        ...
        public int TokenSaveCount { get; private set; }
        ...
        {
            SaveCount++;
            if (request.DocumentKind == OpenIddictGroundworkJson.TokenDocumentKind)
                TokenSaveCount++;
...
#pragma warning disable GW0004
        public Task<IReadOnlyList<DocumentEnvelope>> QueryAsync(DocumentStoreQuery query, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException("Not exercised by the token store.");
