...
public sealed class OutboxDrainAdapterTests
{
    ...
    public async Task Runs_frozen_correctness_and_prepares_operations_over_real_sqlite_runtime_stores()
    {
        ...
        var database = Path.Combine(root, "outbox.db");
        ...
        try
        {
            ...
            await using var adapter = new OutboxDrainAdapter(request, connectionString, root);
            ...
            Assert.Equal(RuntimeOutboxDrainWorkload.ExpectedResultDigest, correctness.ObservedResultDigestSha256);
    ...
    private static RunRequest Request() => new(
        ...
        WorkloadId: RuntimeOutboxDrainWorkload.WorkloadId,
        ...
        PhysicalForm: OutboxDrainAdapter.PhysicalForm,
        Scale: "small",
        ...
        Seed: RuntimeOutboxDrainWorkload.Seed,
        InputFingerprintSha256: RuntimeOutboxDrainWorkload.ExpectedInputFingerprint,
        NativePlanIdentity: "outbox-drain-test-provenance",
        NativePlanEvidenceReference: "outbox-drain.sqlite.set.native-plan.json",
