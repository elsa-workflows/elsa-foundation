...
public sealed class Wave8PublishingMinimalApiCollectibilityTests
{
    ...
    private static async Task<Evidence> CreateAndReleaseAsync(int cycle)
    {
        ...
        serviceDescriptors.Replace(ServiceDescriptor.Singleton<IWorkflowActivationAuthority, CollectibilitySlotStore>());
    ...
    /// <summary>
    /// T117 moved the definition-keyed authority out of publishing under neutral runtime names, so this probe now
    /// implements <see cref="IWorkflowActivationAuthority"/>. Its job is unchanged: observe construction and
    /// disposal so the collectibility assertions can prove the module unloads.
    /// </summary>
    private sealed class CollectibilitySlotStore(CollectibilityProbeState observations) : IWorkflowActivationAuthority, IDisposable
    {
        private static WorkflowActivationSlot Slot =>
            new("collectibility-slot", "definition", "default", null, null, 0, DateTimeOffset.UtcNow);
        ...
        public ValueTask<WorkflowActivationSlot?> FindAsync(string workflowDefinitionId, string slotName, CancellationToken cancellationToken = default)
        {
            ...
            return ValueTask.FromResult<WorkflowActivationSlot?>(Slot);
        }
        ...
        {
            observations.SlotCalls++;
            return ValueTask.FromResult<IReadOnlyCollection<WorkflowActivationSlot>>([Slot]);
        }
        ...
        public ValueTask<WorkflowActivationTransition> TryActivateAsync(WorkflowActivationSlotRequest request, CancellationToken cancellationToken = default) =>
            ValueTask.FromResult(new WorkflowActivationTransition(
                true,
                Slot with { ActiveActivationId = request.ActivationId, Source = request.Source, Revision = request.ExpectedRevision + 1 }));
        ...
        public ValueTask<WorkflowActivationTransition> TryDeactivateAsync(
            ...
            ValueTask.FromResult(new WorkflowActivationTransition(true, Slot with { Revision = expectedRevision + 1 }));
