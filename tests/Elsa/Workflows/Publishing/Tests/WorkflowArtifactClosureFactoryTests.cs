...
public sealed class WorkflowArtifactClosureFactoryTests
{
    ...
    public async Task Roots_at_the_named_version_when_a_neighbouring_version_is_also_published()
    {
        ...
        // The neighbouring version is published first so a broad source-reference read cannot accidentally
        // choose it when both versions are present.
        ...
        Assert.Equal(target.Identity.ArtifactId, Assert.Single(closure.SourceReferences).ArtifactId);
    }
