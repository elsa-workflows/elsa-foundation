...
public sealed class ActivityUpgradeGroundworkTests
{
    ...
    {
        await using var harness = await Harness.CreateAsync(workflowExpectedRevision: 99);
    ...
    {
        await using var persistence = CreateCatalog();
        ...
        var projection = new ActivityDependencyProjection(ActivityStore(persistence), versions);
    ...
    {
        await using var persistence = CreateCatalog();
        var receipts = new DurableUpgradePlanStore(ActivityStore(persistence));
    ...
    {
        await using var harness = await Harness.CreatePlannerGeneratedAsync();
    ...
    {
        await using var harness = await Harness.CreateAsync(1);
    ...
    private sealed class Harness(
        ...
        ActivityUpgradePlan plan) : IAsyncDisposable
    {
        private GroundworkV2TestPersistence Persistence { get; } = persistence;
        private GroundworkV2ActivityDesignStore ActivityStore { get; } = activityStore;
        private GroundworkDesignStorage WorkflowStorage { get; } = workflowStorage;
        ...
        {
            var persistence = CreateCatalog();
            var access = persistence.Access();
            var activityStore = new GroundworkV2ActivityDesignStore(persistence.Sessions, access);
            var workflowStorage = new GroundworkDesignStorage(persistence.Sessions, access);
            // The subject commits through the session source, so the transaction-begin hook belongs there.
            // Seeding stays on the raw source so the hook only observes the apply under test.
            var subjectSessions = onTransactionBegin is null
                ? persistence.Sessions
                : new BeginActionSessionSource(persistence.Sessions, onTransactionBegin);
            var subjectActivityStore = new GroundworkV2ActivityDesignStore(subjectSessions, access);
            var subjectWorkflowStorage = new GroundworkDesignStorage(subjectSessions, access);
            var subjectPublishingStorage = new GroundworkPublishingStorage(subjectSessions, access);
            ...
            var projection = new ActivityDependencyProjection(activityStore, versions);
            // Seeding runs this writer too, so it stays on the unhooked store: the hook must observe the
            // apply under test, not the fixture being built.
            ...
            await SaveActivityAsync(activityStore, ActivitiesDesignStorageManifest.ActivityDefinitionDocumentKind, ActivitiesDesignStorageManifest.ActivityDefinitionCollection, Definition());
            await SaveActivityAsync(activityStore, ActivitiesDesignStorageManifest.ActivityDefinitionDraftDocumentKind, ActivitiesDesignStorageManifest.ActivityDefinitionDraftCollection, ActivityDraft());
            await SaveActivityAsync(activityStore, ActivitiesDesignStorageManifest.ActivityDefinitionDraftLayoutDocumentKind, ActivitiesDesignStorageManifest.ActivityDefinitionDraftLayoutCollection, ActivityLayout());
            await SaveActivityAsync(activityStore, ActivitiesDesignStorageManifest.ActivityDefinitionAuthoringStateDocumentKind, ActivitiesDesignStorageManifest.ActivityDefinitionAuthoringStateCollection, Authoring());
            await SaveActivityAsync(activityStore, ActivitiesDesignStorageManifest.ActivityDefinitionVersionPublicationDocumentKind, ActivitiesDesignStorageManifest.ActivityDefinitionVersionPublicationCollection, from);
            await SaveActivityAsync(activityStore, ActivitiesDesignStorageManifest.ActivityDefinitionVersionPublicationDocumentKind, ActivitiesDesignStorageManifest.ActivityDefinitionVersionPublicationCollection, to);
            ...
            await activityStore.SaveAsync(ToActivitySave(
                ...
                new ApplyReceiptDocument(
                    ...
                    Receipt(plan)),
                GroundworkActivitiesDesignJson.Options));
            // A workflow draft is a workflow-design row: its unit declares different columns from the
            // activity lane's, so it has to be written through that lane's own storage.
            workflowStorage.Upsert(WorkflowDraftUnit, WorkflowDraftValues(WorkflowDraft(), workflowJson));
            ...
            var activityState = new DocumentActivityStateStore(activityStore);
            var subject = new PublishingUpgradePlanStore(
                subjectActivityStore,
                subjectWorkflowStorage,
                subjectPublishingStorage,
                payloads,
                projection,
                projection,
                activityState,
                ...
                new ImmediateLockProvider());
            return new(persistence, activityStore, workflowStorage, workflowJson, payloads, versions, workflowVersions, projection, subject, plan);
        }
        ...
        public static async Task<Harness> CreatePlannerGeneratedAsync(Func<Task>? onTransactionBegin = null)
        {
            ...
            var durable = new DurableUpgradePlanStore(seeded.ActivityStore);
            ...
            return new(
                seeded.Persistence,
                seeded.ActivityStore,
                seeded.WorkflowStorage,
                seeded.WorkflowJson,
                seeded.Payloads,
                seeded.Versions,
        ...
        public async Task<ActivityDefinitionDraft> LoadActivityDraftAsync()
        {
            ...
            return JsonSerializer.Deserialize<GroundworkV2ActivityDesignDocument<ActivityDefinitionDraft>>(envelope!.ContentJson, GroundworkActivitiesDesignJson.Options)!.Entity;
        }

        public Task<(WorkflowDefinitionDraft Entity, long EnvelopeVersion)> LoadWorkflowDraftAsync()
        {
            var entry = workflowStorage.Read(WorkflowDraftUnit, "workflow-draft")!;
            return Task.FromResult((workflowStorage.MapDraft(entry, workflowJson), workflowStorage.Version(entry)!.Value));
        }
        ...
        {
            var durable = new DurableUpgradePlanStore(ActivityStore);
        ...
        public async Task<ActivityDependencyProjectionState> LoadProjectionAsync()
        {
            ...
            return JsonSerializer.Deserialize<GroundworkV2ActivityDesignDocument<ActivityDependencyProjectionState>>(
                envelope!.ContentJson,
        ...
        public Task ChangeWorkflowDependencyOutsidePublishingAsync(string versionId)
        {
            var draft = ReadWorkflowDraft(out var version);
            draft.State = draft.State with { RootActivity = draft.State.RootActivity! with { ActivityVersionId = versionId } };
            WriteWorkflowDraftOutsidePublishing(draft, version);
            return Task.CompletedTask;
        }

        public Task ChangeWorkflowStateOutsidePublishingAsync(WorkflowDefinitionState state)
        {
            var draft = ReadWorkflowDraft(out var version);
            draft.State = state;
            WriteWorkflowDraftOutsidePublishing(draft, version);
        ...
        private WorkflowDefinitionDraft ReadWorkflowDraft(out long version)
        {
            var entry = workflowStorage.Read(WorkflowDraftUnit, "workflow-draft")!;
            version = workflowStorage.Version(entry)!.Value;
            return workflowStorage.MapDraft(entry, workflowJson);
        }

        /// <summary>An edit landing on the row behind publishing's back, at the revision it last observed.</summary>
        private void WriteWorkflowDraftOutsidePublishing(WorkflowDefinitionDraft draft, long version) =>
            workflowStorage.ConditionalUpsert(
                WorkflowDraftUnit,
                WorkflowDraftValues(draft, workflowJson),
        ...
        public async Task SaveVersionLifecycleAsync(string versionId)
        {
            var envelope = await activityStore.LoadAsync(
                ...
                versionId);
            // A losing CAS throws here rather than reporting a status, which is the assertion this replaces.
            await activityStore.SaveAsync(GroundworkV2ActivityDesignDocumentWriter.ToSaveRequest(
                ActivitiesDesignStorageManifest.ActivityDefinitionVersionPublicationDocumentKind,
                ActivitiesDesignStorageManifest.ActivityDefinitionVersionPublicationCollection,
                ActivitiesDesignStorageManifest.SchemaVersion,
                versions.FindRequired(versionId),
                GroundworkActivitiesDesignJson.Options) with { ExpectedVersion = envelope!.Version });
        }
    ...
    /// </summary>
    private const string WorkflowDraftUnit = WorkflowsDesignStorageManifest.WorkflowDefinitionDraftDocumentKind;

    private static StorageValues WorkflowDraftValues(WorkflowDefinitionDraft draft, JsonSerializerOptions workflowJson) =>
    ...
    private static GroundworkV2ActivityDesignStore ActivityStore(GroundworkV2TestPersistence persistence) =>
        new(persistence.Sessions, persistence.Access());

    /// <summary>
    /// The plan, receipt and workflow-draft rows are wrapper records rather than entities, so they cannot go
    /// through the entity writer; they carry the same envelope shape and are serialized as they are.
    /// </summary>
    private static ActivityDesignSaveRequest ToActivitySave<TDocument>(
        string documentKind,
        string id,
        ...
        new(documentKind, id, schemaVersion, JsonSerializer.Serialize(document, jsonOptions), expectedVersion);
    ...
    private sealed class DocumentActivityStateStore(GroundworkV2ActivityDesignStore activityStore) :
    ...
    /// <summary>
    /// The v2 equivalent of the old store's transaction-begin hook: a lane opens its transaction through the
    /// session source, so that is where an interleaved writer has to be injected. Fires once, as before.
    /// </summary>
    private sealed class BeginActionSessionSource(IGroundworkStorageSessionSource inner, Func<Task> action)
        : IGroundworkStorageSessionSource, IGroundworkStorageCapabilitySource
    {
        private int injected;

        // Publishing refuses to stage without an evidenced atomic commit, so a wrapper that hid the
        // capability seam would turn every apply into a NotSupportedException instead of the case under test.
    ...
    private sealed class Ids : Elsa.Primitives.Contracts.IIdentityGenerator
    {
        private int _id;
