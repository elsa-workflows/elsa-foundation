...
using Elsa.Workflows.Publishing.Api.Tests;
...
public sealed class ActivityDraftTestRunTests
{
    ...
    public async Task Groundwork_sqlite_graph_run_suspends_restarts_in_runtime_only_host_then_reopens_status()
    {
        ...
        // A file-backed catalog, reopened between generations: that reopen is the process restart this
        // test is about, and it is what proves the run survives in the database rather than in the host.
        await using var persistence = CreateCatalog("sqlite");
        var authoring = AuthoringState.Create();
...
, IsBoundaryInput);
        ...
        Func<IWorkflowExecutableSourceReferenceStore, IWorkflowExecutableSourceReferenceStore>? decorateSourceReferences = null)
    {
        ...
        services.AddSingleton<IActivityExecutionInspectionAuthorizationContext>(
            new ScopedInspectionContext(persistenceScope ?? DefaultScope));
        ...
        // After the store registrations, which install the shell's own provider-backed source: the last
        // singleton registration is the one that resolves, and the catalog's is the one under test.
        ...
        IExternalPayloadStore? externalPayloadStore = null)
    {
        ...
        services.AddSingleton(externalPayloadStore ?? new InMemoryExternalPayloadStore());
        services.AddSingleton<IActivityExecutionInspectionAuthorizationContext>(
            new ScopedInspectionContext(DefaultScope));
        new WorkflowsRuntimeApiFeature().ConfigureServices(services);
        new ActivitiesRuntimeFeature().ConfigureServices(services);
        new GraphActivitiesRuntimeFeature().ConfigureServices(services);
        services.AddGroundworkV2RuntimeStores();
        services.AddSingleton<IGroundworkStorageSessionSource>(persistence.Sessions);
        services.Replace(ServiceDescriptor.Scoped<IPersistenceAccessContextAccessor>(
            _ => persistence.Access(DefaultScope)));
    ...
    /// the query's tenant scope to name the persistence scope its session is opened in. This grants the same
    /// blanket permissions and reports a scope that agrees with the catalog.
    /// </summary>
#pragma warning disable CS0618 // The scoped adapter implements the obsolete host contract, as the shipped one does.
    private sealed class ScopedInspectionContext(string scope) :
        IActivityExecutionInspectionAuthorizationContext,
        IActivityInspectionContextAsync
    {
        public string TenantScope { get; } = $"tenant:{scope}";
        public string AuthorizationProfile => "structure+values";
        public string AuditSubject => "scoped-test";
...
#pragma warning restore CS0618
    ...
    /// <summary>
    /// A test run writes runtime material and a publishing receipt in one transaction, so both lanes are
    /// declared against one provider, exactly as a single-target host has them.
    /// </summary>
    private static GroundworkV2TestPersistence CreateCatalog(string provider = "memory") =>
        GroundworkV2TestPersistence.Create(
            provider,
