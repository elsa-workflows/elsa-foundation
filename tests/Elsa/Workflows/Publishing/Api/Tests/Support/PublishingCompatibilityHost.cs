...
public sealed class PublishingCompatibilityHost(IHost host) : IAsyncDisposable
{
    ...
    public static async Task<PublishingCompatibilityHost> StartAsync()
    {
        ...
        var slotStore = host.Services.GetRequiredService<IWorkflowActivationAuthority>();
        // T117: the definition-keyed authority replaced IPublicationSlotStore under neutral runtime
        // names. Publishing owns the slot here, so the request declares a publishing source.
        await slotStore.TryActivateAsync(new WorkflowActivationSlotRequest(
            "definition-route",
            "default",
            "publication-capture",
            new WorkflowActivationSource("publishing", null),
            ExpectedRevision: 0,
