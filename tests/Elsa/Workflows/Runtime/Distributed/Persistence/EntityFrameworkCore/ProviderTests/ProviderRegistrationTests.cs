...
public sealed class ProviderRegistrationTests
{
    ...
    [Theory]
    [InlineData("Sqlite", typeof(ExecutionCommandTransportSqliteDbContext), ExecutionCommandTransportSqliteDbContext.ExpectedProviderName, "Data Source=:memory:")]
    [InlineData("SqlServer", typeof(ExecutionCommandTransportSqlServerDbContext), ExecutionCommandTransportSqlServerDbContext.ExpectedProviderName, "Server=localhost;Database=unused;User Id=sa;Password=Pass@word1;TrustServerCertificate=True")]
    [InlineData("PostgreSql", typeof(ExecutionCommandTransportPostgreSqlDbContext), ExecutionCommandTransportPostgreSqlDbContext.ExpectedProviderName, "Host=localhost;Database=unused;Username=unused;Password=unused")]
    [InlineData("MySql", typeof(ExecutionCommandTransportMySqlDbContext), ExecutionCommandTransportMySqlDbContext.ExpectedProviderName, "Server=localhost;Database=unused;User Id=unused;Password=unused")]
    public void Every_provider_registration_builds_and_resolves_its_selected_command_transport_context_and_store(
        string providerName,
        Type expectedContextType,
        string expectedProviderName,
        string connectionString)
    {
        ...
        Assert.IsType(expectedContextType, context);
        Assert.Equal(expectedProviderName, context.Database.ProviderName);
        Assert.IsType<EfExecutionCommandTransport>(scope.ServiceProvider.GetRequiredService<IExecutionCommandTransport>());
    }
}
