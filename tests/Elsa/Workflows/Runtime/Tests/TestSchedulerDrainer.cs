...
/// the historic positional shape <c>(queue, handlers, timeProvider, pauseGate, ambientAccessor, stateStore, ...)</c> and
/// defaults the state store to a fresh empty <see cref="InMemoryWorkflowExecutionStateStore"/> when a test does not
/// supply one — behaviour-identical to the pre-RT-8 no-store path (an empty store reports no terminal execution, so the
