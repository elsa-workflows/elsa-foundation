...
public sealed class ClrReconciliationTests
{
    ...
    public async Task FolderSource_ReconcilesAuthorControlledVersions_AndIsIdempotent()
    {
        ...
        // The fixture assembly carries ten concrete activities, including the three input-options
        // fixtures added for option metadata coverage (the abstract bases are not activity types).
        Assert.Equal(10, store.Versions.Count);
    ...
    public async Task BumpedVersion_AppendsNewRow_RetainingTheOldOne()
    {
        ...
        // First pass via the real CLR source persists all ten concrete fixture activities (ten rows).
