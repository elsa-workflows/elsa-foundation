...
public sealed class ClrReconciliationTests
{
    ...
    public async Task FolderSource_ReconcilesAuthorControlledVersions_AndIsIdempotent()
    {
        ...
        // The fixture assembly carries seven concrete activities: UnannotatedFixtureActivity,
        // VersionedFixtureActivity, ComplexInputFixtureActivity, InheritedVersionFixtureActivity, and
        // InheritsRequiredFixtureActivity, StructuredFixtureActivity, and TriggerFixtureActivity
        // (the abstract bases are not activity types).
        Assert.Equal(7, store.Versions.Count);
    ...
    public async Task BumpedVersion_AppendsNewRow_RetainingTheOldOne()
    {
        ...
        // First pass via the real CLR source persists all seven concrete fixture activities (seven rows).
        ...
        Assert.Equal(8, store.Versions.Count);
