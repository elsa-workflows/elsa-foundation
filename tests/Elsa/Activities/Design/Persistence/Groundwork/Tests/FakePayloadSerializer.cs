...
/// authored projection (de)serialization to the host's payload serializer (rather than serializing the
/// collections inline), which is the contract that matters here; the production serializer's polymorphic
/// expression handling is exercised by the serializer's own tests and by composition-level tests.
/// </summary>
