...
public sealed class FlowchartExecutionDeterminismCharacterizationTests
{
    ...
    // serializer's own output — pinned literally. Re-frozen for W32: completed loop-iteration scopes
    // scope:3 and scope:13 are pruned, while scope:24 remains because the last durable document still has
    // an active path in that iteration. The path/scope/arrival/diag id stream through
    // sequence:74 is unchanged — id determinism is preserved through the last durable private-state document.
