...
/// <summary>
/// Local copy of the CLR activity construction test helper (the original lives in the Runtime tests assembly and
/// is <c>internal</c>, and the sibling Http unit-test assembly keeps its own copy). Builds the stable-alias
/// descriptor payload the production <c>ClrActivityConstructor</c> consumes, so the HTTP activities are
/// constructed through the same descriptor → construct → bind → execute path the real host uses.
/// </summary>
